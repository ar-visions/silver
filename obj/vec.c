...
Vec Vec_with(Class cself, Class c, ...) {
...
Vec Vec_with_count(Class cself, int count) {
    Vec self = (Vec)new_obj((class_Base)Vec_cl, sizeof(double) * max(0, count - 2));
    return autorelease(self);
}

Vec Vec_from_cstring(Class cself, const char *value) {
    if (!value)
        return NULL;
...
String Vec_to_string(Vec self) {
    char *buf = (char *)calloc(1, 64 + 64 * self->count);
    int len = 0;
    buf[0] = 0;
    for (int i = 0; i < self->count; i++) {
