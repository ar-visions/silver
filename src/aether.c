...
// Active codegen instance. During build_fn, codegen helpers resolve the
// emitting instance (its builder/lexical/stack) from here instead of
// operand->mod. A worker clone shares context/module/registry/autype with the
// original, so shared operands carry mod=original; without this they would emit
// through the original's (unpositioned) builder. Thread-local so each worker
// thread routes to its own instance. null outside build_fn -> fall back to ->mod.
__thread aether au_codegen_active = null;
AU_EXPORT aether au_codegen_active_get(void)     { return au_codegen_active; }
AU_EXPORT void   au_codegen_active_set(aether a) { au_codegen_active = a; }

static void restore_codegen_active(aether* prev) { au_codegen_active = *prev; }

// off-thread, a worker clone must resolve back to the root — following a
// node's mod into another core's context emits into the wrong module
AU_EXPORT aether au_active(aether from) {
    if (au_codegen_active) return au_codegen_active;
    return (from && from->root) ? from->root : from;
}

// emission locks only the context it touches, so cores never contend.
// a type mint writes every context, so it takes them all — after dropping
...
static __thread int _ctx_saved  = 0;

__attribute__((constructor)) static void _ctx_mtx_init() {
    pthread_mutexattr_t at;
    pthread_mutexattr_init(&at);
    pthread_mutexattr_settype(&at, PTHREAD_MUTEX_RECURSIVE);
    for (int i = 0; i < AU_CORES; i++)
        pthread_mutex_init(&_ctx_mtx[i], &at);
}

static inline void emit_lock() {
    if (_ctx_depth++ == 0) {
        _ctx_core = au_codegen_active ? au_codegen_active->core : 0;
        pthread_mutex_lock(&_ctx_mtx[_ctx_core]);
    }
}
static inline void emit_unlock() {
    if (_ctx_depth > 0 && --_ctx_depth == 0)
        pthread_mutex_unlock(&_ctx_mtx[_ctx_core]);
}

static inline void type_lock() {
    if (_type_depth++ == 0) {
        _ctx_saved = _ctx_depth;
        if (_ctx_depth) { _ctx_depth = 0; pthread_mutex_unlock(&_ctx_mtx[_ctx_core]); }
        pthread_mutex_lock(&_type_mtx);
        for (int i = 0; i < AU_CORES; i++) pthread_mutex_lock(&_ctx_mtx[i]);
    }
}
static inline void type_unlock() {
    if (_type_depth > 0 && --_type_depth == 0) {
        for (int i = AU_CORES - 1; i >= 0; i--) pthread_mutex_unlock(&_ctx_mtx[i]);
        pthread_mutex_unlock(&_type_mtx);
        if (_ctx_saved) {
            pthread_mutex_lock(&_ctx_mtx[_ctx_core]);
            _ctx_depth = _ctx_saved;
        }
    }
}

// exported for lldb.c / coverage.c (same DSO)
void aether_emit_lock  () { emit_lock();   }
void aether_emit_unlock() { emit_unlock(); }
static void _emit_guard_end(char* g) { (void)g; emit_unlock(); }
static void _type_guard_end(char* g) { (void)g; type_unlock(); }
...
static inline int  emit_suspend() {
    int d = _ctx_depth;
    if (d) { _ctx_depth = 0; pthread_mutex_unlock(&_ctx_mtx[_ctx_core]); }
    return d;
}
static inline void emit_resume(int d) {
    if (d) { pthread_mutex_lock(&_ctx_mtx[_ctx_core]); _ctx_depth = d; }
}

// longjmp (halt) skips cleanup handlers: reset from catch sites
AU_EXPORT void aether_emit_recover() {
    while (_type_depth) type_unlock();
    if (_ctx_depth) { _ctx_depth = 1; emit_unlock(); }
}

AU_EXPORT Au subprocedure_invoke(subprocedure a, Au arg);

// deferred-parse builders run without the lock held
static Au sp_invoke(subprocedure s, Au arg) {
    int d = emit_suspend();
    Au  r = subprocedure_invoke(s, arg);
    emit_resume(d);
    return r;
}

// Parallel codegen core index (0 = main/serial). Each codegen worker is a clone
// with its own LLVMContext and its own `core` field; context-bound TYPE handles
// are cached per core in etype->types (indexed by `core`). The index comes off
// the *active* instance (au_active routes to the worker), so no thread-local is
// needed. Nothing keys off the LLVMTypeRef → the N slots are fully independent.
#define AU_MAX_WORKERS 64
#define AU_CORES       8
#define au_core(e) (au_active((e)->mod)->core)

AU_EXPORT void        _lltype_set(etype e, LLVMTypeRef v);
LLVMTypeRef _lltype_slot(etype e);
LLVMTypeRef _lltype_core(etype e, int core);

// one struct field, described so every context can build its own copy
typedef struct ll_field {
    etype        e;        // source type (unused when bits or percore is set)
    LLVMTypeRef* percore;  // pre-built type, one per core
    int          bits;     // integer width, when there is no etype
    int          elements; // array length, when arr is set
    bool         arr;      // wrap the field in an array of `elements`
    bool         raw;      // use the type's own slot (no class → ptr walk)
} ll_field;

AU_EXPORT LLVMTypeRef ll_field_at(aether a, ll_field* f, int core);

AU_EXPORT int               core_index(Au o);
AU_EXPORT LLVMTargetDataRef ll_td(aether a);
AU_EXPORT LLVMBasicBlockRef _llentry      (enode n);
AU_EXPORT void              ll_const_int (aether a, enode n, etype t, unsigned long long v, bool sext);
AU_EXPORT void              ll_const_real(aether a, enode n, etype t, double v);
AU_EXPORT void              ll_const_null(aether a, enode n, etype t);
AU_EXPORT void              ll_func      (aether a, enode n, etype ft, symbol name);
AU_EXPORT void              ll_param     (aether a, enode n, enode fn, int index);
AU_EXPORT void              ll_const_gep (aether a, enode n, etype st, enode base, int index);
AU_EXPORT void              ll_entry     (aether a, enode n, symbol label);
AU_EXPORT void              ll_linkage   (aether a, enode n, LLVMLinkage linkage);
void              ll_global    (aether a, enode n, etype t, symbol name,
                        bool external, bool reuse, bool set_init);
AU_EXPORT etype             etype_prep   (aether a, Au_t au);
AU_EXPORT void              lldebug_set  (etype e, LLVMMetadataRef v);
AU_EXPORT LLVMMetadataRef   _lldebug     (etype e);
AU_EXPORT void              llscope_set  (etype e, LLVMMetadataRef v);
LLVMMetadataRef   _llscope     (etype e);
AU_EXPORT void              lldbg_set    (enode n, LLVMMetadataRef v);
...
AU_EXPORT void etype_register(aether a, Au key, Au value, bool overwrite) {
    type_guard;
    if (!overwrite) {
    ...
    if (m && !tm) {
        ...
        if (mt == AU_MEMBER_ENUMV) {
            ...
            if (c_enum && m->value) {
                ...
                enode n = enode(mod, a, autype, m, literal, lit, loaded, true);
                ll_const_int(a, n, etype_prep(a, typeid(i32)), (unsigned long long)ival, true);
                etype_register(a, (Au)m, (Au)hold(n), false);
                return (etype)n;
            }
        }
        else if (is_func((Au)m) || is_func_ptr((Au)m)) {
            return (etype)efunc(mod, a, loaded, true, autype, m);
        }
        else if (mt == AU_MEMBER_VAR) {
            enode n = enode(mod, a, loaded, false, autype, m);
            etype_register(a, (Au)m, (Au)hold(n), false);
            return (etype)n;
        } else {
            etype t = etype(mod, a, autype, m); // this should be held in it's init
            return t;
        }
    }
    return tm;
...
AU_EXPORT etype etype_prep(aether a, Au_t au) { sequencer
    if (!au) return null;
...
AU_EXPORT token aether_loc(aether a) {
    emit_guard;
    if (!a->tokens || !len(a->tokens)) return token("[none]");
    // the fault is on what was read, not on the token after it
    i64 i = (i64)a->cursor - 1;
...
// construct, then place the value in this core's slot
#define with_value(V, CTOR) ({ \
    enode _wv_ = (enode)(CTOR); \
    _llvalue_set(_wv_, (V)); \
    _wv_; \
})

#define int_value(b,l) \
    with_value(LLVMConstInt(lltype(etype_prep(a, typeid(i##b))), *(i##b*)l, 0), \
        enode(mod, a, literal, l, autype, typeid(i##b), loaded, true))

#define uint_value(b,l) \
    with_value(LLVMConstInt(lltype(etype_prep(a, typeid(u##b))), *(u##b*)l, 0), \
        enode(mod, a, literal, l, autype, typeid(u##b), loaded, true))

#define f32_value(b,l) \
    with_value(LLVMConstReal(lltype(etype_prep(a, typeid(f##b))), *(f##b*)l), \
        enode(mod, a, literal, l, autype, typeid(f##b), loaded, true))
...
#define unichar_value(l) \
    with_value(LLVMConstInt(lltype(etype_prep(a, typeid(unichar))), *(unichar*)l, 0), \
        enode(mod, a, literal, l, autype, typeid(unichar), loaded, true))


#define value(m,vr) with_value(vr, enode(mod, a, autype, (m)->autype, loaded, true))
...
AU_EXPORT LLVMMetadataRef debug_scope(aether a) {
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
        etype ctx_u = u(etype, ctx);
        if (ctx_u && _llscope(ctx_u))
            return _llscope(ctx_u);
    }
    return a->compile_unit;
}

AU_EXPORT LLVMMetadataRef debug_type_for      (aether a, Au_t src);
AU_EXPORT LLVMMetadataRef debug_struct_type   (aether a, Au_t type_au);
AU_EXPORT LLVMMetadataRef debug_enum_type     (aether a, Au_t type_au);
AU_EXPORT LLVMMetadataRef debug_pointer_type  (aether a, Au_t type_au);
AU_EXPORT LLVMMetadataRef debug_funcptr_type  (aether a, Au_t type_au);
AU_EXPORT LLVMMetadataRef debug_subroutine_type(aether a, Au_t fn_au);
AU_EXPORT void            emit_debug_function (aether a, efunc fn, bool);
AU_EXPORT void            update_current_file (aether a, path source_file);
AU_EXPORT void            emit_debug_variable (aether a, enode var, u32 arg_no, u32 line);
AU_EXPORT LLVMMetadataRef debug_create_lexical_block(aether a, u32 line, u32 column);
AU_EXPORT LLVMMetadataRef debug_au_header_type(aether a, Au_t schema);
AU_EXPORT void emit_debug_params              (aether a, efunc fn);
...
AU_EXPORT void report_coverage(aether a);
...
AU_EXPORT void emit_debug_loc(aether a, cstr src, u32 line, u32 column) {
    emit_guard;
...
AU_EXPORT LLVMValueRef LLVMFetchGlobal(LLVMModuleRef module_ref, LLVMTypeRef type, cstr name) {
    emit_guard;
...
AU_EXPORT LLVMValueRef entry_alloca(aether a, LLVMTypeRef ty, cstr name) {
    emit_guard;
...
AU_EXPORT etype etype_copy(etype mem) {
    Au_t au = isa(mem);
...
AU_EXPORT enode enode_ref(aether a, enode expr, etype ref_type) {
    emit_guard;
    LLVMValueRef v = _llvalue(expr);
    if (is_func((Au)expr->autype)) {
        ...
        if (!_llvalue((enode)fn) && !a->no_build) {
            // extern C fn never called directly: declare it now
            cstr n = expr->autype->alt ? expr->autype->alt
                                       : expr->autype->ident;
            LLVMValueRef fv = LLVMGetNamedFunction(a->module_ref, n);
            if (!fv && _lltype_slot((etype)fn))
                fv = LLVMAddFunction(a->module_ref, n,
...
// the vector class by identity, or by name up the class chain: a
// module that imports Au carries its own Au_t for the runtime class
AU_EXPORT bool au_is_vector(Au_t t) {
    if (!t) return false;
    if (inherits(t, typeid(vector))) return true;
    for (Au_t c = t; c; c = c->context) {
        if (c->is_class && c->ident && strcmp(c->ident, "vector") == 0) return true;
        if (c == c->context) break;
    }
    return false;
}

// a vector whose meta element is a packed primitive (element-wise ops)
static bool vec_prim_elem(enode n, Au_t* elem) {
    if (!n || !n->autype) return false;
    Au_t au = n->autype;
    while (au && au->member_type == AU_MEMBER_VAR) au = au->src;
    if (!au || !au_is_vector(au)) return false;
    Au_t m = n->autype->meta.a ? n->autype->meta.a : (n->meta_a ? (Au_t)n->meta_a :
        (n->autype->member_type == AU_MEMBER_VAR && n->autype->src ? n->autype->src->meta.a : null));
    if (!m || !m->is_primitive || m->is_pointer) return false;
    *elem = m;
    return true;
}

// a vector whose meta element is a class
static bool vec_class_elem(enode n, Au_t* elem) {
    if (!n || !n->autype) return false;
    Au_t au = n->autype;
    while (au && au->member_type == AU_MEMBER_VAR) au = au->src;
    if (!au || !au_is_vector(au)) return false;
    Au_t m = n->autype->meta.a ? n->autype->meta.a : (n->meta_a ? (Au_t)n->meta_a :
        (n->autype->member_type == AU_MEMBER_VAR && n->autype->src ? n->autype->src->meta.a : null));
    if (!m || !m->is_class || m->is_c || m == typeid(Au)) return false;
    *elem = m;
    return true;
}

AU_EXPORT enode enode_value(enode mem, bool force_load);
enode etype_access(etype target, string name);

// the vector's element memory and its count, loaded
static LLVMValueRef vec_origin(aether a, enode v) {
    enode obj = v->loaded ? v : enode_value(v, true);
    enode on  = etype_access((etype)obj, string("origin"));
    return LLVMBuildLoad2(B, LLVMPointerTypeInContext(a->module_ctx, 0), _llvalue((enode)on), "origin");
}

static LLVMValueRef vec_count(aether a, enode v) {
    enode obj = v->loaded ? v : enode_value(v, true);
    enode cn  = etype_access((etype)obj, string("count"));
    return LLVMBuildLoad2(B, LLVMInt32TypeInContext(a->module_ctx), _llvalue((enode)cn), "count");
}
...
AU_EXPORT enode enode_value(enode mem, bool force_load) { sequencer
    ...
    if (!mem->loaded && (force_load || !is_struct(canonical(mem)) || mem->autype->is_explicit_ref) &&
       ...
       (!is_func((Au)mem) || is_func_ptr((Au)mem))) {
        ...
        LLVMValueRef mem_v = _llvalue(mem);
        if (!force_load && is_struct((Au)au) && !mem->autype->is_explicit_ref)
            return mem;
        if (au->elements > 0) {
            ...
            if (mem_v && LLVMGetInstructionOpcode(mem_v) == LLVMAlloca &&
                LLVMGetTypeKind(LLVMGetAllocatedType(mem_v)) == LLVMPointerTypeKind) {
                LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
                LLVMValueRef loaded = LLVMBuildLoad2(B, ptr_ty, mem_v, "var_arr_load");
                return with_value(loaded, enode(mod, a, loaded, true, autype, au));
            }
            return mem;
        }

        string id = f(string, "load_%i_%s", seq, mem->autype->ident ? mem->autype->ident : "");
        Au info = head(mem);
        ...
        if (mem->autype->is_explicit_ref) {
            if (mem_v && LLVMGetValueKind(mem_v) == LLVMArgumentValueKind) {
                return with_value(mem_v, enode(mod, a, loaded, true, autype, au,
                    is_explicit_ref, true));
            }
            LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
            LLVMValueRef loaded_v = LLVMBuildLoad2(B, ptr_ty, mem_v, id->chars);
            return with_value(loaded_v, enode(mod, a, loaded, true, autype, au,
                is_explicit_ref, true));
        }
        ...
        // a loaded value keeps the slot's meta (vec T stays a vector of T)
        enode  res = with_value(loaded, enode(mod, a, loaded, true, autype, au,
            meta_a, mem->meta_a ? mem->meta_a : (Au)mem->autype->meta.a,
            meta_b, mem->meta_b ? mem->meta_b : mem->autype->meta.b));
        return res;
    } else {
        return mem;
    }
...
AU_EXPORT token aether_peek_safe(aether a) {
    if (a->cursor == len(a->tokens)) {
        if (!a->cursor)
            return token("[none]");
...
static inline bool is_fp_value(LLVMValueRef v) {
    ...
    return k == LLVMFloatTypeKind || k == LLVMDoubleTypeKind || k == LLVMHalfTypeKind || k == LLVMBFloatTypeKind;
}
...
static LLVMValueRef build_arith_op(
    ...
    if (L_ptr || R_ptr) {
        ...
        if (L_ptr && R_ptr && optype == OPType__sub) {
            // ptr - ptr = ptrdiff
            LLVMValueRef Li = LLVMBuildPtrToInt(b, L, i64_ty, "ptr2int_l");
            LLVMValueRef Ri = LLVMBuildPtrToInt(b, R, i64_ty, "ptr2int_r");
            return LLVMBuildSub(b, Li, Ri, name);
        }
        if (L_ptr && !R_ptr && (optype == OPType__add || optype == OPType__sub)) {
            // ptr +/- int = GEP
            LLVMTypeRef i8_ty = LLVMInt8TypeInContext(a->module_ctx);
            LLVMValueRef idx = R;
            if (Rk != LLVMIntegerTypeKind || LLVMGetIntTypeWidth(LLVMTypeOf(R)) != 64)
                idx = LLVMBuildIntCast2(b, R, i64_ty, is_signed, "idx_cast");
            if (optype == OPType__sub)
                idx = LLVMBuildNeg(b, idx, "neg_idx");
            return LLVMBuildGEP2(b, i8_ty, L, &idx, 1, name);
        }
        if (!L_ptr && R_ptr && optype == OPType__add) {
            // int + ptr = GEP
            LLVMTypeRef i8_ty = LLVMInt8TypeInContext(a->module_ctx);
            LLVMValueRef idx = L;
            if (Lk != LLVMIntegerTypeKind || LLVMGetIntTypeWidth(LLVMTypeOf(L)) != 64)
...
AU_EXPORT etype pointer(aether mod, Au a);

#if 0
AU_EXPORT etype evar_type(evar aa) {
    aether a = au_active(aa->mod);
    if (aa->autype->member_type == AU_MEMBER_VAR)
        return u(etype, aa->autype->src);
    return u(etype, aa->autype);
}
#else
AU_EXPORT etype evar_type(evar aa) {
    aether a = au_active(aa->mod);
...
AU_EXPORT void aether_ensure_terminator(aether a, enode f) {
    emit_guard;
    if (_llvalue((enode)f)) {
        LLVMTypeRef tr = instanceof(f, efunc) ? _lltype_slot((etype)f) : (LLVMTypeRef)null;
        LLVMTypeRef ret_ty = LLVMGetReturnType(tr);
        bool is_void = LLVMGetTypeKind(ret_ty) == LLVMVoidTypeKind;
        LLVMBasicBlockRef bb = LLVMGetFirstBasicBlock(_llvalue((enode)f));
        while (bb) {
...
AU_EXPORT bool is_explicit_ref(enode arg) {
...
enode aether_e_assign(aether a, enode L, Au R, OPType op_val) { sequencer
    ...
    bool  is_init      = !L->autype;
    bool  is_reassign  = L->autype && L->autype->is_assigned;
    ...
    if ((is_class_set || is_shaped_slot) && is_member_slot && is_reassign_op) {
        LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
        prev_value = LLVMBuildLoad2(B, ptr_ty, _llvalue((enode)L), "prev_ref");
    }
     ...
     * ------------------------------------------------------------ */
    if (is_init) {
        L->autype = (Au_t)hold(rR->autype);
        L->autype->is_const = a->is_const_op;
    ...
    LLVMValueRef store_target = _llvalue((enode)L);
    if (L->autype->is_explicit_ref && op_val > OPType__assign) {
        if (is_struct(L->autype->src)) {
            // struct refs: L->value is already the pointer to struct
            store_target = _llvalue((enode)L);
        } else {
            // compound operators (+=, -=, etc): dereference the ref to modify pointed-to data
            LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
            store_target = LLVMBuildLoad2(B, ptr_ty, _llvalue((enode)L), "ref_target");
        }
    }
    ...
    if (op_val <= OPType__assign) {
            ...
            do {
                for (int i = 0; i < rec_au->members.count; i++) {
                    ...
                    if (mem->operator_type == op_val) {
                        ...
                        verify(Lt->autype->args.count == 2,
                            "expected 2 arguments for operator method");
                        res = e_fn_call(a, (efunc)Lt, a(cur, rR), false, true);
                        no_store = true;
                        break;
                    }
                }
        ...
        if (!res) {
            ...
            if (!is_ptr(canonical(L)))
                rR = e_create(a, (etype)canonical(L), (Au)rR, false);
    ...
    if (!no_store) {
        ...
        * ------------------------------------------------------------ */
        // a vector of another element type converts element-wise
        Au_t le9 = null, re9 = null;
        bool vconv9 = op_val <= OPType__assign && vec_prim_elem(L, &le9) &&
            vec_prim_elem(res, &re9) && le9 != re9;
        // class elements too, unless the source's already are the target's
        if (!vconv9 && op_val <= OPType__assign && vec_class_elem(L, &le9) &&
                vec_class_elem(res, &re9) && le9 != re9 && !inherits(re9, le9))
            vconv9 = true;
        if (vconv9) {
            efunc f_cv = (efunc)u(efunc, find_member(typeid(vector), "vector_convert",
                AU_MEMBER_FUNC, 0, false));
            res = e_fn_call(a, f_cv, a(res, e_typeid(a, etype_prep(a, le9))), false, false);
        }
        ...
        * ------------------------------------------------------------ */
        ...
        if (!res->loaded && res->autype && res->autype->member_type == AU_MEMBER_VAR &&
            res->autype->src && (res->autype->src->is_pointer || res->autype->src->elements > 0) &&
            _llvalue((enode)res) && LLVMGetInstructionOpcode(_llvalue((enode)res)) == LLVMAlloca) {
            LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
            LLVMValueRef ld = LLVMBuildLoad2(B, ptr_ty, _llvalue((enode)res), "store_ptr_load");
            res = with_value(ld, enode(mod, a, autype, res->autype, loaded, true));
        }

        // for struct construction results (unloaded alloca), alias the evar
        // to the struct alloca directly instead of storing a pointer.
        //
        // gate on is_struct() (the function) rather than res->autype->is_struct
        // (the field): a local struct var like `m: mat4f` has res->autype of
        ...
        if (lfx9 && res->autype && !res->loaded && array_slot(_llvalue((enode)L)) &&
            ...
            if (_llvalue((enode)L) && _llvalue((enode)L) != _llvalue((enode)res)) {
                ...
                etype st = u(etype, res->autype->src ? res->autype->src : (Au_t)res->autype);
                if (!st || !_lltype_slot(st)) st = u(etype, res->autype);
                if (!st || !_lltype_slot(st)) st = etype_prep(a, au_arg_type((Au)res->autype));
                if (st && _lltype_slot(st)) {
                    LLVMValueRef loaded = LLVMBuildLoad2(B, _lltype_slot(st), _llvalue((enode)res), "struct_cp");
                    LLVMBuildStore(B, loaded, _llvalue((enode)L));
                } else {
                    _llvalue_set((enode)L, _llvalue((enode)res));
                }
            } else {
                _llvalue_set((enode)L, _llvalue((enode)res));
            }
        } else {
            LLVMValueRef store_val = _llvalue((enode)res);
            if (!res->loaded && LLVMGetInstructionOpcode(store_val) == LLVMGetElementPtr) {
                ...
                if (!skip) {
                    ...
                    if (rt && _lltype_slot(rt)) {
                        ...
                        if (L->autype && (L->autype->is_pointer || L->autype->is_class ||
                            (L->autype->src && (L->autype->src->is_pointer || L->autype->src->is_class)))) {
                            LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
            ...
            if (dst_is_struct && val_is_ptr && dst_is_alloca) {
                ...
                if (!st || !_lltype_slot(st)) st = etype_prep(a, au_arg_type((Au)L->autype));
                if (st && _lltype_slot(st)) {
                    u64 sz = LLVMABISizeOfType(ll_td(a), _lltype_slot(st));
    ...
    if (a->watch_member && !a->no_build && res && _llvalue((enode)res) && L && L->autype &&
        ...
        LLVMTypeRef  vt = LLVMTypeOf(_llvalue((enode)res));
        ...
        const char*  conv = "%p";
        LLVMValueRef arg  = _llvalue((enode)res);
        if (vk == LLVMIntegerTypeKind) {
            unsigned bits = LLVMGetIntTypeWidth(vt);
            if      (bits <  32) { arg = LLVMBuildZExt(B, _llvalue((enode)res), i32_ty, ""); conv = "%i"; }
            else if (bits == 32) { conv = "%i"; }
            else                 { conv = "%lli"; }
        } else if (vk == LLVMFloatTypeKind)  { arg = LLVMBuildFPExt(B, _llvalue((enode)res), dbl_ty, ""); conv = "%f"; }
    ...
    bool  _in_init = _p65_fn && _p65_fn->autype && strcmp(_p65_fn->autype->ident, "init") == 0;
    if (!_in_init && !no_store && is_member_slot && _llvalue((enode)L) && L->autype &&
        L->autype->context && L->autype->member_type == AU_MEMBER_VAR &&
        LLVMGetInstructionOpcode(_llvalue((enode)L)) == LLVMGetElementPtr) {
        Au_t parent_type = (Au_t)L->autype->context;
        ...
        if (has_fbits) {
            ...
            if (parent_etype && _lltype_slot(parent_etype)) {
                ...
                if (idx >= 0 && idx < AF_WORDS * 64) {
                    ...
                    LLVMValueRef base_ptr = LLVMGetOperand(_llvalue((enode)L), 0);
                    enode parent_node = with_value(base_ptr, enode(mod, a, autype, parent_type, loaded, false));
    ...
    if (_p7_ctor && is_member_slot && _llvalue((enode)L) &&
        ...
        LLVMValueRef _p7_self = LLVMGetParam(_llvalue((enode)_p65_fn), 0);
    ...
    if ((is_class_set || is_shaped_slot) && is_member_slot && !_p7_self_init) {
        ...
        if (prev_value) {
            ...
            enode prev_node = with_value(prev_value, enode(mod, a, autype, L_type->autype, loaded, true));
...
AU_EXPORT none etype_implement(etype t, bool);
...
etype etype_ptr(aether a, Au_t au, enode eshape) {
    ...
    if (!u(etype, au_ptr)) {
        etype t = etype(mod, a, autype, au_ptr);
        if (eshape) {
...
AU_EXPORT etype shape_pointer(aether mod, Au a, enode eshape) {
    Au_t au = au_arg(a);
...
etype _etype_traits(Au input, int traits, bool poly) {
    ...
    if (isa(input) == typeid(etype)) {
        ...
        au = ((etype)input)->autype;
        a  = ((etype)input)->mod;
        is_etype = true;
    }
    else if (isa(input) == typeid(enode)) {
        au = ((enode)input)->autype;
        a  = ((enode)input)->mod;
    }
    ...
    etype u = u(etype, au);
    return (u->autype->traits & traits) == traits ? u : null;
}

AU_EXPORT etype etype_traits(Au input, int traits) {
    return _etype_traits(input, traits, false);
}

/// C type rules implemented
AU_EXPORT etype determine_rtype(aether a, OPType optype, etype L, etype R) {
    ...
    // Integer promotion
    int L_size = L->autype->abi_size;
    int R_size = R->autype->abi_size;
    if (L_size > R_size)
        return canonical(L);
...
// one LLVM context per core; a type registers into every context at once
#define ll_n(a) ((a)->ncores ? (a)->ncores : 1)


AU_EXPORT LLVMTargetDataRef ll_td(aether a) {
    ARef td = a->target_datas[core_index((Au)a)];
    return td ? (LLVMTargetDataRef)td : ll_td(a);
}

AU_EXPORT LLVMContextRef ll_ctx(aether a, int core) {
    array c = a->ctxs;
    return (c && core < c->count && c->origin[core]) ?
        (LLVMContextRef)c->origin[core] : a->module_ctx;
}

AU_EXPORT LLVMModuleRef ll_mod(aether a, int core) {
    array m = a->mods;
    return (m && core < m->count && m->origin[core]) ?
        (LLVMModuleRef)m->origin[core] : a->module_ref;
}

// the slot belongs to the thread doing the emitting, never to whoever
// happened to create the node
AU_EXPORT int core_index(Au o) {
    return au_codegen_active ? au_codegen_active->core : 0;
}

AU_EXPORT void _lltype_set(etype e, LLVMTypeRef v) {
    emit_guard;
    e->lltypes[core_index((Au)e)] = (ARef)v;
}

LLVMTypeRef _lltype_slot(etype e) {
    return ((LLVMTypeRef)e->lltypes[core_index((Au)e)]);
}


// one Au_t, one set of LLVM types. a second etype for the same Au_t shares
// what the registered one already built instead of making a rival type
static bool ll_share_owner(aether a, etype t) {
    etype owner = u(etype, t->autype);
    if (!owner || owner == t || !owner->lltypes[0]) return false;
...
#define ll_creator(NAME, EXPR) \
    LLVMTypeRef NAME(aether a, etype t) { \
        ...
        for (int i = 0; i < ll_n(a); i++) { \
            LLVMContextRef c = ll_ctx(a, i); (void)c; \
            t->lltypes[i] = (ARef)EXPR; \
        } \
        return _lltype_slot(t); \
    }

ll_creator(ll_ptr,  LLVMPointerTypeInContext(c, 0))
ll_creator(ll_f32,  LLVMFloatTypeInContext (c))
ll_creator(ll_f64,  LLVMDoubleTypeInContext(c))
ll_creator(ll_void, LLVMVoidTypeInContext  (c))
ll_creator(ll_bf16, LLVMBFloatTypeInContext(c))
ll_creator(ll_fp16, LLVMHalfTypeInContext  (c))
...
AU_EXPORT LLVMTypeRef ll_int(aether a, etype t, int bits) {
    ...
    emit_guard;
    for (int i = 0; i < ll_n(a); i++)
        t->lltypes[i] = (ARef)LLVMIntTypeInContext(ll_ctx(a, i), bits);
    return _lltype_slot(t);
}

AU_EXPORT LLVMTypeRef ll_named(aether a, etype t, symbol name) {
    type_guard;
    if (ll_share_owner(a, t)) return _lltype_slot(t);
    emit_guard;
    for (int i = 0; i < ll_n(a); i++)
        t->lltypes[i] = (ARef)LLVMStructCreateNamed(ll_ctx(a, i), name);
    return _lltype_slot(t);
}

AU_EXPORT LLVMTypeRef ll_array(aether a, etype t, etype src, int elements) {
    type_guard;
    if (ll_share_owner(a, t)) return _lltype_slot(t);
    emit_guard;
    for (int i = 0; i < ll_n(a); i++)
        t->lltypes[i] = (ARef)LLVMArrayType(_lltype_core(src, i), elements);
    return _lltype_slot(t);
}

// alias/enum: share the source type, per context
AU_EXPORT LLVMTypeRef ll_same(aether a, etype t, etype src) {
    type_guard;
    if (ll_share_owner(a, t)) return _lltype_slot(t);
    emit_guard;
    for (int i = 0; i < ll_n(a); i++)
        t->lltypes[i] = (ARef)_lltype_core(src, i);
    return _lltype_slot(t);
}
...
AU_EXPORT LLVMTypeRef ll_fn(aether a, etype t, etype rtype, etype* args, int n_args, bool va) {
    ...
    if (ll_share_owner(a, t)) return _lltype_slot(t);
    emit_guard;
    LLVMTypeRef* at = calloc(n_args + 1, sizeof(LLVMTypeRef));
    for (int i = 0; i < ll_n(a); i++) {
...
AU_EXPORT LLVMTypeRef ll_wrap_array(aether a, etype t, int elements) {
    type_guard;
    emit_guard;
    for (int i = 0; i < ll_n(a); i++)
        t->lltypes[i] = (ARef)LLVMArrayType((LLVMTypeRef)t->lltypes[i], elements);
    return _lltype_slot(t);
}

// a struct field, described so it can be rebuilt in every context
AU_EXPORT LLVMTypeRef ll_field_at(aether a, ll_field* f, int core) {
    LLVMTypeRef v = f->percore ? f->percore[core] :
                    f->bits    ? LLVMIntTypeInContext(ll_ctx(a, core), f->bits) :
                    f->raw     ? ((LLVMTypeRef)f->e->lltypes[core]) : _lltype_core(f->e, core);
    if (v && f->arr && LLVMGetTypeKind(v) != LLVMArrayTypeKind)
        v = LLVMArrayType(v, f->elements);
    return v;
}
...
// a named struct with no etype of its own, one per context
AU_EXPORT LLVMTypeRef* ll_named_raw(aether a, symbol name, ll_field* fields, int n, bool packed) {
    type_guard;
    emit_guard;
    LLVMTypeRef* out = calloc(ll_n(a), sizeof(LLVMTypeRef));
    LLVMTypeRef* ft  = calloc(n + 1, sizeof(LLVMTypeRef));
    for (int i = 0; i < ll_n(a); i++) {
        out[i] = LLVMStructCreateNamed(ll_ctx(a, i), name);
        for (int j = 0; j < n; j++)
            ft[j] = ll_field_at(a, &fields[j], i);
        LLVMStructSetBody(out[i], ft, n, packed);
    }
    free(ft);
    return out;
}

// value, entry block and debug metadata: one slot per core on the node
AU_EXPORT void _llvalue_set(enode n, LLVMValueRef v) {
    n->values[core_index((Au)n)] = (ARef)v;
}

// a literal carries no context of its own — rebuild it for whichever
// core asks, so shared constant nodes work on every worker
AU_EXPORT LLVMValueRef _llvalue(enode n) {
    int          core = core_index((Au)n);
    LLVMValueRef v    = (LLVMValueRef)n->values[core];
    if (!v && n->literal && n->values[0]) {
        aether a     = au_active(n->mod);
        enode  fresh = e_operand(a, n->literal, canonical((etype)n));
        v = (LLVMValueRef)fresh->values[core];
        n->values[core] = (ARef)v;
    }
    return v;
}

// the entry block belongs to whichever core is emitting the body; create
// it on first use so a worker never positions the builder at null
AU_EXPORT LLVMBasicBlockRef _llentry(enode n) {
    int core = core_index((Au)n);
    LLVMBasicBlockRef bb = (LLVMBasicBlockRef)n->entries[core];
    if (!bb && n->values[core]) {
        aether a = au_active(n->mod);
        string label = f(string, "%s_entry",
            n->autype && n->autype->ident ? n->autype->ident : "fn");
        bb = LLVMAppendBasicBlockInContext(
            ll_ctx(a, core), (LLVMValueRef)n->values[core], label->chars);
        n->entries[core] = (ARef)bb;
    }
    return bb;
}

void lldebug_set(etype e, LLVMMetadataRef v) { e->lldebugs [core_index((Au)e)] = (ARef)v; }
void llscope_set(etype e, LLVMMetadataRef v) { e->llscopes [core_index((Au)e)] = (ARef)v; }
void lldbg_set  (enode n, LLVMMetadataRef v) { n->last_dbgs[core_index((Au)n)] = (ARef)v; }
LLVMMetadataRef _lldebug(etype e) { return (LLVMMetadataRef)e->lldebugs[core_index((Au)e)]; }
LLVMMetadataRef _llscope(etype e) { return (LLVMMetadataRef)e->llscopes[core_index((Au)e)]; }
LLVMMetadataRef _lldbg  (enode n) { return ((LLVMMetadataRef)n->last_dbgs[core_index((Au)n)]); }

// every creator below writes one value per module, under the emit lock
AU_EXPORT void ll_const_int(aether a, enode n, etype t, unsigned long long v, bool sext) {
    type_guard;
    emit_guard;
    for (int i = 0; i < ll_n(a); i++)
        n->values[i] = (ARef)LLVMConstInt(_lltype_core(t, i), v, sext);
}
...
AU_EXPORT void ll_const_real(aether a, enode n, etype t, double v) {
    ...
    emit_guard;
    for (int i = 0; i < ll_n(a); i++)
        n->values[i] = (ARef)LLVMConstReal(_lltype_core(t, i), v);
}
...
AU_EXPORT void ll_func(aether a, enode n, etype ft, symbol name) {
    ...
    for (int i = 0; i < ll_n(a); i++) {
        ...
        LLVMValueRef  f = LLVMGetNamedFunction(m, name);
        if (!f) f = LLVMAddFunction(m, name, ((LLVMTypeRef)ft->lltypes[i]));
        n->values[i] = (ARef)f;
    }
}

// constant field address inside a global — no builder, so every core can hold it
AU_EXPORT void ll_const_gep(aether a, enode n, etype st, enode base, int index) {
    type_guard;
    emit_guard;
    for (int i = 0; i < ll_n(a); i++) {
        LLVMTypeRef  i32_ty = LLVMInt32TypeInContext(ll_ctx(a, i));
        LLVMValueRef idx[2] = {
            LLVMConstInt(i32_ty, 0, 0),
            LLVMConstInt(i32_ty, index, 0) };
        n->values[i] = (ARef)LLVMConstInBoundsGEP2(
            _lltype_core(st, i), (LLVMValueRef)base->values[i], idx, 2);
    }
}
...
AU_EXPORT void ll_param(aether a, enode n, enode fn, int index) {
    emit_guard;
    for (int i = 0; i < ll_n(a); i++)
        n->values[i] = (ARef)LLVMGetParam((LLVMValueRef)fn->values[i], index);
}

// entry block: only the core building the body may define the function
AU_EXPORT void ll_entry(aether a, enode n, symbol label) {
    emit_guard;
    int core = core_index((Au)n);
    n->entries[core] = (ARef)LLVMAppendBasicBlockInContext(
        ll_ctx(a, core), (LLVMValueRef)n->values[core], label);
}

AU_EXPORT void ll_linkage(aether a, enode n, LLVMLinkage linkage) {
    emit_guard;
    int  core  = core_index((Au)n);
    bool split = ll_n(a) > 1 && linkage == LLVMInternalLinkage;
    LLVMValueRef v = (LLVMValueRef)n->values[core];
    LLVMSetLinkage(v, split ? LLVMExternalLinkage : linkage);
    if (split) LLVMSetVisibility(v, LLVMHiddenVisibility);
}
...
void ll_global(aether a, enode n, etype t, symbol name,
    ...
    for (int i = 0; i < ll_n(a); i++) {
        LLVMModuleRef m  = ll_mod(a, i);
        LLVMTypeRef   ty = _lltype_core(t, i);
        LLVMValueRef  g  = reuse ? LLVMGetNamedGlobal(m, name) : null;
        if (!g) {
            g = (external || i > 0) ? LLVMFetchGlobal(m, ty, (cstr)name) :
            ...
            // set_init is what says we define it here: export those, import
            // the rest. both flags are set for a module's own type info
            // set_init marks it as ours. core 0 holds the storage and exports
            // it; the other cores of this module link into the same dll, so
            // they declare it plainly -- importing it there is LNK4217
            if (set_init)
...
LLVMTypeRef _lltype_core(etype e, int core) {
    aether a = au_active(e->mod);
    Au_t  au = e->autype;

    if (e->autype->member_type == AU_MEMBER_VAR) {
        e = u(etype, e->autype->src);
        au = e->autype;
    }

    while (au && (!e || (e && !((LLVMTypeRef)e->lltypes[core])))) {
        au = au->src;
        e  = u(etype, au);
    }
    ...
    if (e->autype->is_class) {
        verify(e->autype->ptr, "expected ptr for class");
        e = u(etype, e->autype->ptr);
        verify(e, "resolution failure on %o", au);
    }

    verify((LLVMTypeRef)e->lltypes[core], "no type found on %o", e);
    return ((LLVMTypeRef)e->lltypes[core]);
}

AU_EXPORT LLVMTypeRef _lltype(etype e) {
    return _lltype_core(e, core_index((Au)e));
}
...
AU_EXPORT enode aether_e_add(aether a, Au L, Au R) {
...
AU_EXPORT enode aether_e_div(aether a, Au L, Au R) {
    return e_op(a, OPType__div, string("div"), L, R);
}

AU_EXPORT enode aether_value_default(aether a, Au L, Au R) {
    return e_op(a, OPType__value_default, string("value_default"), L, R);
}

AU_EXPORT enode aether_cond_value   (aether a, Au L, Au R) {
    return e_op(a, OPType__cond_value, string("cond_value"), L, R);
}


AU_EXPORT enode aether_e_inherits(aether a, enode L, Au R) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, etypeid(bool));

    // Get the type pointer for L
    enode L_type =  e_offset(a, L, _i64(-sizeof(Au)), false);
    ...
    LLVMAddIncoming(phi, &(LLVMValueRef){_llvalue((enode)L_ptr)}, &block, 1);
    ...
    LLVMBuildCondBr(B, _llvalue((enode)loop_cond), loop_block, exit_block);

    // Update the phi enode
    LLVMAddIncoming(phi, &(LLVMValueRef){_llvalue((enode)parent)}, &loop_block, 1);

    // Exit block
    LLVMPositionBuilderAtEnd(B, exit_block);
    LLVMValueRef result = LLVMBuildPhi(B, lltype(cmp), "inherit_result");
    LLVMAddIncoming(result, &(LLVMValueRef){_llvalue((enode)cmp)}, &loop_block, 1);
    LLVMAddIncoming(result, &(LLVMValueRef){LLVMConstInt(LLVMInt1TypeInContext(a->module_ctx), 0, 0)}, &block, 1);

    //enode(mod, a, value, vr, autype, m->autype, loaded, true)
    Au_t au = (etypeid(bool))->autype;

    return with_value(result, enode(mod, a, autype, au, loaded, true));
}
...
AU_EXPORT enode aether_e_cmp_op(aether a, OPType optype, enode L, enode R) {
    ...
    if ((is_system((Au)L) || is_system((Au)R)) &&
        LLVMTypeOf(_llvalue((enode)L)) == LLVMTypeOf(_llvalue((enode)R))) {
        struct cmp_entry* cmp = cmp_lookup(optype);
        verify(cmp, "invalid comparison operator");
        char N[64];
        sprintf(N, "actual_op_%i_%s", seq, cmp->name);
        return value(bool_t, LLVMBuildICmp(B, cmp->ui_pred, _llvalue((enode)L), _llvalue((enode)R), N));
    }
    ...
    if (is_enum(L) || is_enum(R)) {
        ...
        L = e_create(a, cmp_type, (Au)L, false);
        R = e_create(a, cmp_type, (Au)R, false);
    } else {
        etype operand_type = determine_rtype(a, OPType__add, (etype)L, (etype)R);
        L = e_create(a, operand_type, (Au)L, false);
        R = e_create(a, operand_type, (Au)R, false);
    }
    ...
    if (is_prim(L) && is_prim(R)) {
        LLVMValueRef Lv = _llvalue((enode)L), Rv = _llvalue((enode)R);
    ...
    if (L->autype->is_system || R->autype->is_system) {
    ...
    if (L->autype != R->autype && (!L->autype->is_system && !R->autype->is_system)) {
        if (L->autype->abi_size >= R->autype->abi_size)
            R = e_create(a, (etype)L, (Au)R, false);
        else
            L = e_create(a, (etype)R, (Au)L, false);
    }
    ...
    if (is_class(L) || is_class(R)) {
        ...
        LLVMValueRef l_is_null = LLVMBuildICmp(B, LLVMIntEQ, _llvalue((enode)L), null_ptr, "cmp_l_null");
        LLVMBasicBlockRef cur_bb  = LLVMGetInsertBlock(B);
        LLVMValueRef cur_fn       = LLVMGetBasicBlockParent(cur_bb);
        LLVMBasicBlockRef cmp_bb  = LLVMAppendBasicBlockInContext(a->module_ctx, cur_fn, "cmp_nonnull");
        LLVMBasicBlockRef merge_bb = LLVMAppendBasicBlockInContext(a->module_ctx, cur_fn, "cmp_merge");
        LLVMValueRef ptr_eq = LLVMBuildICmp(B, cmp->ui_pred, _llvalue((enode)L), _llvalue((enode)R), "ptr_eq");
        LLVMBuildCondBr(B, l_is_null, merge_bb, cmp_bb);
        ...
        LLVMValueRef cmp_bool = LLVMBuildICmp(B, cmp->si_pred, _llvalue((enode)cmp_result), zero, "cmp_bool");
        LLVMBasicBlockRef cmp_end_bb = LLVMGetInsertBlock(B);
        LLVMBuildBr(B, merge_bb);

        LLVMPositionBuilderAtEnd(B, merge_bb);
        LLVMValueRef phi = LLVMBuildPhi(B, LLVMInt1TypeInContext(a->module_ctx), "cmp_result");
        LLVMValueRef    phi_vals[] = { ptr_eq,  cmp_bool  };
        LLVMBasicBlockRef phi_bbs[] = { cur_bb, cmp_end_bb };
        LLVMAddIncoming(phi, phi_vals, phi_bbs, 2);

        return value(bool_t, phi);
    }
    ...
    if (is_struct(L) && is_struct(R) && (optype == OPType__equal || optype == OPType__not_equal)) {
        verify(au_arg_type((Au)L) == au_arg_type((Au)R), "struct type mismatch");
        bool L_ptr = !L->loaded;
        bool R_ptr = !R->loaded;
        ...
        for (int i = 0; i < L->autype->members.count; i++) {
            Au_t m = (Au_t)L->autype->members.origin[i];
            if (m->member_type != AU_MEMBER_VAR)
                continue;
            evar mem = u(evar, m);
            etype mt = u(etype, mem->autype->src);

            LLVMValueRef lv = L_ptr ?
                LLVMBuildLoad2(B, lltype(mt),
                    LLVMBuildStructGEP2(B, lltype(L_type), _llvalue((enode)L), m->member_index, "l_gep"), "l_mem") :
                LLVMBuildExtractValue(B, _llvalue((enode)L), m->member_index, "");
            
            LLVMValueRef rv = R_ptr ?
                LLVMBuildLoad2(B, lltype(mt),
                    LLVMBuildStructGEP2(B, lltype(R_type), _llvalue((enode)R), m->member_index, "r_gep"), "r_mem") :
                LLVMBuildExtractValue(B, _llvalue((enode)R), m->member_index, "");

            enode le = value(u(etype, m), lv);
            enode re = value(u(etype, m), rv);

            enode sub = aether_e_cmp_op(a, OPType__equal, le, re);
            accum = LLVMBuildAnd(B, accum, _llvalue((enode)sub), "and");
        }
    ...
    if (is_realistic(L)) {
        return value(bool_t,
            LLVMBuildFCmp(B, cmp->fp_pred,
                          _llvalue((enode)L), _llvalue((enode)R), N));
    }

    LLVMIntPredicate pred = (is_unsign(L) && is_unsign(R)) ? cmp->ui_pred : cmp->si_pred;
    return value(bool_t,
        LLVMBuildICmp(B, pred, _llvalue((enode)L), _llvalue((enode)R), N));
}
...
enode aether_e_eq(aether a, enode L, enode R) {
        ...
        if (is_realistic(L) || is_realistic(R)) {
            ...
            return value(bool_t,
                LLVMBuildFCmp(B, LLVMRealOEQ,
                              _llvalue((enode)L), _llvalue((enode)R), "eq-f"));
        }

        return value(bool_t,
            LLVMBuildICmp(B, LLVMIntEQ,
                          _llvalue((enode)L), _llvalue((enode)R), "eq-i"));
    }

    // 2. If types differ, normalize the smaller to the larger
    if (L->autype != R->autype) {
        if (L->autype->abi_size >= R->autype->abi_size)
            R = e_create(a, (etype)L, (Au)R, false);
        else
            L = e_create(a, (etype)R, (Au)L, false);
    ...
    if (is_class(L) || is_class(R)) {
        ...
        Au_t fn = find_member(L->autype, "compare", AU_MEMBER_FUNC, 0, true);
        verify(fn, "class %s has no compare() method", L->autype->ident);

        enode cmp = e_fn_call(a, u(efunc, fn), a(L, R), false, true);
        ...
        return value(bool_t,
            LLVMBuildICmp(B, LLVMIntEQ,
                          _llvalue((enode)cmp), zero, "cmp-zero"));
    }
    ...
    if (is_realistic(L))
        return with_value(LLVMBuildFCmp(B, LLVMRealOEQ,
                          _llvalue((enode)L), _llvalue((enode)R), "eq-f"), enode(mod, a, autype, bool_t->autype, loaded, true));

    LLVMValueRef nv = LLVMBuildICmp(B, LLVMIntEQ, _llvalue((enode)L), _llvalue((enode)R), "eq-i");
    return with_value(nv, enode(mod, a, loaded, true, autype, bool_t->autype));
}

AU_EXPORT etype etype_test(aether a) {
    etype str = etypeid(string);
    return str;
}


AU_EXPORT enode aether_e_eq_prev(aether a, enode L, enode R) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, etypeid(bool));
    ...
    if (is_prim(L) && is_prim(R)) {
        ...
        return value(etypeid(bool), LLVMBuildICmp(B, LLVMIntEQ, _llvalue((enode)cmp), z, "cmp-i"));
    }
    ...
    if (Lc || Rc) {
        ...
        Au_t au_f = find_member(L->autype, "eq", AU_MEMBER_FUNC, 0, true);
        if (au_f) {
            ...
            return e_fn_call(a, eq, a(L, R), false, true);
        }
    } else if (Ls || Rs) {
        ...
        verify(Ls && Rs && (L->autype == R->autype),
            "struct type mismatch %o != %o", L->autype, R->autype);
        
        for (int i = 0; i < L->autype->members.count; i++) {
            Au_t mem = (Au_t)L->autype->members.origin[i];
            if (mem->member_type != AU_MEMBER_VAR)
                continue;

            LLVMValueRef lv  = LLVMBuildExtractValue(B, _llvalue((enode)L), mem->member_index, "lv");
            LLVMValueRef rv  = LLVMBuildExtractValue(B, _llvalue((enode)R), mem->member_index, "rv");
            enode        le  = value(u(etype, mem), lv);
            enode        re  = value(u(etype, mem), rv);
            enode        cmp = e_cmp_op(a, OPType__equal, le, re);
            result = LLVMBuildAnd(B, result, _llvalue((enode)cmp), "eq-struct-and");
        }
        return value(etypeid(bool), result);

    } else if (L->autype != R->autype) {
        if (Lr && Rr) {
            ...
            L = e_create(a, (etype)L, (Au)R, false);
        } else {
            if (L->autype->abi_size > R->autype->abi_size) {
                R = e_create(a, (etype)L, (Au)R, false);
            } else {
    ...
    if (Lr || Rr)
        return value(etypeid(bool), LLVMBuildFCmp(B, LLVMRealOEQ, _llvalue((enode)L), _llvalue((enode)R), "eq-f"));
    
    return value(etypeid(bool), LLVMBuildICmp(B, LLVMIntEQ, _llvalue((enode)L), _llvalue((enode)R), "eq-i"));
}
...
enode aether_e_eval(aether a, string value) { sequencer
    ...
    // stamp each token with the origin source/line/column so errors report the
    // actual location of the puts/string expression, not line 1 of the mini-stream
    token origin = a->statement_origin;
    if (origin) {
        for (int i = 0; i < len(t); i++) {
            token tk = (token)get(t, i);
            if (tk) {
                tk->source = origin->source;
                tk->line   = origin->line;
                tk->column = origin->column;
            }
        }
    }
    ...
    if (n && !n->loaded && is_prim(n->autype))
        n = e_load(a, n, null);
    enode s = e_create(a, etypeid(string), (Au)n, false);
    pop_tokens(a, false);
    return s;
}
...
enode aether_e_interpolate(aether a, string str) { sequencer
    ...
    each (sp, ipart, s) {
        ...
        if (s->is_expr) {
            ...
            a->statement_origin = statement;
            val = e_create(a, mdl, (Au)expr, false);
        } else {
            const_string cs = const_string(chars, s->content->chars, ref_length, s->content->count);
            val = e_create(a, mdl, (Au)cs, false);
        }
        if (!accum && !s->is_expr)
            accum = val;
        else if (!accum) {
            enode fresh = e_create(a, mdl, (Au)const_string(chars, ""), false);
    ...
    a->statement_origin = statement;
    if (!accum)
        accum = e_create(a, mdl, (Au)const_string(chars, ""), false);
    return accum;
}
...
AU_EXPORT enode e_operand_primitive(aether a, Au op) {
    ...
    Au_t t = isa(op);
        ...
        return with_value(cp, enode(
            mod,     a,
            loaded,  true,
            autype,      typeid(handle),
            literal, op
        ));
    }
        ...
        return with_value(const_cstr(a, (cstr)op, strlen((symbol)op)), enode(mod, a, loaded, true,
            autype, typeid(symbol), literal, op));
    }
    else if (instanceof(op, string)) { // this works for const_string too
        string str = string(((string)op)->chars);
        return with_value(const_cstr(a, str->chars, str->count), enode(
            mod,     a,
            loaded,  true,
            autype,      typeid(symbol),
            literal, op));
    }
...
enode aether_e_op(aether a, OPType optype, string op_name, Au L, Au R) { sequencer
    ...
    if (op_name && instanceof(R, enode) && is_rec(R)) {
        ...
        if (reverse_op) {
            ...
            if (rec_r) {
                etype Rt = null;
                for (int i = 0; i < rec_r->autype->members.count; i++) {
                    Au_t mem = (Au_t)rec_r->autype->members.origin[i];
                ...
                if (Rt) {
                    verify(Rt->autype->args.count == 2,
                        "expected 2 arguments for reverse operator method");
                    return e_fn_call(a, (efunc)Rt, a(RV, LV), false, true);  // note: R is self, L is the arg
                }
            }
        }
    }
    ...
    if (op_name && instanceof(L, enode) && is_rec(L)) {
        ...
        if (rec) {
            etype Lt = null;
            for (int i = 0; i < rec->autype->members.count; i++) {
                Au_t mem = (Au_t)rec->autype->members.origin[i];
            ...
            if  (Lt) {
                ...
                return e_fn_call(a, (efunc)Lt, a(mL, RV), false, true);
            }
        }
    }
    ...
    LLVMValueRef RES;
    enode LL = (enode)LV; // optype <= OPType__assign ? LV : e_create(a, rtype, (Au)LV, false); // we dont need the 'load' in here, or convert even
    enode RL = (enode)RV; // e_create(a, rtype, (Au)RV, false);
    symbol N = cstring(f(string, "%i_%o", seq, op_name));
    Au literal = null;

    // element-wise ops on primitive arrays and vectors of primitives
    // (no operator override found); a vector result is a new vector
    Au_t Lve = null, Rve = null;
    bool L_vec = vec_prim_elem(LL, &Lve);
    bool R_vec = vec_prim_elem(RL, &Rve);
    bool L_arr = L_vec || (LL->autype->elements > 0 && LL->autype->src && LL->autype->src->is_primitive);
    bool R_arr = R_vec || (RL->autype->elements > 0 && RL->autype->src && RL->autype->src->is_primitive);
    if ((L_arr || R_arr) && optype >= OPType__add && optype <= OPType__mod && !a->no_build) {
        Au_t L_elem   = L_vec ? Lve : (L_arr ? LL->autype->src : LL->autype);
        Au_t R_elem   = R_vec ? Rve : (R_arr ? RL->autype->src : RL->autype);
        etype L_ety   = u(etype, L_elem);
        etype R_ety   = u(etype, R_elem);
        etype elem_ty = determine_rtype(a, optype, L_ety, R_ety);
        Au_t elem_au  = elem_ty->autype;
        bool sgn      = is_sign(elem_ty);
        LLVMTypeRef i32t = LLVMInt32TypeInContext(a->module_ctx);

        // element count and element bases: a vector reads its count and
        // origin; a sized array is its own pointer with a static count
        LLVMValueRef cnt, Lbase = null, Rbase = null;
        if (L_vec) Lbase = vec_origin(a, LL);
        else if (L_arr) Lbase = _llvalue((enode)LL);
        if (R_vec) Rbase = vec_origin(a, RL);
        else if (R_arr) Rbase = _llvalue((enode)RL);
        if (L_vec || R_vec)
            cnt = vec_count(a, L_vec ? LL : RL);
        else
            cnt = LLVMConstInt(i32t, L_arr ? LL->autype->elements : RL->autype->elements, 0);

        // result storage: a vector of the element type, or a stack array
        LLVMTypeRef  lt = lltype(elem_ty);
        LLVMValueRef result, res_obj = null;
        if (L_vec || R_vec) {
            etype vt  = etype_prep(a, typeid(vector));
            etype vm  = etype(mod, a, autype, vt->autype, meta_a, (Au)elem_au);
            enode vn  = e_create(a, vm, null, false);
            ...
            result  = vec_origin(a, vn);
            rtype   = vm;
        } else {
            i32 count = L_arr ? LL->autype->elements : RL->autype->elements;
            result = entry_alloca(a, LLVMArrayType(lt, count), "arr_op_result");
            Au_t res_au = def(a->autype, null, AU_MEMBER_TYPE, 0);
            res_au->src = elem_au;
            res_au->elements = count;
            res_au->is_pointer = true;
            rtype = etype(mod, a, autype, res_au);
        }
        ...
        // an empty vector runs zero iterations
        LLVMValueRef nz = LLVMBuildICmp(B, LLVMIntUGT, cnt, zero, "arr_nz");
        LLVMBuildCondBr(B, nz, loop_bb, done_bb);
        LLVMPositionBuilderAtEnd(B, loop_bb);
        LLVMValueRef idx = LLVMBuildPhi(B, i32t, "idx");

        // load elements (using source types, then promote)
        LLVMTypeRef L_lt = lltype(L_ety);
        LLVMTypeRef R_lt = lltype(R_ety);
        LLVMValueRef Lval, Rval;
        if (L_arr) {
            LLVMValueRef gep_l = LLVMBuildGEP2(B, L_lt, Lbase, &idx, 1, "l_elem_ptr");
            Lval = LLVMBuildLoad2(B, L_lt, gep_l, "l_elem");
        } else {
            Lval = _llvalue((enode)LL);
        }
        if (R_arr) {
            LLVMValueRef gep_r = LLVMBuildGEP2(B, R_lt, Rbase, &idx, 1, "r_elem_ptr");
            Rval = LLVMBuildLoad2(B, R_lt, gep_r, "r_elem");
        } else {
            Rval = _llvalue((enode)RL);
        }
        // promote to common type using e_create
        if (L_elem != elem_au) {
            enode tmp = with_value(Lval, enode(mod, a, autype, L_elem, loaded, true));
            Lval = _llvalue((enode)((enode)e_create(a, elem_ty, (Au)tmp, false)));
        }
        if (R_elem != elem_au) {
            enode tmp = with_value(Rval, enode(mod, a, autype, R_elem, loaded, true));
            Rval = _llvalue((enode)((enode)e_create(a, elem_ty, (Au)tmp, false)));
        }
        ...
        if (!ptr_arith) {
            LL = e_create(a, rtype, (Au)LL, false);
            RL = e_create(a, rtype, (Au)RL, false);
        }
        if (!a->no_build) {
            RES = build_arith_op(a, B, optype, _llvalue((enode)LL), _llvalue((enode)RL), N, is_sign(rtype));
        }
        if (ptr_arith) {
            // ptr - ptr returns i64; ptr +/- int returns ptr type
            if (is_ptr(LL) && is_ptr(RL))
                rtype = etypeid(i64);
            else
                rtype = is_ptr(LL) ? (etype)LL : (etype)RL;
        }
        ...
        if (!RL->loaded && (is_prim(RL->autype) || is_ptr(RL)))
            RL = e_load(a, RL, null);
        LL = e_create(a, rtype, (Au)LL, false); // generate compare != 0 if not already i1
        RL = e_create(a, rtype, (Au)RL, false);
        ...
        if (!a->no_build) {
            ...
            if (optype <= OPType__assign) {
                RES = a->no_build ? null : _llvalue((enode)RL);
                literal = RL->literal;
            } else {
                ...
                RES = a->no_build ? null :
                    build_arith_op(a, B, base_op, _llvalue((enode)LL_val), _llvalue((enode)RL_val), N, is_sign(rtype));
            }
            LLVMBuildStore(B, RES, _llvalue((enode)mL));
        } else {
            if (optype <= OPType__assign)
                literal = RL->literal;
        }
...
#define evar_is(n, id) (n->autype && n->autype->ident && strcmp(n->autype->ident, id) == 0)
        ...
        while (walk && walk != typeid(Au)) {
            ...
            members(walk, mem) {
                ...
                if (walk_is_record && self) {
                    Au_t self_au = au_arg_type((Au)self->autype);
                    if (self_au && (self_au == walk || inherits(self_au, walk)))
                        return access(self, string(mem->ident));
                }
                // an instance member with no self has no storage to read
                if (walk_is_record && !mem->is_static) continue;
                evar v = u(evar, mem);
                if (v) return (enode)v;
            }
            arg_list(walk, mem) {
...
AU_EXPORT void aether_e_assign_if_null(aether a, enode prop, Au value) {
    emit_guard;
    ...
    LLVMValueRef is_null = LLVMBuildICmp(B, LLVMIntEQ,
        _llvalue((enode)loaded), LLVMConstNull(LLVMTypeOf(_llvalue((enode)loaded))), "is_null");
...
AU_EXPORT bool etype_inherits(etype mdl, etype base);
...
static void resolve_context_members(enode target, map user_provides) {
    ...
    Au_t type = canonical(target)->autype;

    if (!is_rec(target->autype))
        return;
...
AU_EXPORT enode aether_e_operand(aether a, Au op, etype src_model) {
    emit_guard;
    ...
    if (!op) {
        ...
        return with_value(LLVMConstNull(lltype(src_model)), enode(mod, a, autype, src_model->autype,
            loaded, true));
    }
    
    if (isa(op) == typeid(string))
        return e_create(a, src_model,
            (Au)e_interpolate(a, (string)op), false);
    
    if (isa(op) == typeid(const_string))
        return e_create(a, src_model, (Au)e_operand_primitive(a, op), false); //e_create(a, src_model, (Au)e_operand_primitive(a, op), false);
    
    if (isa(op) == typeid(map)) {
        return e_create(a, src_model, op, false);
    }
    ...
    return src_model ? e_create(a, src_model, (Au)r, false) : r;
}

AU_EXPORT enode aether_e_null(aether a, etype mdl) {
    emit_guard;
    if (!mdl) mdl = etypeid(handle);
    verify(is_ptr(mdl) || is_func_ptr(mdl), "%o not compatible with null value", mdl);
    return with_value(LLVMConstNull(lltype(mdl)), enode(mod, a, loaded, true, autype, mdl->autype,
...
AU_EXPORT none aether_e_vector_init(aether a, etype element_type, enode vec, array nodes) {
    emit_guard;
    a->is_const_op = false;
    ...
    if (all_const) {
        ...
        LLVMTypeRef i8ptr = LLVMPointerTypeInContext(a->module_ctx, 0);
        LLVMValueRef dst  = LLVMBuildBitCast(B, _llvalue((enode)vec), i8ptr, "vec.dst");
        LLVMValueRef src  = LLVMBuildBitCast(B, G, i8ptr, "const.src");
        u64 byte_size = (u64)ln * (element_type->autype->abi_size / 8);
        ...
        for (int i = 0; i < ln; i++) {
            ...
            LLVMValueRef gep = LLVMBuildGEP2(B, lltype(element_type), _llvalue((enode)vec), &idx, 1, "elem.ptr");
            LLVMValueRef val = _llvalue((enode)node);
            if (!node->loaded)
...
AU_EXPORT none aether_e_setenv(aether a, symbol key, symbol val) {
    emit_guard;
    if (a->no_build) return;
...
static enode e_meta_a_node(aether a, Au meta_a) {
    if (!meta_a) return e_null(a, etypeid(Au_t));
    if (instanceof(meta_a, enode)) return (enode)meta_a;
    if (instanceof(meta_a, etype)) return e_typeid(a, (etype)meta_a);
    return e_typeid(a, u(etype, (Au_t)meta_a));
}

static enode e_meta_b_node(aether a, Au meta_b) {
    if (!meta_b) return e_null(a, etypeid(Au));
    if (instanceof(meta_b, enode)) return (enode)meta_b;
    if (instanceof(meta_b, etype)) return e_typeid(a, (etype)meta_b);
    if (instanceof(meta_b, shape)) {
        shape s = (shape)meta_b;
        array ar = array(alloc, s->count);
        for (int ii = 0; ii < s->count; ii++)
            push(ar, _i64(s->data[ii]));
        return e_create(a, etypeid(shape), (Au)m(
            "count", _i64(s->count),
            "data",  e_const_array(a, etypeid(i64), ar)), false);
    }
    return e_typeid(a, u(etype, (Au_t)meta_b));
}

AU_EXPORT enode aether_e_meta_ids(aether a, Au meta_a, Au meta_b) {
    emit_guard;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        LLVMBuildStore(B, _llvalue((enode)metas[i]), gep);
    }

    return with_value(arr_alloc, enode(mod, a, loaded, true, autype, etype_ptr(a, atype, null)->autype));
}

AU_EXPORT none aether_e_cond_return(aether a, enode cond, Au value) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return;
    ...
    LLVMValueRef test = _llvalue((enode)e_create(a, etypeid(bool), (Au)cond, false));
...
AU_EXPORT enode aether_e_expect_begin(aether a, enode cond) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, etypeid(bool));
    ...
    return with_value(LLVMBasicBlockAsValue(cont), enode(mod, a, autype, etypeid(bool)->autype, loaded, true));
}

AU_EXPORT enode aether_e_expect_end(aether a, enode ctx, enode msg) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, etypeid(bool));
    ...
    if (msg) {
        ...
        enode chars_node = chars_mem ? etype_access((etype)str_msg, string("chars")) : str_msg;
        enode cstr_val = enode_value(chars_node, true);

        LLVMTypeRef i8ptr  = LLVMPointerTypeInContext(a->module_ctx, 0);
        LLVMTypeRef i32_ty = LLVMInt32TypeInContext(a->module_ctx);
        // fputs to stderr
        LLVMTypeRef  fputs_args[] = { i8ptr, i8ptr };
        LLVMTypeRef  fputs_ty = LLVMFunctionType(i32_ty, fputs_args, 2, 0);
        ...
        LLVMValueRef args[] = { _llvalue((enode)cstr_val), stderr_val };
        LLVMBuildCall2(B, fputs_ty, fputs_fn, args, 2, "");
        LLVMValueRef nl = LLVMBuildGlobalStringPtr(B, "\n", "expect_nl");
        LLVMValueRef args_nl[] = { nl, stderr_val };
        LLVMBuildCall2(B, fputs_ty, fputs_fn, args_nl, 2, "");
    ...
    // a failed expect is a runtime error, not a crash: exit(1)
    LLVMTypeRef  i32_ty2  = LLVMInt32TypeInContext(a->module_ctx);
    LLVMTypeRef  void_ty  = LLVMVoidTypeInContext(a->module_ctx);
    LLVMTypeRef  exit_ty  = LLVMFunctionType(void_ty, &i32_ty2, 1, 0);
    LLVMValueRef exit_fn  = LLVMGetNamedFunction(a->module_ref, "exit");
    if (!exit_fn)
        exit_fn = LLVMAddFunction(a->module_ref, "exit", exit_ty);
    LLVMValueRef one = LLVMConstInt(i32_ty2, 1, 0);
    LLVMBuildCall2(B, exit_ty, exit_fn, &one, 1, "");
    LLVMBuildUnreachable(B);

    // ---- continue block ----
    LLVMPositionBuilderAtEnd(B, LLVMValueAsBasicBlock(_llvalue((enode)ctx)));

    // past the branch the condition held: expect reads as true
    return e_operand(a, (Au)_bool(true), etypeid(bool));
}

AU_EXPORT enode aether_e_expect(aether a, enode cond, enode msg) {
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, etypeid(bool));
...
AU_EXPORT catcher context_catcher(aether a);


AU_EXPORT enode enode_super(etype n, enode efn) {
    aether a = au_active(n->mod);
    return enode(
        mod, (aether)a, autype, n->autype->context, avoid_ftable, true, target, efn, is_super, true, loaded, true);
}

AU_EXPORT bool aether_e_fn_return(aether a, Au o) {
    emit_guard;
    ...
    if (cat && cat->rtype) {
        enode conv = e_create(a, cat->rtype, o, false);
        LLVMAddIncoming(_llvalue((enode)cat->phi), &(LLVMValueRef){_llvalue((enode)conv)},
            &(LLVMBasicBlockRef){LLVMGetInsertBlock(B)}, 1);
        LLVMBuildBr(B, cat->block);
        return true;
    } else {
        f->autype->has_return = true;
...
AU_EXPORT etype formatter_type(aether a, cstr input) {
    cstr p = input;
    ...
    switch (*p) {
        ...
        case 'p': return u(etype, etypeid(symbol)->autype->ptr);
    }
...
static etype func_target(etype fn) {
    aether a = au_active(fn->mod);
    if (fn->autype->is_imethod)
        return u(etype, fn->autype->context);
...
static etype ensure_pointer(etype t) {
    if (!t || t->autype->is_pointer || t->autype->is_class) return t;
    return etype_ptr(t->mod, t->autype, null);
}

AU_EXPORT enode aether_e_is(aether a, enode L, Au R) {
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, etypeid(bool));

    enode L_type =  e_offset(a, L, _i64(-sizeof(struct _Au)), false);
...
AU_EXPORT enode e_short_circuit_pair(aether a, OPType combine, enode L, enode R) {
    emit_guard;
    etype m_bool = etypeid(bool);
    if (a->no_build) return e_noop(a, m_bool);
    
    enode L_bool = e_create(a, m_bool, (Au)L, false);
    enode R_bool = e_create(a, m_bool, (Au)R, false);
    ...
    if (combine == OPType__or) {
        LLVMBuildCondBr(B, _llvalue((enode)L_bool), merge_block, eval_block);
    } else {
        LLVMBuildCondBr(B, _llvalue((enode)L_bool), eval_block, merge_block);
    }
    ...
    LLVMValueRef      vals[] = { _llvalue((enode)L_bool), _llvalue((enode)R_bool) };
    LLVMBasicBlockRef bbs[]  = { L_block,       R_block };
    LLVMAddIncoming(phi, vals, bbs, 2);
    
    return with_value(phi, enode(mod, a, autype, m_bool->autype, loaded, true));
}

AU_EXPORT enode convertible(etype fr, etype to);

// no disassemble!
AU_EXPORT enode aether_e_short_circuit(aether a, OPType optype, enode L) {
    emit_guard;
    if (a->no_build) {
    ...
    if (L_ref) {
        ...
        enode L_bool = e_create(a, etypeid(bool), (Au)L, false);
        ...
        if (optype == OPType__or)
            LLVMBuildCondBr(B, _llvalue((enode)L_bool), merge_block, eval_r_block);
        else
            LLVMBuildCondBr(B, _llvalue((enode)L_bool), eval_r_block, merge_block);
        LLVMPositionBuilderAtEnd(B, eval_r_block);
        enode R = (enode)a->parse_expr((Au)a, null);
        if (!R->loaded) R = enode_value(R, false);
        // R not a pointer (`ref && flag`): degrade result to bool
        if (LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)R))) != LLVMPointerTypeKind) {
            enode R_bool = e_create(a, etypeid(bool), (Au)R, false);
            LLVMBasicBlockRef R_block = LLVMGetInsertBlock(B);
            LLVMBuildBr(B, merge_block);
            LLVMPositionBuilderAtEnd(B, merge_block);
            ...
            LLVMAddIncoming(phi, vals, bbs, 2);
            return with_value(phi, enode(mod, a, autype, etypeid(bool)->autype, loaded, true));
        }
        verify(LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)L))) == LLVMPointerTypeKind,
        ...
        LLVMValueRef      vals[] = { _llvalue((enode)L), _llvalue((enode)R) };
        LLVMBasicBlockRef bbs[]  = { L_final,  R_block };
        LLVMAddIncoming(phi, vals, bbs, 2);
        return with_value(phi, enode(mod, a, autype, au_arg_type((Au)L), loaded, true,
            is_explicit_ref, true));
    }
    ...
    etype m_bool = etypeid(bool);
    enode L_conv = e_create(a, rtype, (Au)L, false);
    enode L_bool = e_create(a, m_bool, (Au)L, false);
    ...
    enode R_conv = e_create(a, rtype, (Au)R, false);
    LLVMBasicBlockRef R_block = LLVMGetInsertBlock(B);
    LLVMBuildBr(B, merge_block);
    ...
    LLVMValueRef      vals[] = { _llvalue((enode)L_conv), _llvalue((enode)R_conv) };
    LLVMBasicBlockRef bbs[]  = { L_final,       R_block };
    LLVMAddIncoming(phi, vals, bbs, 2);

    return with_value(phi, enode(
        mod,    a,
        autype,     rtype->autype,
        loaded, true));
}
...
AU_EXPORT none enode_inspect(enode a) {
}
...
AU_EXPORT enode aether_lambda_fcall(aether a, efunc mem, array user_args) {
    ...
    return e_fn_call(a, fn_ptr, args, false, false);
}

// a method on a typed vector that returns Au returns its element type
static Au_t call_elem_type(efunc fn, etype rtype, array args) {
    if (!fn || !fn->target || !rtype) return null;
    aether a   = au_active(fn->mod);
    etype  aue = etypeid(Au);
    if (rtype != aue && rtype->autype != aue->autype && rtype->autype != typeid(Au))
        return null;
    // the receiver rides as args[0]; fn->target is the self parameter
    enode t = (args && len(args)) ? (enode)instanceof(args->origin[0], enode) : null;
    if (!t) t = (enode)fn->target;
    Au_t  tau = t->autype ? au_arg_type((Au)t->autype) : null;
    if (!tau || !au_is_vector(tau)) return null;
    Au_t m = t->autype->meta.a ? t->autype->meta.a : (Au_t)t->meta_a;
    if (!m && t->autype->member_type == AU_MEMBER_VAR && t->autype->src)
        m = t->autype->src->meta.a;
    return m;
}

enode aether_e_fn_call(aether a, efunc fn, array args, bool is_super, bool is_poly) { sequencer
    emit_guard;
    static i64 test = 0;
    test++;
    ...
    a->is_const_op = false;
    if (a->no_build) {
        Au_t rtype_au = fn->autype->rtype;
        if (fn->autype->member_type == AU_MEMBER_VAR) {
            // funcptr variable: walk src until funcptr, then rtype is return type
            Au_t fp = fn->autype;
            while (fp && !fp->is_funcptr && fp->src) fp = fp->src;
            rtype_au = fp->rtype;
        }
        etype rt9 = u(etype, rtype_au);
        Au_t  e9  = call_elem_type(fn, rt9, args);
        return e_noop(a, e9 ? etype_prep(a, e9) : rt9);
    }
    // before the target is re-typed to the method's self parameter
    Au_t elem9 = call_elem_type(fn, u(etype, fn->autype->rtype), args);
    ...
    if (target_type && first_arg) {
        ...
        if (direct_Au) {
            ...
            bool value_is_ptr = _llvalue((enode)first_arg) &&
                LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)first_arg))) == LLVMPointerTypeKind;
    ...
    verify(funcptr || n_args == fn->autype->args.count ||
        (n_args > fn->autype->args.count && fn->autype->is_vargs),
        "arg count mismatch on %s.%s (given %i, expected %i)",
        fn->autype->context ? fn->autype->context->ident : "?", fn->autype->ident, n_args, fn->autype->args.count);
    ...
    LLVMValueRef  V          = _llvalue((enode)fn);

    // function pointer members need loading — V is the slot address, not the pointer value
    if (funcptr && V && !fn->loaded)
        V = LLVMBuildLoad2(B, LLVMPointerTypeInContext(a->module_ctx, 0), V, "fptr_load");

    bool is_abstract = fn->autype->is_abstract;
    Au_t ctx = fn->autype->context;
    while (ctx) {
        Au_t m = find_member(ctx, fn->autype->ident, 0, 0, false);
        if (m && m->is_abstract) {
            is_abstract = true;
            break;
        }
    ...
    bool extra_qualifiers = !fn->is_super && !is_super && fn->autype->access_type != interface_intern &&
        (is_abstract || (((!a->direct && first_arg) || (first_arg && first_arg->is_any)) && !first_is_alloc &&
         fn->autype->is_imethod && !funcptr &&
         !(target_type && target_type->target && target_type->target->avoid_ftable))) &&
         (!target_type || (!is_struct((Au)target_type->autype->src) && !is_struct((Au)target_type)));
        
    if ((is_poly || (first_arg && first_arg->is_any)) && extra_qualifiers) {
        ...
        LLVMValueRef instance = _llvalue((enode)first_arg);
        ...
        V = LLVMBuildBitCast(B, func_ptr_void, LLVMTypeOf(_llvalue((enode)fn)), "vmethod");
    }
    ...
    if (target_type) {
        ...
        Au_t au_type = target_type->autype;
        if (au_type->member_type == AU_MEMBER_VAR) au_type = au_type->src;

        etype cast_to = u(etype, au_type);
        enode r = e_create(a, cast_to, (Au)first_arg, false);
        arg_values[index++] = _llvalue((enode)r);
    }

    int fmt_idx = -1;
    int arg_idx = -1;
    for (int i = target_type ? 1 : 0; i < fn->autype->args.count; i++) {
        Au_t arg = (Au_t)fn->autype->args.origin[i];
        if (arg->is_formatter) {
            fmt_idx = i;
            arg_idx = i + 1;
            break;
        ...
        each (args, Au, arg_value) {
            ...
            if (is_lambda(fn) || (funcptr && fn->meta_a)) {
                ...
                if (arg_mem) {
                    ...
                    // an anonymous C pointer type (void*) may have no etype yet
                    if (!arg_t && !arg_mem->is_explicit_ref && au_arg_type((Au)arg_mem))
                        arg_t = etype_prep(a, au_arg_type((Au)arg_mem));
                    conv = arg_mem->is_explicit_ref ?
                        (enode)instanceof(arg_value, enode) :
                        e_create(a, arg_t, arg_value, false);
                    // a vector handed to a @T param passes its origin
                    if (arg_mem->is_explicit_ref && conv && conv->autype &&
                            au_arg_type((Au)conv->autype) &&
                    ...
                    if (conv && !conv->loaded && is_prim(conv->autype))
                        conv = enode_value(conv, true);
                    arg_t = (etype)conv->autype ? u(etype, conv->autype) : null;
                }
                ...
                if (!conv->loaded && not_ref &&
                    conv->autype && conv->autype->member_type == AU_MEMBER_VAR &&
                    conv->autype->src && (conv->autype->src->is_pointer || conv->autype->src->elements > 0)) {
                    LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
                    LLVMValueRef ld = LLVMBuildLoad2(B, ptr_ty, _llvalue((enode)conv), "arg_ptr_load");
    ...
    if (fmt_idx >= 0) {
        ...
        verify(fmt_str, "expected string literal at index %i for format-function: %o",
            fmt_idx, fn->autype->ident);
        arg_values[fmt_idx] = _llvalue((enode)fmt_node);
        int soft_args = 0;
        ...
        while  (p[0]) {
            if (p[0] == '%' && p[1] != '%') {
                ...
                enode conv      = e_create(a, arg_type, (Au)n_arg, false);
                arg_values[arg_idx + soft_args] = _llvalue((enode)conv);
                soft_args++;
                index    ++;
                p += 2;
            } else
        ...
        verify((istart + soft_args) == len(args), "%o: formatter args mismatch", fn->autype->ident);
    }
    
    Au_t rtype_au = fn->autype->rtype;
    if (funcptr && fn->autype->member_type == AU_MEMBER_VAR) {
        Au_t fp = fn->autype;
        while (fp && !fp->is_funcptr && fp->src) fp = fp->src;
        rtype_au = fp->rtype;
    }
    ...
    Au result_meta_a = target_type ? (Au)target_type->autype : null;
    ...
    // an Au out of a vector method is its element: a class element is
    // the object; any other comes back as a pointer at the packed value
    if (elem9) {
        result->autype = elem9;
        if (elem9->is_primitive && !elem9->is_pointer) {
            etype et9 = u(etype, elem9);
            if (!et9) et9 = etype_prep(a, elem9);
            _llvalue_set((enode)result, LLVMBuildLoad2(B, lltype(et9), R, "elem"));
        } else if (!elem9->is_class)
            result->loaded = false;
    }
...
AU_EXPORT enode constructable(etype fr, etype to);
...
AU_EXPORT enode castable(etype fr, etype to) {
    ...
    if (fr->autype && to->autype && fr->autype->is_class && to->autype->is_class &&
        (inherits(fr->autype, to->autype) || inherits(to->autype, fr->autype)) &&
        !constructable(fr, to))
        return (enode)true;
...
AU_EXPORT enode constructable(etype fr, etype to) {
    ...
    bool fr_cstr = canonical(fr) == etypeid(cstr)   ||
                   (is_ptr(fr) && fr->autype->src == typeid(i8));
    ...
    while (ctx) {
        ...
        for (int ii = 0; ii < ctx->members.count; ii++) {
            ...
            if  (!fn) continue;
            if (fn->autype->args.count != 2) {
                fn->autype->args.count = fn->autype->args.count;
            }
            verify(fn->autype->args.count == 2, "unexpected argument count for constructor"); // target + with-type
            Au_t with_arg = (Au_t)fn->autype->args.origin[1];
            ...
            Au_t fr_src   = fr->autype->is_pointer      ? fr->autype->src   : fr->autype;
            if ((with_arg->src == typeid(cstr) && fr_cstr) || (with_arg->src == typeid(symbol) && fr_sym) ||
                 with_arg->src == fr->autype || (with_arg->is_explicit_ref && with_src == fr_src))
                return (enode)u(enode, mem);
    ...
    if (realistic_count == 1 && fr->autype->is_realistic)
        return (enode)u(enode, realistic);

    if (integral_count == 1 && fr->autype->is_integral)
...
AU_EXPORT bool etype_inherits(etype mdl, etype base) {
    ...
    while (m) {
        ...
        if (!m->autype->context) break;
        if (u(etype, m->autype->context) == m) break;
        m = u(etype, m->autype->context);
    }
    return false;
}
...
AU_EXPORT enode convertible(etype fr, etype to) {
    ...
    // a vector hands a raw pointer (or @T slot) its packed elements (origin)
    if (ma->autype && au_is_vector(ma->autype) && !mb->autype->is_class &&
        (mb->autype->is_pointer || (mb->autype->traits & AU_TRAIT_POINTER) ||
         mb->is_explicit_ref || mb->autype->is_explicit_ref ||
         to->autype->is_pointer || (to->autype->traits & AU_TRAIT_POINTER) ||
         to->is_explicit_ref || to->autype->is_explicit_ref))
        return (enode)true;
    ...
    if (ma->autype->is_pointer && ma->autype->src && ma->autype->src->is_struct && mb->autype->is_struct &&
        ma->autype->src->typesize == mb->autype->typesize && mb->autype->typesize > 0)
        return (enode)true;
    if (ma->autype->is_struct && mb->autype->is_pointer && mb->autype->src && mb->autype->src->is_struct &&
        ma->autype->typesize == mb->autype->src->typesize && ma->autype->typesize > 0)
        return (enode)true;
    ...
    if (ma->autype->is_enum && (mb->autype->is_integral || mb->autype->is_realistic))
        return (enode)true;
    if (mb->autype->is_enum && (ma->autype->is_integral || ma->autype->is_realistic))
        return (enode)true;
    
    if (ma->autype->is_lambda && mb->autype == typeid(callback))
        return (enode)true;
    if ((!ma->autype->is_class && is_ptr(ma)) && ma->autype->src->is_struct && mb->autype == typeid(Au))
        return (enode)true;

    if (is_func(ma) && (mb == etypeid(ARef) || mb == etypeid(handle)))
        return (enode)true;
    if (ma->autype == typeid(Au_t) && mb->autype == typeid(ARef))
        return (enode)true;
    if (ma->autype->is_class && mb->autype->is_pointer) {
        enode mcast = castable(ma, mb);
        return mcast ? mcast : (enode)true;
    }
    // class ↔ class on the same inheritance chain: pointer cast, unless an
    // explicit constructor exists for this pair.
    if (ma->autype->is_class && mb->autype->is_class &&
        (inherits(ma->autype, mb->autype) || inherits(mb->autype, ma->autype)) &&
        !constructable(ma, mb))
        return (enode)true;
    if (is_ptr(ma->autype) && mb->autype == typeid(Au))
        return (enode)true;
    if ((is_func(ma) || is_func(resolve(ma)) || ma->autype->is_funcptr) && mb->autype->is_funcptr)
        return (enode)true;
    if (ma->autype->is_funcptr && mb->autype == typeid(bool))
        return (enode)true;
    ...
    if ((ma->autype == typeid(Au) || ma->autype == typeid(handle)) &&
        (mb->autype->is_struct || mb->autype->is_primitive || mb->autype->is_pointer || mb->autype->is_class))
        return (enode)true;

    // handle/Au into explicit ref targets (pointer storage)
    if ((ma->autype == typeid(handle) || ma->autype == typeid(Au)) && to->is_explicit_ref)
        return (enode)true;


    // trust the engineer to use their types in normal ways
    if (ma == etypeid(handle) && (is_class(mb) || mb->autype->is_pointer))
        return (enode)true;

    // schema table conversion
    if (ma->autype->is_schema && mb->autype->is_pointer)
        return (enode)true;
    if (mb->autype->is_schema && ma->autype->is_pointer)
        return (enode)true;
    if (ma->autype->is_schema && mb->autype == typeid(Au_t))
        return (enode)true;
    if (mb->autype->is_schema && ma->autype == typeid(Au_t))
        return (enode)true;

    // unloaded enodes are the same as reference enodes of pointer type
    enode fr_node = instanceof(fr, enode);
    if (fr_node && !fr_node->loaded && (pointer(a, (Au)fr_node->autype) == u(etype, mb->autype) || mb->autype == typeid(Au)))
        return (enode)true;

    // allow pointers of any sort, to boolean
    if  (is_ptr(ma) && mb->autype == typeid(bool)) return (enode)true;
    ...
    if (ma->autype->is_system || mb->autype == typeid(handle))
        return (enode)true;
    ...
    if (!constructable(ma, mb) && is_prim(ma) && inherits(mb->autype, typeid(string)))
        return (enode)true;

    if (is_rec(ma) || is_rec(mb)) {
        Au_t au_type = au_lookup("Au_t");
        if (is_ptr(ma) && ma->autype->src == au_type && mb->autype == au_type)
            return (enode)true;
        ...
        // can only do this if there are no other conversion
        if (!mcast && ma->autype->is_integral && ma->autype->is_pointer && (mb->autype->is_pointer || mb->autype->is_class))
            return (enode)true;
...
static bool is_addressable(enode e) {
    ...
    if (_llvalue((enode)e) && LLVMIsAGlobalValue(_llvalue((enode)e))) return true;
    return false;
}

AU_EXPORT enode eshape_from_indices(aether a, array indices) {
    emit_guard;
    ...
    for (i32 i = 0; i < count; i++) {
        ...
        enode idx_i64 = e_create(a, etypeid(i64), (Au)idx, false);
        ...
        LLVMBuildStore(B, _llvalue((enode)idx_i64), elem_ptr);
    }
    ...
    enode data_node = with_value(data_ptr, enode(mod, a, loaded, true, autype, etypeid(ref_i64)->autype));
    ...
    return e_fn_call(a, (efunc)u(efunc, shape_from_fn), a(count_node, data_node), false, false);
}

AU_EXPORT void aether_eputs(aether a, string output) {
    if (a->no_build) return;
...
AU_EXPORT void aether_listen_gate_create(aether a) {
    emit_guard;
...
AU_EXPORT void aether_listen_gate_store(aether a, enode cond) {
    emit_guard;
    if (a->no_build || !a->listen_gate || !cond || !_llvalue((enode)cond)) return;
    LLVMValueRef v  = _llvalue((enode)cond);
...
AU_EXPORT void aether_emit_listen_line(aether a, const char* msg) {
...
void aether_emit_listen_value(aether a, enode n) {
    ...
    if (!n || !_llvalue((enode)n) || !n->loaded)          return; // need the scalar value, not an address
    ...
    {
        LLVMValueRef _cf = LLVMGetBasicBlockParent(_ibb), _vf = null, _v = _llvalue((enode)n);
    ...
    LLVMTypeRef  vt     = LLVMTypeOf(_llvalue((enode)n));
    LLVMTypeKind vk     = LLVMGetTypeKind(vt);
    LLVMTypeRef  i32_ty = LLVMInt32TypeInContext(a->module_ctx);
    LLVMTypeRef  dbl_ty = LLVMDoubleTypeInContext(a->module_ctx);
    ...
    if (vk == LLVMIntegerTypeKind) {
        unsigned bits = LLVMGetIntTypeWidth(vt);
        if      (bits <  32) { arg = LLVMBuildZExt(B, _llvalue((enode)n), i32_ty, ""); conv = "%i";   }
        else if (bits == 32) { arg = _llvalue((enode)n);                              conv = "%i";   }
        else                 { arg = _llvalue((enode)n);                              conv = "%lli"; }
    } else if (vk == LLVMFloatTypeKind)  { arg = LLVMBuildFPExt(B, _llvalue((enode)n), dbl_ty, ""); conv = "%f"; }
    else if   (vk == LLVMDoubleTypeKind) { arg = _llvalue((enode)n);                              conv = "%f"; }
...
AU_EXPORT bool aether_has_listen(aether a) { return a->listen_active; }
AU_EXPORT void aether_clear_listen(aether a) {
...
AU_EXPORT bool aether_has_listen(aether a) { return false; }
...
enode etype_access(etype target, string name) { sequencer
    ...
    if (is_ptr(t) && !is_class(t)) t = u(etype, t->autype->src); // check source; t must always be a is_class or is_struct
    ...
    if (is_func((Au)m)) {
        ...
        if (is_prim(n->autype->src) && !is_addressable(n)) {
            etype res = resolve(n);
            LLVMValueRef temp = entry_alloca(a, lltype(res), "prim.recv.addr");
            LLVMBuildStore(B, _llvalue((enode)n), temp);
            n = value(pointer(a, (Au)n->autype->src), temp);
        }
    ...
    if (m->member_type == AU_MEMBER_ENUMV) {
        ...
        enode ev = u(enode, real_m);
        if (a->core > 0)
            return with_value(_llvalue(ev), enode(mod, a, autype, real_m, loaded, true));
        return ev;
    }

    enode n = u(enode, m);
    if (n && _llvalue((enode)n) && !n->loaded) {
        // cached node only stands for globals/statics; instance members GEP
        bool inst_member = m->context && !m->is_static &&
            (m->context->is_struct || m->context->is_class);
        if (!inst_member)
            return n;
    }
    ...
    if (a->no_build) {
        ...
        // au so downstream mem->autype->meta.a reads the member's declared meta
    ...
    LLVMValueRef tnode_v = tnode ? _llvalue(tnode) : null;
    ...
    if (m->context == (Au)typeid(Au)) {
        ...
        if (direct_au) {
            ...
            // a class instance data pointer steps back to its Au header
        ...
        LLVMValueRef au_typed = LLVMBuildBitCast(B, base, LLVMPointerType(_lltype_slot(au), 0), "au.typed");
        return with_value(LLVMBuildStructGEP2(B, _lltype_slot(au), au_typed, m->member_index, id->chars), enode(
            mod,    a, autype,       m,
    ...
    if (is_typeid && m->offset >= 0) {
        ...
        LLVMValueRef byte_ptr = LLVMBuildGEP2(B, i8_ty, tnode_v, &offset_val, 1, id->chars);
        return with_value(byte_ptr, enode(mod, a, autype, m, loaded, false, debug_id, id));
    }
    if (_lltype_slot(t) && has_scalable_vector(_lltype_slot(t)) && m->offset >= 0) {
        LLVMTypeRef  i8_ty      = LLVMInt8TypeInContext(a->module_ctx);
        LLVMValueRef offset_val = LLVMConstInt(LLVMInt32TypeInContext(a->module_ctx), m->offset, 0);
        LLVMValueRef byte_ptr   = LLVMBuildGEP2(B, i8_ty, tnode_v, &offset_val, 1, id->chars);
        return with_value(byte_ptr, enode(mod, a, autype, m, loaded, false, debug_id, id));
    ...
    if (_lltype_slot(t) && LLVMGetTypeKind(_lltype_slot(t)) == LLVMStructTypeKind) {
        unsigned num_elements = LLVMCountStructElementTypes(_lltype_slot(t));
        if (gep_index >= (int)num_elements) {
            Au_t parent = m->context ? m->context : t->autype;
            int ordinal = 0;
    ...
    if (is_loaded_struct) {
        if (_lltype_slot(t) && !LLVMIsOpaqueStruct(_lltype_slot(t))) {
            LLVMValueRef tmp = entry_alloca(a, _lltype_slot(t), "tmp.val");
            LLVMBuildStore(B, base, tmp);
            base = tmp;
        } else {
            LLVMValueRef extracted = LLVMBuildExtractValue(B, base, gep_index, id->chars);
            return with_value(extracted, enode(mod, a, autype, m, loaded, true, debug_id, id));
        }
    }
    ...
    return with_value(LLVMBuildStructGEP2(
            B, _lltype_slot(t), base,
            gep_index, id->chars), enode(
        mod,    a,
        autype,     m,
        meta_a, inst_meta_a,
        meta_b, inst_meta_b,
...
AU_EXPORT enode aether_e_inherited_access(aether a, enode instance, Au_t override_member) {
    ...
    if (!base_et || !_lltype_slot(base_et)) base_et = etype_prep(a, inherited->context);
    ...
    // so Canvas* + _lltype_slot(base_et) + inherited->member_index targets the right slot.
    string id = f(string, "override_access_%s", inherited->ident);
    LLVMValueRef gep = LLVMBuildStructGEP2(B, _lltype_slot(base_et),
        _llvalue((enode)instance), inherited->member_index, id->chars);
    return with_value(gep, enode(mod, a, autype, inherited, loaded, false));
}
...
AU_EXPORT etype implement_type_id(etype t);

enode e_runtime_type(enode instance) {
    emit_guard;
    aether a = au_active(instance->mod);

    if (a->no_build)
        return e_noop(a, etypeid(Au_t));

    LLVMTypeRef  i8_ptr_ty  = LLVMPointerTypeInContext(a->module_ctx, 0);
    LLVMValueRef i8_this    = LLVMBuildBitCast(B, _llvalue((enode)instance), i8_ptr_ty, "this_i8");
...
enode aether_e_typeid(aether a, etype mdl) { sequencer
    ...
    if (!mdl || (!mdl->autype->is_funcptr && !mdl->autype->is_lambda))
        mdl = canonical(mdl);

    if (a->debug_typeid) {
        fprintf(stderr, "  e_typeid[quants]: mdl=%s(%p) module=%s ptr=%d is_ptr=%d is_rec=%d\n",
             mdl->autype->ident, (void*)mdl->autype,
            mdl->autype->module ? mdl->autype->module->ident : "(null)",
            mdl->autype->is_pointer, is_ptr(mdl), (int)(uintptr_t)is_rec(mdl));
        a->debug_typeid = false;
    }

    // pointer types with their shapes need registration
    // for this we merely need to def() them
    if (is_ptr(mdl) && !is_rec(mdl) && mdl->autype->src->is_primitive && mdl->autype->src->ptr) {
        char ref_primitive[64];
        sprintf(ref_primitive, "ref_%s", mdl->autype->src->ident);
    ...
    if (instanceof(original, enode) && _llvalue((enode)((enode)original)) &&
    ...
    if (instanceof(mdl, enode) && _llvalue((enode)((enode)mdl))) { // so we can take in member variables
    ...
    etype to_register = (original->autype->is_alias && original->autype->ident) ? original : mdl;
    ...
    if (!to_register->autype->ident && to_register->autype->is_pointer && to_register->autype->src) {
        etype src_e = u(etype, to_register->autype->src);
        if (src_e && src_e->autype->ident)
            to_register = src_e;
    }

    if (!to_register->type_id) {
        bool is_c = au_is_c(to_register->autype);
        if (is_c || (is_module(to_register->autype) || !to_register->autype->module->is_au)) {
            implement_type_id(to_register);
        }
    }

    if (mdl->autype->is_alias && !mdl->type_id)
        implement_type_id(mdl);
    ...
    if (!n) {
        ...
        bool is_au_family = mdl->autype == typeid(Au) || mdl->autype == typeid(Au_t) ||
                            mdl->autype->is_schema || is_au_type((Au)mdl->autype);
        verify(is_au_family, "no type_id registered for %s",
               original->autype->ident ? original->autype->ident :
               (to_register->autype->ident ? to_register->autype->ident : "(anonymous)"));
...
AU_EXPORT bool is_map(etype t) {
    if (!instanceof(t, etype) || !t->autype) return false;
    return inherits(t->autype, typeid(map));
}
...
AU_EXPORT enode is_set(enode n, evar prop) {
    emit_guard;
    int index  = fbits_index(canonical((etype)n)->autype, prop->autype);
    aether a = au_active(n->mod);
    // interns have no af slot (af_index 0 -> -1): callers can never set
    // them, so their default always applies. the -1 previously shifted
    // into bit 63 and aliased every intern onto one phantom flag
    if (index < 0)
        return with_value(LLVMConstInt(LLVMInt1TypeInContext(a->module_ctx), 0, 0), enode(mod, a, autype, etypeid(bool)->autype));
    int word   = index / 64;
    int index0 = index % 64;
    ...
    LLVMValueRef fbits_struct = LLVMBuildStructGEP2(B, _lltype_slot(t), _llvalue((enode)n), 1, "fbits");
    ...
    return with_value(LLVMBuildICmp(B, LLVMIntNE, chk, zero, "fset"), enode(mod, a, autype, etypeid(bool)->autype));
}
...
AU_EXPORT void set_fbits_exact(enode n, u64* masks) {
    emit_guard;
    aether a = au_active(n->mod);
    etype i64_type = etypeid(i64);
    etype t  = canonical((etype)n);
...
AU_EXPORT void aether_apply_overrides(aether a, enode alloc) {
    ...
    // stores FIRST: each is gated on is_set of its own bit, so marking
    // before emitting suppressed every override default (black canvas —
    // Canvas.load_existing [ true ] never landed, passes cleared per paint).
    // the marks only need to precede the BASE init chain, which runs later.
    if (a->emit_overrides) {
        // construction-site stores are raw: hold_members owns them
        bool saved = a->init_props_retain_skip;
        a->init_props_retain_skip = true;
        { int _sd = emit_suspend();
...
AU_EXPORT void assign_if_cond(aether a, enode targ, enode cond, subprocedure expr_builder) {
    emit_guard;
    LLVMBasicBlockRef cur = LLVMGetInsertBlock(B);
    LLVMValueRef fn = LLVMGetBasicBlockParent(cur);
    ...
    LLVMBuildCondBr(B, _llvalue((enode)cond), merge_bb, apply_bb);
    LLVMPositionBuilderAtEnd(B, apply_bb);
    Au expr = sp_invoke((subprocedure)expr_builder, (Au)targ);
...
AU_EXPORT enode aether_e_init(aether a, enode alloc, map props, efunc ctr, enode ctr_input) {
    ...
    bool is_post_ctr = ctr && ctr->autype->is_default;
        ...
        pairs(props, i) {
            ...
            if (instanceof(k, enode)) {
                efunc f_set_prop = (efunc)u(efunc,
                    find_member(etypeid(Au)->autype, "set_property", AU_MEMBER_FUNC, 0, false));
                e_fn_call(a, f_set_prop, a(alloc, k, i->value), false, false);
            } else {
    ...
    if (alloc_type && is_class(alloc_type) &&
        ...
        while (ctx && ctx != typeid(Au)) {
            for (int i = 0; i < ctx->members.count; i++) {
                ...
                if (mb->member_type == AU_MEMBER_VAR && mb->is_required && !mb->is_context) {
    ...
    if (a->direct || !alloc->is_any) {
        ...
        each(chain, etype, mdl) {
            if (mdl->autype == typeid(Au)) continue;
            Au_t init_mem = find_member(mdl->autype, "init", AU_MEMBER_FUNC, 0, false);
...
enode e_create_from_map(aether a, etype t, map m) {
    ...
    if (is_m) {
        ...
        efunc f_initialize = (efunc)u(efunc,
            find_member(etypeid(Au)->autype, "initialize", AU_MEMBER_FUNC, 0, false));
        e_fn_call(a, f_initialize, a(res), false, false);
        pairs(m, i) {
            e_fn_call(a, f_mset, a(res, i->key, i->value), false, false);
        }
    } else {
...
enode e_create_from_array(aether a, etype t, array ar) {
    ...
    efunc f_push_vdata = (efunc)u(efunc, find_member(etypeid(array)->autype, "push_vdata", AU_MEMBER_FUNC, 0, true));
    ...
    bool  all_const = t->autype == typeid(array) && !a->building_initializer;
    int   ln = len(ar);
    ...
    if (all_const)
        for (int i = 0; i < ln; i++) {
            ...
            if (node && node->literal && _llvalue((enode)node) && LLVMIsConstant(_llvalue((enode)node)))
                continue;
            all_const = false;
            break;
        }
    ...
    if (all_const) {
        ...
        for (int i = 0; i < ln; i++) {
            ...
            LLVMValueRef v  = _llvalue((enode)node);
            ...
            if (LLVMTypeOf(v) != ety) {
                ...
                bool e_fp  = ek == LLVMFloatTypeKind || ek == LLVMDoubleTypeKind || ek == LLVMHalfTypeKind || ek == LLVMBFloatTypeKind;
                bool v_fp  = vk == LLVMFloatTypeKind || vk == LLVMDoubleTypeKind || vk == LLVMHalfTypeKind || vk == LLVMBFloatTypeKind;
        ...
        const_vector = with_value(glob, enode(mod, a, autype, ptr->autype, loaded, false));
    }
 
    if (t->autype == typeid(array)) {
        ...
        if (element_type && element_type->autype &&
            (element_type->autype->traits & AU_TRAIT_CLASS)) {
        ...
        // populate after init
        if (const_vector) {
            e_fn_call(a, f_push_vdata, a(res, const_vector, array_len, e_typeid(a, element_type)), false, false);
        } else {
            for (int i = 0; i < ln; i++) {
                ...
                enode conv = e_create(a, u(etype, t->meta_a), element, false);
                e_fn_call(a, f_push, a(res, conv), false, false);
            }
        }
    } else if (au_is_vector(t->autype)) {
        // a vector literal: init with its meta, then push each element
        res->meta_a   = t->meta_a;
        res->is_alloc = !a->building_initializer;
        res = e_init(a, res, null, null, null);
        efunc f_vpush = (efunc)u(efunc, find_member(typeid(vector), "push",
            AU_MEMBER_FUNC, 0, true));
        for (int i = 0; i < ln; i++) {
            enode conv = e_create(a, element_type, ar->origin[i], false);
            e_fn_call(a, f_vpush, a(res, conv), false, false);
        }
    } else if (ln == 0) {
        return e_create(a, t, (Au)null, false);
    } else if (ln == 1) {
        return e_create(a, t, ar->origin[0], false);
    } else {
        fault("e_create_from_array for %o (non-array, %i elements) not implemented", t, ln);
    }
...
enode e_convert_or_cast(aether a, etype output, enode input) {
    ...
    // a vector hands a raw pointer its packed elements: origin
    Au_t iau9 = input->autype ? au_arg_type((Au)input->autype) : null;
    Au_t og9 = output->autype ? au_arg_type((Au)output->autype) : null;
    if (iau9 && og9 && au_is_vector(iau9) && !og9->is_class &&
            (og9->is_pointer || (og9->traits & AU_TRAIT_POINTER) ||
             output->is_explicit_ref || output->autype->is_explicit_ref)) {
        enode obj9 = input->loaded ? input : enode_value(input, true);
        enode on9  = etype_access((etype)obj9, string("origin"));
        LLVMValueRef op9 = LLVMBuildLoad2(B, LLVMPointerTypeInContext(a->module_ctx, 0),
            _llvalue((enode)on9), "origin");
        return value(output, op9);
    }
    ...
    LLVMTypeRef typ = LLVMTypeOf(_llvalue((enode)input));
    LLVMTypeKind k = LLVMGetTypeKind(typ);

    // check if these are either Au_t class typeids, or actual compatible instances
    if (k == LLVMPointerTypeKind || input->autype->is_funcptr) {
        a->is_const_op = false;
        if (a->no_build) return e_noop(a, output);
        // C array types: ptr is already correct, just retype
        if (output->autype->elements > 0)
            return with_value(_llvalue((enode)input), enode(mod, a, loaded, input->loaded, autype, output->autype));
        if (output->autype == typeid(bool)) {
            ...
            if (!input->loaded) {
                ...
                LLVMTypeRef et = elem && _lltype_slot(elem)
                    ? lltype(elem)
                    : LLVMPointerTypeInContext(a->module_ctx, 0);
                LLVMValueRef loaded_v = LLVMBuildLoad2(B, et, _llvalue((enode)input), "slot_to_bool_load");
                return value(output,
                    LLVMBuildICmp(B, LLVMIntNE, loaded_v,
                        LLVMConstNull(et), "slot_to_bool"));
            }
            return value(output,
                LLVMBuildICmp(B, LLVMIntNE, _llvalue((enode)input),
                    LLVMConstNull(LLVMPointerTypeInContext(a->module_ctx, 0)), "ptr_to_bool"));
        }
        if ((is_prim(output) || output->autype->is_enum) && !output->autype->is_pointer && !is_ptr(output) &&
            input->autype->is_class && !input->autype->is_pointer) {
            // class object → primitive/enum: load primitive from object memory (e.g. __convert result)
            LLVMValueRef loaded_val = LLVMBuildLoad2(B, lltype(output), _llvalue((enode)input), "prim_from_ptr");
...
AU_EXPORT etype base_model(etype m) {
    if (!m) return null;
    aether a  = au_active(m->mod);
    Au_t   au = m->autype;
    while (au && au->src) {
        if (au == au->src)
            break;
...
AU_EXPORT etype get_etype(enode n) {
    aether a = au_active(n->mod);
    if (n->autype->member_type == AU_MEMBER_VAR && !n->is_explicit_ref)
        return u(etype, n->autype->src);
    return (etype)n;
}


// no_pool: the instance is allocated OUT of the auto-free vector. the pool
// frees what it holds at refs==0 with Au_free, not a drop -- so an object
// nothing owns at birth is reaped behind the refcount unless it opts out
enode aether_e_create(aether a, etype mdl, Au args, bool no_pool) { sequencer
    emit_guard;
    ...
    etype og_mdl = mdl;
    // a vector handed to a raw pointer type (void* included) passes its
    // origin; read before canonical() strips the pointer to its pointee
    if (input && input->autype && mdl && mdl->autype) {
        Au_t tg9 = au_arg_type((Au)mdl->autype);
        bool raw9 = tg9 && !tg9->is_class &&
            (tg9->is_pointer || (tg9->traits & AU_TRAIT_POINTER) ||
             mdl->is_explicit_ref || mdl->autype->is_explicit_ref);
        Au_t iv9 = raw9 ? au_arg_type((Au)input->autype) : null;
        if (iv9 && au_is_vector(iv9))
    ...
    // pointer identity: both are pointers to the same source type at the same ref level
    if (input && is_ptr(mdl) && is_ptr(canonical(input)) &&
        mdl->autype->src && mdl->autype->src == canonical(input)->autype->src &&
        ref_level2(mdl->autype) == ref_level2(canonical(input)->autype))
        return input;

    // C array decay: type with elements > 0 is already a pointer to its element type
    if (input && is_ptr(mdl) && canonical(input)->autype->elements > 0 &&
        canonical(input)->autype->src == mdl->autype->src) {
        ...
        if (!input->loaded && input->autype && input->autype->member_type == AU_MEMBER_VAR &&
            _llvalue((enode)input) && LLVMGetInstructionOpcode(_llvalue((enode)input)) == LLVMAlloca) {
            LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
            LLVMValueRef ld = LLVMBuildLoad2(B, ptr_ty, _llvalue((enode)input), "decay_load");
            return with_value(ld, enode(mod, a, autype, input->autype, loaded, true));
        }
        return input;
    }
    ...
    if (input && mdl == etypeid(Au) && input->literal) {
        etype ci = canonical(input);
        if (ci == etypeid(symbol) || ci == etypeid(cstr))
            return e_create(a, etypeid(string), (Au)input, false);
        // a single-quoted single char parses as unichar; same contract
        if (ci == etypeid(unichar) && *(u32*)input->literal < 128) {
            char one[2] = { (char)*(u32*)input->literal, 0 };
            return e_create(a, etypeid(string), (Au)const_string(chars, one), false);
        }
    }
    ...
    if (is_au_t((Au)mdl) && is_au_t(args)) {
        ...
        return value(mdl,
            LLVMBuildBitCast(B, _llvalue((enode)n), lltype(mdl), "is_au_t_cast"));
    }
    ...
    if (is_lambda((Au)mdl)) {
        ...
        etype ctx_type = u(etype, n_mdl->context_node->autype->src);
        // a context made during codegen has no type id: allocate its
        // bytes, typed as the struct for the stores below
        enode ctx_alloc;
        bool  ctx_bytes = !resolve_typeid(a, (Au)ctx_type);
        if (!ctx_bytes)
        ...
        int ctx_index = 0;
        verify(len((array)args) == n_mdl->autype->members.count,
            "lambda initialization member-count (%i) != user-provided args (%i)",
            n_mdl->autype->members.count, len((array)args));
        
        members(n_mdl->autype, mem) {
            if (mem->member_type != AU_MEMBER_VAR) continue;
        ...
        // untyped context: the lambda names the object slots it drops
        if (ctx_bytes) {
            u64 mask = 0;
            members(n_mdl->autype, cm) {
                if (cm->member_type != AU_MEMBER_VAR) continue;
                // a capture's src is the outer variable: take its type
                Au_t ct = cm->src;
                while (ct && ct->member_type == AU_MEMBER_VAR && ct->src) ct = ct->src;
                ct = ct ? au_arg_type((Au)ct) : null;
                if (!ct || !ct->is_class || ct->is_c) continue;
                Au_t fm = find_member(ctx_type->autype, cm->ident, AU_MEMBER_VAR, 0, false);
                if (!fm) continue;
                u64 off = LLVMOffsetOfElement(ll_td(a), _lltype_slot(ctx_type), fm->member_index);
                if (off % 8 == 0 && off / 8 < 64) mask |= (u64)1 << (off / 8);
            }
            if (mask)
                e_assign(a, access(res, string("ctx_objs")),
                    (Au)e_operand(a, _u64(mask), etypeid(u64)), OPType__assign);
        }
    ...
    if (input) {
        ...
        if (!is_system(input) && (!is_ptr(input) || input_estr) && mdl->autype == typeid(Au) &&
            ...
            if (!input_estr && input->loaded && _llvalue((enode)input) &&
                    LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)input))) == LLVMPointerTypeKind)
                return value(canonical(mdl),
                    LLVMBuildBitCast(B, _llvalue((enode)input), lltype(canonical(mdl)), "ref_to_au"));
            ...
            boxed->autype = is_enumerable ?
                u(etype, input_type->autype->src)->autype : canonical(input_type)->autype;
            if (!input->loaded && is_struct(input) && _lltype_slot(input_type)) {
                // struct boxing: memcpy from alloca into boxed data
                u64 sz = LLVMABISizeOfType(ll_td(a), _lltype_slot(input_type));
                LLVMBuildMemCpy(B, _llvalue((enode)boxed), 1, _llvalue((enode)input), 1,
                    LLVMConstInt(LLVMInt64TypeInContext(a->module_ctx), sz, false));
            } else {
                LLVMBuildStore(B, _llvalue((enode)input), _llvalue((enode)boxed));
            }
            return value(canonical(mdl), 
                LLVMBuildBitCast(B, _llvalue((enode)boxed), lltype(canonical(mdl)), "box_to_au"));
        }


        // a byte vector (vec i8 [ n ]) constructs a string-taking type
        // through its chars, never by casting the vector object
        {
            Au_t iv9 = canonical(input) ? au_arg_type((Au)canonical(input)->autype) : null;
        ...
        if (!fmem && input->literal && isa(input->literal) == typeid(unichar) &&
            ...
            return e_create(a, mdl, (Au)s_node, false);
        }
        ...
        if (!fmem && canonical(input) && canonical(input)->autype == typeid(tokens) &&
            ...
            (constructs_with(canonical(mdl)->autype, typeid(string)) ||
             constructs_with(canonical(mdl)->autype, typeid(cstr))   ||
             constructs_with(canonical(mdl)->autype, typeid(symbol)))) {
            enode joined = e_create(a, etypeid(string), (Au)input, false);
            return e_create(a, mdl, (Au)joined, false);
        }
        ...
        if (!fmem && canonical(input) && canonical(input)->autype == typeid(string)) {
            ...
            if (f_convert) {
                ...
                enode result = e_fn_call(a, u(efunc, f_convert), a(type_node, input), false, false);
        ...
        if (fn && fn->autype->member_type == AU_MEMBER_CONSTRUCT) {
            ctr = (efunc)fmem;
        } else if (fn && fn->autype->member_type == AU_MEMBER_CAST) {
    ...
    if (is_ptr(mdl) && is_struct(mdl->autype->src) && static_a) {
        static int ident = 0;
        char name[32];
        sprintf(name, "static_arr_%i", ident++);

        i64          ln     = len(static_a);
        etype        emdl   = (etype)u(etype, mdl->autype->src);
        ...
        for (i32 i = 0; i < ln; i++) {
            ...
            verify(LLVMIsConstant(_llvalue((enode)n)), "static_array must contain constant statements");
            verify(n->autype == mdl->autype->src, "type mismatch");
            elems[i] = _llvalue((enode)n);
        }
        LLVMValueRef init = LLVMConstArray(lltype(emdl), elems, ln);
        LLVMSetInitializer(G, init);
        LLVMSetGlobalConstant(G, 1);
        free(elems);
        res = with_value(G, enode(mod, a, loaded, false, autype, mdl->autype));
        ...
        if (instanceof(args, array)) {
            ...
            enode alloc = e_alloc(a, m, no_pool);
        ...
        if (canonical(mdl) == etypeid(Au) && u(etype, ctr->autype->context) != canonical(mdl)) {
            mdl = u(etype, ctr->autype->context);
        }

        if (is_struct(mdl)) {
            res = with_value(entry_alloca(a, lltype(mdl), "alloca-ptr"), enode(mod, a, loaded, false, autype, mdl->autype));
            // we have to call the constructor on this one..
            // the value is the pointer though since its not loaded
            LLVMBuildStore(B, LLVMConstNull(lltype(mdl)), _llvalue((enode)res));
            e_fn_call(a, ctr, a(res, input), false, false);

        } else {
            enode alloc = e_alloc(a, mdl, no_pool);
            alloc->is_alloc = !a->building_initializer;
        ...
        if (is_arr && mdl->autype->elements > 0 && mdl->autype->src) {
            array ar = (array)args;
            int ln = len(ar);
            etype elem_type = u(etype, mdl->autype->src);
            if (!elem_type) elem_type = etype_prep(a, mdl->autype->src);
            LLVMTypeRef elem_lltype = lltype(elem_type);
            int elems = mdl->autype->elements;
            ...
            for (int i = 0; i < ln; i++) {
                ...
                val = e_create(a, elem_type, (Au)val, false);
                LLVMValueRef idx[] = {
                ...
                if (is_struct(elem_type) && !val->loaded) {
                    u64 sz = LLVMABISizeOfType(ll_td(a), elem_lltype);
                    LLVMBuildMemCpy(B, gep, 0, _llvalue((enode)val), 0,
                        LLVMConstInt(LLVMInt64TypeInContext(a->module_ctx), sz, 0));
                } else {
                    LLVMBuildStore(B, _llvalue((enode)val), gep);
                }
            }
            res = with_value(alloc, enode(mod, a, loaded, false, autype, mdl->autype));
            return res;
        }
        ...
        etype_prep(a, resolve(rmdl)->autype);

        if (is_struct(mdl) || is_ref_struct) {
            int field_count = LLVMCountStructElementTypes(_lltype_slot(rmdl));
            ...
            if (is_arr) {
                ...
                // if arg count != field count, treat as single-value construction
                if (ln != field_count && ln == 1) {
                    enode single = (enode)ar->origin[0];
                    if (single) {
                        enode result = e_create(a, rmdl, (Au)single, false);
                        free(fields);
                        return result;
                    }
                }
                ...
                for (int i = 0; i < field_count; i++) {
                    ...
                    int current_index = 0;
                    for (int ri = 0; ri < rmdl->autype->members.count; ri++) {
                        Au_t smem = (Au_t)rmdl->autype->members.origin[ri];
                    ...
                    enode op = val ? e_operand(a, val, field_type) :
                                     e_create(a, field_type, null, false);
                    if (!op->literal) all_const = false;
                    fields[i] = _llvalue((enode)op);
                }
                if (ln < field_count)
                    field_count = ln;
                ...
                pairs(imap, i) {
                    ...
                    if (!value->loaded && value->autype && value->autype->member_type == AU_MEMBER_VAR &&
                        value->autype->src && (value->autype->src->is_pointer || value->autype->src->elements > 0) &&
                        _llvalue((enode)value) && LLVMGetInstructionOpcode(_llvalue((enode)value)) == LLVMAlloca &&
                        LLVMGetTypeKind(LLVMGetAllocatedType(_llvalue((enode)value))) == LLVMPointerTypeKind) {
                        LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
                ...
                for (int i = 0; i < field_count; i++) {
                    ...
                    LLVMTypeRef tr = LLVMStructGetTypeAtIndex(_lltype_slot(rmdl), i);
                    fields[i] = value ? _llvalue((enode)value) : LLVMConstNull(tr);
                }
            }

            // struct alloca + field stores
            res = with_value(entry_alloca(a, lltype(mdl), "alloca-mdl"), enode(mod, a, loaded, false, autype, mdl->autype));
            res = e_zero(a, res);
            int mem_idx = 0;
            for (int i = 0; i < field_count; i++) {
                if (fields[i] && !LLVMIsNull(fields[i])) {
                    LLVMValueRef gep = LLVMBuildStructGEP2(B, lltype(mdl), _llvalue((enode)res), i, "");
                    // find the member to check for C array fields
                    Au_t smem = null;
                    int ci = 0;
                    for (int ri = 0; ri < rmdl->autype->members.count; ri++) {
                        Au_t m = (Au_t)rmdl->autype->members.origin[ri];
                    ...
                    if (smem && smem->elements > 0) {
                        ...
                        u64 sz = LLVMABISizeOfType(ll_td(a), arr_ty);
...
AU_EXPORT none copy_lambda_info(enode mem, enode lambda_fn) {
    ...
    _llvalue_set((enode)mem, _llvalue((enode)lambda_fn));
}


AU_EXPORT enode aether_e_vector(aether a, etype t, enode shape_data) {
    etype tc = canonical(t);

    efunc f_alloc    = (efunc)u(efunc,
        find_member(etypeid(Au)->autype, "alloc_vector", AU_MEMBER_FUNC, 0, false));
    enode ma = e_meta_a_node(a, t->meta_a ? t->meta_a : (tc ? tc->meta_a : null));
    enode mb = e_meta_b_node(a, t->meta_b ? t->meta_b : (tc ? tc->meta_b : null));
    bool is_shape = shape_data && canonical(shape_data) && canonical(shape_data)->autype == typeid(shape);
    enode n_src; Au n_line, n_seq;
    alloc_origin_args(a, &n_src, &n_line, &n_seq);
    if (is_shape)
...
AU_EXPORT enode aether_e_const_array(aether a, etype mdl, array arg) {
    emit_guard;
    etype atype = etypeid(Au_t);
    etype vector_type = etype_ptr(a, mdl->autype, null);
    a->is_const_op = false;
    ...
    for (i32 i = 0; i < ln; i++) {
        ...
        elems[i] = _llvalue((enode)n);  // each is an Au_t*
    }
    ...
    return with_value(G, enode(mod, a, loaded, true, autype, vector_type->autype));
}

AU_EXPORT enode aether_e_default_value(aether a, etype mdl) {
    return aether_e_create(a, mdl, null, false);
}


AU_EXPORT enode aether_e_zero(aether a, enode n) {
    emit_guard;
    etype      mdl = (etype)n;
    LLVMValueRef v = _llvalue((enode)n);
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, mdl);
    LLVMValueRef zero   = LLVMConstInt(LLVMInt8TypeInContext(a->module_ctx), 0, 0);          // value for memset (0)
    LLVMValueRef size   = LLVMConstInt(LLVMInt64TypeInContext(a->module_ctx), mdl->autype->abi_size / 8, 0); // size of alloc
    LLVMValueRef memset = LLVMBuildMemSet(B, v, zero, size, 1);
    return n;
}


AU_EXPORT etype prefer_mdl(etype m0, etype m1) {
    aether a = au_active(m0->mod);
    if (m0 == m1)
        return m0;
    ...
    if (inherits(m0->autype, m1->autype))
        return m1;
...
AU_EXPORT enode aether_e_ternary(aether a, enode cond_expr, enode true_expr, enode false_expr) {
    ...
    if (false_expr && !true_expr->loaded &&
        LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)true_expr))) == LLVMPointerTypeKind) {
    ...
    LLVMValueRef true_value = !false_expr ? _llvalue((enode)cond_expr) : _llvalue((enode)true_expr);
    ...
    if (false_expr && !false_expr->loaded &&
        LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)false_expr))) == LLVMPointerTypeKind) {
    ...
    enode default_expr = !false_expr ? e_create(a, (etype)rmdl, (Au)true_expr, false) : null;
    if (false_expr) false_expr = e_create(a, (etype)rmdl, (Au)false_expr, false);
    LLVMValueRef false_value = !false_expr ? _llvalue((enode)default_expr) : _llvalue((enode)false_expr);
    ...
    if (LLVMGetTypeKind(result_type) == LLVMPointerTypeKind) {
        ...
        if (!st || !_lltype_slot(st)) st = u(etype, rau);
        if (!st || !_lltype_slot(st)) st = etype_prep(a, term);
        LLVMTypeRef rt = st ? _lltype_slot(st) : NULL;
        if (rt && !is_cls) {
    ...
    return with_value(result_val, enode(mod, mod, loaded, true, autype, rmdl->autype));
}

AU_EXPORT enode aether_e_coalesce_deferred(aether a, enode cond_expr, array true_tokens, subprocedure expr_builder) {
    emit_guard;
    ...
    if (a->no_build) {
        enode true_node = (enode)sp_invoke((subprocedure)expr_builder, (Au)true_tokens);
    ...
    enode true_node = (enode)sp_invoke((subprocedure)expr_builder, (Au)true_tokens);
    LLVMValueRef true_value = _llvalue((enode)true_node);
    ...
    return with_value(phi_node, enode(mod, a, loaded, !is_struct((Au)true_node->autype), autype, true_node->autype));
}

AU_EXPORT enode aether_e_ternary_deferred(aether a, enode cond_expr, array true_tokens, array false_tokens, subprocedure expr_builder) {
    emit_guard;
    // build both branches deferred so member accesses don't happen before the null check
    if (a->no_build) {
        sp_invoke((subprocedure)expr_builder, (Au)true_tokens);
        enode false_node = (enode)sp_invoke((subprocedure)expr_builder, (Au)false_tokens);
    ...
    if (!true_node->loaded &&
        LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)true_node))) == LLVMPointerTypeKind) {
    ...
    if (!false_node->loaded &&
        LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)false_node))) == LLVMPointerTypeKind) {
    ...
    false_node = e_create(a, rmdl, (Au)false_node, false);
    LLVMValueRef false_value = _llvalue((enode)false_node);
    LLVMBasicBlockRef false_final = LLVMGetInsertBlock(B);
    LLVMBuildBr(B, merge_block);
    ...
    _llvalue_set((enode)result, result_val);
    return result;
}

AU_EXPORT enode aether_e_builder(aether a, subprocedure cond_builder) {
    emit_guard;
    if (!a->no_build) {
        LLVMBasicBlockRef block = LLVMGetInsertBlock(B);
        LLVMPositionBuilderAtEnd(B, block);
    }
    enode n = (enode)sp_invoke((subprocedure)cond_builder, null);
    return n;
}
        ...
        subprocedure    body_builder)
{
    ...
    LLVMTypeRef       Ty    = LLVMTypeOf(_llvalue((enode)switch_val));
    LLVMBasicBlockRef entry = LLVMGetInsertBlock(B);
    LLVMValueRef      fn    = LLVMGetBasicBlockParent(entry);
    ...
    LLVMValueRef SW =
        LLVMBuildSwitch(B, _llvalue((enode)switch_val), default_block, cases->count);
    ...
    pairs(cases, i) {
        ...
        enode key_node = (enode)sp_invoke((subprocedure)expr_builder, i->key);
        Au   au_value = key_node->literal;
        Au_t au_type  = null;

        if (!au_value && (is_enum(key_node->autype) || key_node->autype->member_type == AU_MEMBER_ENUMV)) {
            au_value = (Au)key_node->autype->value;
            au_type  = canonical(key_node)->autype->src;
        }
    ...
    pairs(case_blocks, p) {
        ...
        sp_invoke((subprocedure)body_builder, (Au)body_tokens);
    ...
    if (def_block) {
        ...
        sp_invoke((subprocedure)body_builder, (Au)def_block);
        if (!LLVMGetBasicBlockTerminator(LLVMGetInsertBlock(B))) {
            LLVMBuildBr(B, switch_cat->block);
            has_merge = true;
        }
        ...
        subprocedure    body_builder)
{
    ...
    enode   switch_val = e_expr; // sp_invoke((subprocedure)expr_builder, expr);
    catcher switch_cat = catcher(mod, a,
    ...
    pairs(case_blocks, i) {
        ...
        if (!a->last_return)
            LLVMBuildCondBr(B, eq, case_cat->block, next); // never emit a break when you have a return prior
        else {
            a->last_return = a->last_return;
        }
        cur = next;
    }
                   ...
                   bool reverse)
{
    ...
    if (in_expr && (inherits(in_expr->autype, typeid(map)) || inherits(in_expr->autype, typeid(list)) || inherits(in_expr->autype, typeid(array)))) {
        ...
        {   // ---- null guard: skip loop if collection is null ----
            LLVMValueRef coll_ptr = _llvalue((enode)in_expr);
    ...
    if (in_expr && (inherits(in_expr->autype, typeid(map)) || inherits(in_expr->autype, typeid(list)))) {
        ...
        LLVMTypeRef  cursor_type = LLVMPointerType(_lltype_slot(item_type), 0);
        LLVMValueRef cursor      = entry_alloca(a, cursor_type, "cursor");
        LLVMValueRef start_val   = start_node->loaded ? _llvalue((enode)start_node) :
            LLVMBuildLoad2(B, cursor_type, _llvalue((enode)start_node), "start.val");
        ...
        enode cur_enode = value(pointer(a, (Au)item_type->autype),
        ...
        if (val_var) {
            ...
            enode val_cast = e_create(a, canonical(val_var), (Au)val_node, false);
            LLVMBuildStore(B, _llvalue((enode)val_cast), _llvalue((enode)val_var));
        }
        ...
        if (key_var) {
            ...
            enode key_cast = e_create(a, canonical(key_var), (Au)key_node, false);
            LLVMBuildStore(B, _llvalue((enode)key_cast), _llvalue((enode)key_var));
        }

        if (st && a->coverage) aether_emit_block_probe(a, st->probe_id);

        token saved_origin = a->statement_origin;
        sp_invoke((subprocedure)body_builder, (Au)body_exprs);
        ...
        debug_emit(a);
        enode step_enode = value(pointer(a, (Au)item_type->autype),
            LLVMBuildLoad2(B, cursor_type, cursor, "cur.step"));
        enode adv_node = etype_access((etype)step_enode, string(reverse ? "prev" : "next"));
        LLVMValueRef adv_val = adv_node->loaded ? _llvalue((enode)adv_node) :
            LLVMBuildLoad2(B, cursor_type, _llvalue((enode)adv_node), "adv.val");
        LLVMBuildStore(B, adv_val, cursor);
        LLVMBuildBr(B, cond);

    } else if (in_expr && au_is_vector(in_expr->autype)) {

        // ---- vector: index loop over count/origin, packed by element ----
        Au_t elem_au = in_expr->autype->meta.a ? in_expr->autype->meta.a :
            (in_expr->meta_a ? (Au_t)in_expr->meta_a : typeid(Au));
        etype elem_t = u(etype, elem_au);
        if (!elem_t) elem_t = etype_prep(a, elem_au);
        bool  managed = elem_au->is_class && !elem_au->is_c;
        LLVMTypeRef  ptr_ty   = LLVMPointerTypeInContext(a->module_ctx, 0);
        LLVMTypeRef  i32_type = LLVMInt32TypeInContext(a->module_ctx);
        // a null vector iterates zero times — guard before any field read
        LLVMValueRef obj9 = _llvalue((enode)in_expr);
        if (!in_expr->loaded) obj9 = LLVMBuildLoad2(B, ptr_ty, obj9, "vec_obj");
        LLVMValueRef      fn9  = LLVMGetBasicBlockParent(LLVMGetInsertBlock(B));
        LLVMBasicBlockRef hdr9 = LLVMAppendBasicBlockInContext(a->module_ctx, fn9, "for.vec.hdr");
        LLVMBuildCondBr(B, LLVMBuildICmp(B, LLVMIntNE, obj9,
            LLVMConstPointerNull(ptr_ty), "vec.nn"), hdr9, merge);
        LLVMPositionBuilderAtEnd(B, hdr9);
        enode count_node  = etype_access((etype)in_expr, string("count"));
        enode origin_node = etype_access((etype)in_expr, string("origin"));
        LLVMTypeRef  slot_ty  = managed ? ptr_ty : lltype(elem_t);
        LLVMValueRef idx       = entry_alloca(a, i32_type, "for.idx");
        LLVMValueRef count_val = LLVMBuildLoad2(B, i32_type, _llvalue((enode)count_node), "count");
        if (reverse)
            LLVMBuildStore(B, LLVMBuildSub(B, count_val,
                LLVMConstInt(i32_type, 1, false), "idx.init"), idx);
        else
            LLVMBuildStore(B, LLVMConstInt(i32_type, 0, false), idx);
        LLVMBuildBr(B, cond);

        LLVMPositionBuilderAtEnd(B, cond);
        LLVMValueRef iv  = LLVMBuildLoad2(B, i32_type, idx, "idx");
        LLVMValueRef cmp = reverse
            ? LLVMBuildICmp(B, LLVMIntSGE, iv, LLVMConstInt(i32_type, 0, false), "idx.ge.0")
            : LLVMBuildICmp(B, LLVMIntSLT, iv, count_val, "idx.lt.count");
        LLVMBuildCondBr(B, cmp, body, merge);

        LLVMPositionBuilderAtEnd(B, body);
        if (st && a->coverage) aether_emit_block_probe(a, st->probe_id);
        LLVMValueRef ib = LLVMBuildLoad2(B, i32_type, idx, "idx.body");
        LLVMValueRef origin = LLVMBuildLoad2(B, ptr_ty, _llvalue((enode)origin_node), "origin");
        LLVMValueRef ep = LLVMBuildGEP2(B, slot_ty, origin, &ib, 1, "elem.ptr");
        if (val_var) {
            LLVMValueRef ev = LLVMBuildLoad2(B, slot_ty, ep, "elem");
            enode en = with_value(ev, enode(mod, a, autype, elem_au, loaded, true));
            enode vc = e_create(a, canonical(val_var), (Au)en, false);
            if (!vc->loaded && is_prim((Au)canonical(vc)->autype))
                vc = enode_value(vc, true);
            LLVMBuildStore(B, _llvalue((enode)vc), _llvalue((enode)val_var));
        }
        if (key_var) {
            LLVMValueRef ib64 = LLVMBuildSExt(B, ib, LLVMInt64TypeInContext(a->module_ctx), "idx.i64");
            enode idx_node = value(etypeid(i64), ib64);
            enode ic = e_create(a, canonical(key_var), (Au)idx_node, false);
            LLVMBuildStore(B, _llvalue((enode)ic), _llvalue((enode)key_var));
        }
        token saved_origin = a->statement_origin;
        sp_invoke((subprocedure)body_builder, (Au)body_exprs);
        a->statement_origin = saved_origin;
        ...
        LLVMPositionBuilderAtEnd(B, step);
        debug_emit(a);
        LLVMValueRef is2 = LLVMBuildLoad2(B, i32_type, idx, "idx.step");
        LLVMValueRef nx  = reverse
            ? LLVMBuildSub(B, is2, LLVMConstInt(i32_type, 1, false), "idx.dec")
            : LLVMBuildAdd(B, is2, LLVMConstInt(i32_type, 1, false), "idx.inc");
        LLVMBuildStore(B, nx, idx);
        LLVMBuildBr(B, cond);

    } else if (in_expr && inherits(in_expr->autype, typeid(array))) {
        ...
        LLVMValueRef elem_ptr = LLVMBuildGEP2(B, 
            ptr,
            LLVMBuildLoad2(B, ptr, _llvalue((enode)origin), "count"), &idx_body, 1, "elem.ptr");
        ...
        if (val_var) {
            ...
            enode val_cast = e_create(a, cv, (Au)val_node, false);
            if (!val_cast->loaded && is_struct(cv) && !is_ptr(cv)) {
                LLVMValueRef sv = LLVMBuildLoad2(B, lltype(cv), _llvalue((enode)val_cast), "struct_elem");
                LLVMBuildStore(B, sv, _llvalue((enode)val_var));
            } else {
                LLVMBuildStore(B, _llvalue((enode)val_cast), _llvalue((enode)val_var));
            }
        }
        ...
        if (key_var) {
            validate(key_var->autype->src == typeid(i64) || key_var->autype->src == typeid(u64),
            ...
            enode idx_cast = e_create(a, canonical(key_var), (Au)idx_node, false);
            LLVMBuildStore(B, _llvalue((enode)idx_cast), _llvalue((enode)key_var));
        }
    ...
    } else if (in_expr && inherits(in_expr->autype, typeid(micro))) {
        ...
        LLVMValueRef count_val = LLVMBuildLoad2(B, i32_type, _llvalue((enode)count_node), "micro.count");
        if (reverse) {
            LLVMValueRef init_val = LLVMBuildSub(B, count_val, LLVMConstInt(i32_type, 1, false), "idx.init");
            LLVMBuildStore(B, init_val, idx);
        } else {
        ...
        LLVMValueRef origin_ptr = LLVMBuildLoad2(B, ptr_type, _llvalue((enode)origin_node), "micro.origin");
        ...
        if (val_var) {
            ...
            enode val_cast = e_create(a, cv, (Au)val_node, false);
            if (!val_cast->loaded && is_struct(cv) && !is_ptr(cv)) {
                LLVMValueRef sv = LLVMBuildLoad2(B, lltype(cv), _llvalue((enode)val_cast), "micro.struct_elem");
                LLVMBuildStore(B, sv, _llvalue((enode)val_var));
            } else {
                LLVMBuildStore(B, _llvalue((enode)val_cast), _llvalue((enode)val_var));
            }
        ...
        if (len(init_exprs))
            sp_invoke((subprocedure)init_builder, (Au)init_exprs);
        ...
        // ---- cond ----
        LLVMPositionBuilderAtEnd(B, cond);
        enode cond_res = (enode)sp_invoke((subprocedure)cond_builder, (Au)cond_exprs);
        LLVMValueRef cond_val = _llvalue((enode)e_create(a, etypeid(bool), (Au)cond_res, false));
        ...
        debug_emit(a);
        if (len(step_exprs))
            sp_invoke((subprocedure)step_builder, (Au)step_exprs);
        LLVMBuildBr(B, cond);
    }
...
AU_EXPORT enode aether_e_noop(aether a, etype mdl) {
    enode op = enode(mod, a, loaded, false,
        autype, mdl ? mdl->autype : etypeid(none)->autype,
    ...
    subprocedure expr_builder)
{
    ...
    for (u32 i = 0; i < ln_conds; i++) {
        ...
        enode cond_node = (enode)sp_invoke((subprocedure)cond_builder, conds->origin[i]);

        // coerce to bool and grab value
        enode cond_bool = e_create(a, etypeid(bool), (Au)cond_node, false);
        LLVMValueRef condition = _llvalue((enode)cond_bool);

        LLVMBuildCondBr(B, condition, then_block, else_block);

        // then
        LLVMPositionBuilderAtEnd(B, then_block);
        sp_invoke((subprocedure)expr_builder, exprs->origin[i]);
    ...
    if (has_final_else) {
        sp_invoke((subprocedure)expr_builder, exprs->origin[ln_conds]);
    ...
    subprocedure expr_builder)
{
    ...
    for (int i = 0; i < ln_conds; i++) {
        ...
        enode cond_node = (enode)sp_invoke((subprocedure)cond_builder, (Au)tokens_cond);
        LLVMValueRef condition = e_create(a, etypeid(bool), (Au)cond_node, false)->value;
...
enode aether_e_addr_of(aether a, enode expr, etype mdl) {
    fault("verify e_create usage e_create(a, (etype)expr, (Au)mdl, false);");
    return e_create(a, (etype)expr, (Au)mdl, false);
}

AU_EXPORT enode enode_load(enode e) {
    emit_guard;
    aether a = au_active(e->mod);
    LLVMTypeRef i8_ty = LLVMInt8TypeInContext(a->module_ctx);
    LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
    return with_value(LLVMBuildLoad2(B, ptr_ty, _llvalue((enode)e), "instance_ptr"), enode(mod, a, autype, e->autype, loaded, true));
}

AU_EXPORT enode enode_shape(enode instance) {
    emit_guard;
    aether a = au_active(instance->mod);
    LLVMTypeRef i8_ty = LLVMInt8TypeInContext(a->module_ctx);
    LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
    ...
    LLVMValueRef base = _llvalue((enode)instance);
    if (!instance->loaded) {
    ...
    return with_value(shape, enode(mod, a, autype, typeid(shape), loaded, true));
}

enode aether_e_offset(aether a, enode n, Au offset, bool in_ref) { sequencer
    emit_guard;
    Au_t au = n->autype;
    // a vector indexes its packed elements through origin, typed by meta
    Au_t vdeep9 = au;
    while (vdeep9 && vdeep9->member_type == AU_MEMBER_VAR) vdeep9 = vdeep9->src;
    bool is_vec9 = vdeep9 && au_is_vector(vdeep9);
    Au_t velem9  = null;
    if (is_vec9) {
        velem9 = au->meta.a ? au->meta.a : (n->meta_a ? (Au_t)n->meta_a :
            (au->member_type == AU_MEMBER_VAR && au->src ? au->src->meta.a : null));
        if (!velem9) velem9 = typeid(Au);
    }
    ...
    LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
    LLVMValueRef base = _llvalue((enode)n);
    enode vobj9 = null;
    if (is_vec9) {
    ...
    if (!i->loaded && _llvalue((enode)i) &&
        LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)i))) == LLVMPointerTypeKind &&
    ...
    LLVMValueRef idx    = _llvalue((enode)i);
    ...
    // debug builds check a vector index against its count
    if (is_vec9 && a->debug) {
        LLVMTypeRef  i64t   = LLVMInt64TypeInContext(a->module_ctx);
        LLVMValueRef idx64  = LLVMTypeOf(idx) == i64t ? idx :
            LLVMBuildSExt(B, idx, i64t, "idx.chk");
        enode        cn9    = etype_access((etype)vobj9, string("count"));
        LLVMValueRef cnt9   = LLVMBuildSExt(B, LLVMBuildLoad2(B,
            LLVMInt32TypeInContext(a->module_ctx), _llvalue((enode)cn9), "count"), i64t, "cnt64");
        LLVMValueRef ok9    = LLVMBuildICmp(B, LLVMIntULT, idx64, cnt9, "idx.ok");
        LLVMValueRef      fn9   = LLVMGetBasicBlockParent(LLVMGetInsertBlock(B));
        LLVMBasicBlockRef bad9  = LLVMAppendBasicBlockInContext(a->module_ctx, fn9, "idx.bad");
        LLVMBasicBlockRef good9 = LLVMAppendBasicBlockInContext(a->module_ctx, fn9, "idx.good");
        LLVMBuildCondBr(B, ok9, good9, bad9);
        LLVMPositionBuilderAtEnd(B, bad9);
        efunc f_if = (efunc)u(efunc, find_member(typeid(vector), "vector_index_fault",
            AU_MEMBER_FUNC, 0, false));
        enode in9 = with_value(idx64, enode(mod, a, autype, typeid(i64), loaded, true));
        e_fn_call(a, f_if, a(vobj9, in9), false, false);
        LLVMBuildBr(B, good9);
        LLVMPositionBuilderAtEnd(B, good9);
    }
    ...
    Au_t result_au = is_vec9 ? velem9 : (n->autype->is_explicit_ref ? elem_au : arg_type);
...
enode aether_e_load(aether a, enode mem, enode target) { sequencer
    ...
    LLVMValueRef ptr = _llvalue((enode)mem);
    ...
    if (mem_au->is_pointer || mem->autype->is_explicit_ref) {
        LLVMTypeRef ptr_ty = LLVMPointerTypeInContext(a->module_ctx, 0);
        LLVMValueRef loaded = LLVMBuildLoad2(B, ptr_ty, _llvalue((enode)mem), id->chars);
        enode r = with_value(loaded, enode(mod, a, loaded, true, autype, mem_au, is_any, mem->is_any,
            meta_a, mem->meta_a ? mem->meta_a : (Au)mem->autype->meta.a,
            meta_b, mem->meta_b ? mem->meta_b : mem->autype->meta.b,
    ...
    LLVMValueRef loaded = LLVMBuildLoad2(
        B, load_ty, _llvalue((enode)mem), id->chars);
    // a loaded value keeps the slot's meta (vec T stays a vector of T)
    enode r = with_value(loaded, enode(mod, a, loaded, true, autype, resolve->autype, is_any, mem->is_any,
        meta_a, mem->meta_a ? mem->meta_a : (Au)mem->autype->meta.a,
        meta_b, mem->meta_b ? mem->meta_b : mem->autype->meta.b));
    return r;
}
...
enode aether_e_primitive_convert(aether a, enode expr, etype rtype) { sequencer
    ...
    a->is_const_op &= expr->autype == rtype->autype; // we may allow from-bit-width <= to-bit-width
    ...
    if (!expr->loaded && _llvalue((enode)expr) &&
        LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)expr))) == LLVMPointerTypeKind) {
    ...
    if (expr->literal && isa(expr->literal) == typeid(unichar) &&
        ...
        Au s_lit = (Au)const_string(chars, (cstr)s->chars);
        enode s_node = e_operand(a, s_lit, etypeid(string));
        return (enode)e_create(a, rtype, (Au)s_node, false);
    }

    //
    if (is_prim(F->autype) && T->autype == typeid(string)) {
        // isa(ptr) reads ((struct _Au*)ptr - 1)->type; typesize = sizeof(struct _Au)
        u64 au_header_size = typeid(Au)->typesize;
        u64 prim_size      = F->autype->abi_size / 8;
        ...
        LLVMBuildStore(B, _llvalue((enode)type_val), temp_alloca);
        ...
        LLVMBuildStore(B, _llvalue((enode)expr), data_ptr);
        ...
        enode au_arg = with_value(data_ptr, enode(mod, a, loaded, false, autype, typeid(Au)));
        return e_fn_call(a, u(efunc, fn_cast), a(au_arg), false, true);
    }
    
    LLVMValueRef V = _llvalue((enode)expr);
    ...
    if (F_kind == LLVMIntegerTypeKind &&  T_kind == LLVMIntegerTypeKind) {
        ...
        if (F_bits < T_bits) {
            ...
            if (T_bits == 1)
                V = LLVMBuildICmp(B, LLVMIntNE, V,
                    LLVMConstInt(LLVMIntTypeInContext(a->module_ctx, F_bits), 0, false), "to_bool");
            else
            ...
            V = _llvalue((enode)expr);
    }

    // int to real
    else if (F_kind == LLVMIntegerTypeKind && is_fp_type(lltype(T)))
        V = is_sign(F) ? LLVMBuildSIToFP(B, V, lltype(T), "sitofp")
                         : LLVMBuildUIToFP(B, V, lltype(T), "uitofp");

    // real to int
    else if (is_fp_type(lltype(F)) && T_kind == LLVMIntegerTypeKind)
        V = is_sign(T) ? LLVMBuildFPToSI(B, V, lltype(T), "fptosi")
                         : LLVMBuildFPToUI(B, V, lltype(T), "fptoui");

    // real conversion: bf16/half are 16 bits, float 32, double 64
    else if (is_fp_type(lltype(F)) && is_fp_type(lltype(T)))
        V = F->autype->typesize > T->autype->typesize ?
    ...
    // bitcast for same-size types
        ...
        bool make_temp = is_prim(F->autype) && is_ptr(T) && resolve(T) == F;
        if (make_temp) {
            V = entry_alloca(a, lltype(F), "prim_addr");
            LLVMBuildStore(B, _llvalue((enode)expr), V);
        }
...
AU_EXPORT void aether_push_tokens(aether a, tokens t, num cursor) {
...
AU_EXPORT void aether_pop_tokens(aether a, bool transfer) {
...
AU_EXPORT void aether_push_current(aether a) {
...
AU_EXPORT etype get_type_t_ptr(etype t);

AU_EXPORT void aether_create_type_members(aether a, Au_t ctx) {
...
AU_EXPORT etype aether_e_materialize(aether a, Au_t m) {
    if (!m) return null;
    ...
    etype t = etype(mod, a, autype, m);
    return t;
}

AU_EXPORT etype rlookup(aether a, string name) {
    Au_t au = lexical(a->lexical, cstring(name));
    return etype_prep(a, au);
}

AU_EXPORT array etype_class_list(etype t) {
    aether a = au_active(t->mod);
...
static void set_global_construct(aether a, efunc fn) {
    ...
    LLVMValueRef ctor_func = _llvalue((enode)fn);  // LLVM function
...
static void build_entrypoint(aether a, efunc module_init_fn) {
    ...
    if (live_spec && is_class(live_spec)) {
        ...
        if (live_class) {
            ...
            Au_t fn_run     = find_member(live_spec->autype, "run",     AU_MEMBER_FUNC, 0, false);
            Au_t fn_frame   = find_member(live_spec->autype, "frame",   AU_MEMBER_FUNC, 0, false);
            Au_t fn_destroy = find_member(live_spec->autype, "destroy", AU_MEMBER_FUNC, 0, false);
            ...
            Au_t au_ctor = def_member(a->autype, "silver_live_init", typeid(none), AU_MEMBER_FUNC, 0);
            efunc ctor_fn = efunc(mod, a, autype, au_ctor,
            ...
            LLVMSetValueName2(_llvalue((enode)ctor_fn), "silver_live_init", strlen("silver_live_init"));
            LLVMSetLinkage  (_llvalue((enode)ctor_fn), LLVMExternalLinkage);
            LLVMSetVisibility(_llvalue((enode)ctor_fn), LLVMDefaultVisibility);
            ...
            if (app_element) {
                ...
                root->is_any = true;
                {
                    ...
                    LLVMBuildCall2(B, aa_ty, aa_fn, (LLVMValueRef[]){ _llvalue((enode)root) }, 1, "");
                }
                ...
                inst = e_create(a, live_class, (Au)props, false);
            } else
                inst = e_create(a, live_class, null, false);
            a->skip_context_resolve = saved;
            inst->is_any = true;
            LLVMBuildStore(B, _llvalue((enode)inst), inst_g);
            ...
            if (!app_element) {
                ...
                // instance, so the app receives its own command-line flags via its schema.
                {
                    ...
                    LLVMValueRef aa_fn = LLVMGetNamedFunction(a->module_ref, "au_apply_args");
                    if (!aa_fn) aa_fn = LLVMAddFunction(a->module_ref, "au_apply_args", aa_ty);
                    LLVMBuildCall2(B, aa_ty, aa_fn, (LLVMValueRef[]){ _llvalue((enode)inst) }, 1, "");
                }
...
#ifdef _WIN32
            // the host reaches this by dlsym, so it must be exported
            LLVMSetDLLStorageClass(frame_fn, LLVMDLLExportStorageClass);
#endif
            ...
            if (fn_frame) {
                ...
                LLVMValueRef _hdr  = LLVMBuildGEP2(B, LLVMInt8TypeInContext(a->module_ctx), _inst,
                    (LLVMValueRef[]){LLVMConstInt(i64_ty, -(i64)sizeof(struct _Au), true)}, 1, "hdr");
...
#ifdef _WIN32
            // the host reaches this by dlsym, so it must be exported
            LLVMSetDLLStorageClass(dtor_fn, LLVMDLLExportStorageClass);
#endif
            ...
            if (au_drop) {
                ...
                if (fn_drop && _llvalue((enode)fn_drop)) {
                    ...
                    LLVMBuildCall2(B, drop_ty, _llvalue((enode)fn_drop),
            ...
            // BEFORE any module erase frees the Au_t idents the report reads
            {
                ...
                if (au_lr) {
                    ...
                    if (fn_lr && _llvalue((enode)fn_lr)) {
                        LLVMTypeRef lr_ty = LLVMFunctionType(void_ty, null, 0, false);
                        LLVMBuildCall2(B, lr_ty, _llvalue((enode)fn_lr), null, 0, "");
                    }
                }
            }
...
#ifdef _WIN32
            // the host reaches this by dlsym, so it must be exported
            LLVMSetDLLStorageClass(setwin_fn, LLVMDLLExportStorageClass);
#endif
            ...
            LLVMPositionBuilderAtEnd(B, setwin_bb);
            {
                ...
                if (au_live_window_set) {
                    ...
                    if (fn_lws && _llvalue((enode)fn_lws))
                        LLVMBuildCall2(B, fn1_ty, _llvalue((enode)fn_lws), (LLVMValueRef[]){win_arg}, 1, "");
                }
                LLVMBuildRetVoid(B);
            }
            ...
            LLVMPositionBuilderAtEnd(B, _llentry((enode)module_init_fn));
            return;
        }
    }
    ...
    members(module_base, mem) {
        if (is_class(mem)) {
            etype cls = u(etype, mem);
            if (inherits(cls->autype, main_spec->autype) && !mem->is_abstract) {
    ...
    if (!main_class) {
        etype cov_spec = etypeid(coverage);
        array cov_classes = null;
        if (cov_spec) {
            members(module_base, mem) {
                if (is_class(mem)) {
                    etype cls = u(etype, mem);
                    if (cls->autype->context == cov_spec->autype) {
                        if (!cov_classes) cov_classes = array(16);
                        push(cov_classes, (Au)cls);
                    }
                }
            }
        }
        ...
        each(cov_classes, etype, cls) {
            enode inst = e_create(a, cls, null, false);
            e_fn_call(a, (efunc)u(efunc, fn_run), a(inst), false, true);
        }
    ...
    a->skip_context_resolve = true;
    enode m = e_create(a, main_class, (Au)u(evar, argv), false); // a loop would be nice, since none of our members will be ref+1'd yet
    m->is_any = true;
...
AU_EXPORT Au_t arg_type(Au_t au) {
    Au_t res = au;
    while (res) {
...
AU_EXPORT etype etype_canonical(etype t) {
    aether a = au_active(t->mod);
    Au_t au = t->autype;
    if (au->member_type == AU_MEMBER_MODULE)
        return t;
...
AU_EXPORT etype etype_resolve(etype t) {
    aether a = au_active(t->mod);
    Au_t au = t->autype;
    ...
    while (au && !au->is_funcptr && au->src) {
        ...
        if (au_user && _lltype_slot(au_user))
            break;
    }
...
etype struct_from_au(aether a, string name, Au_t au, bool is_system) {
    // a second implement of the same lambda reuses its struct
    Au_t prior = find_member(a->autype, name->chars, AU_MEMBER_TYPE, 0, false);
    etype prior_t = prior ? u(etype, prior) : null;
    if (prior_t) return prior_t;
    Au_t struct_au = def(a->autype, name->chars, AU_MEMBER_TYPE, AU_TRAIT_STRUCT | (is_system ? AU_TRAIT_SYSTEM : 0));
    int  index = 0;

    for (int i = 0; i < au->members.count; i++) {
        Au_t arg = (Au_t)au->members.origin[i];
        // a capture's src is the outer variable: the field takes its type
        Au_t ft = arg->src;
        while (ft && ft->member_type == AU_MEMBER_VAR && ft->src) ft = ft->src;
        Au_t mem = def_member(struct_au, arg->ident, ft, AU_MEMBER_VAR, 0);
...
// 
AU_EXPORT none push_lambda_members(aether a, efunc f) {
    emit_guard;
    if (a->no_build) return;
    ...
    members(fn, mem) {
        ...
        _llvalue_set((enode)ctx_evar, loaded);
...
AU_EXPORT none etype_init(etype t) {
    ...
    if (_lltype_slot(t) && !t->is_schema || (n && _llvalue(n) && n->symbol_name))
        return;
    ...
    if (etype_is_module) {
        ...
        a->name = hold(n);
        //au = t->autype = def_module(a->name->chars);
        //if (!u(etype, au)) {
        ...
        if (source_au->is_c) {
            t->autype->is_c = true;
            t->autype->typesize = source_au->typesize;
        }
        ...
        for (int i = 0; i < ref->members.count; i++) {
            ...
            if (au_mem->ident && strcmp(au_mem->ident, "ft") == 0) {
                ...
                if (!au->is_c) {
                    ...
                    each (cl,  etype, tt) {
                        for (int ai = 0; ai < tt->autype->members.count; ai++) {
                            Au_t ai_mem = (Au_t)tt->autype->members.origin[ai];
                            if (!is_func(ai_mem))
                                continue;
                            ...
                            //set(a->registry, (Au)fn, enode(mod, a, autype, fn));
                ...
                etype ft = etype(mod, a, autype, ft_type);
                break;
            }
            ...
            new_mem->context = t->autype; // copy entire member and reset for our for context
        }

        // schema struct type is a named LLVM struct; body set at implement
        if (!_lltype_slot(t))
    ...
    if (is_func((Au)au)) {
        ...
        // one-time (core 0): renames au->alt and builds the remote proxy func.
        if (t->remote_code && a->core == 0 && au->ident && (strcmp(au->ident, "init") == 0 || strcmp(au->ident, "dealloc") == 0)) {
            ...
            remote_au->rtype = au->rtype ? au->rtype : etypeid(none)->autype;
            remote_au->alt   = cstr_copy(orig_name);
            def_arg(remote_au, "a", au->context, 0);
            fn->remote_func = efunc(mod, a, autype, remote_au);
        ...
        arg_list(au, arg) {
            ...
            if (!u(etype, t) && !(arg->is_explicit_ref && !t->typesize))
                etype(mod, a, autype, t);

            bool convert_prim_target = is_prim(t) && arg->is_target;
            bool struct_by_ref = is_struct(u(etype, t)) && !arg->is_inlay;
            etype mdl = null;
        ...
        arg_types(au, arg)
            etype_create(a, arg);
        if (au->rtype) etype_create(a, au->rtype);
        ll_ptr(a, t);
    } else if (au && au->is_pointer && au->src && !au->src->is_primitive) {
        ll_ptr(a, t);
    } else if (au->elements > 0 && au->src) {
        ...
        if (au->is_alias) {
            ...
            if (au != typeid(Au_t) && !au->is_system)
                etype_ptr(a, t->autype, null);
        }
    } else if (is_enum(t)) {
        etype_prep(a, au->src);
        ll_same(a, t, au->src ? u(etype, au->src) : etypeid(i32));
    }
    else if (au == typeid(f32))  ll_f32 (a, t);
    else if (au == typeid(f64))  ll_f64 (a, t);
    else if (au == typeid(none)) ll_void(a, t);
    else if (au == typeid(bool)) ll_int (a, t, 1);
        ...
        ll_field members[] = { { .e = etype_prep(a, typeid(handle)) } }; // char* → i8*
        ll_named(a, t, "cereal");
        ll_body (a, t, members, 1, true);
    } else if (au == typeid(micro)) {
        ll_field members[] = {
            { .e = etype_prep(a, typeid(handle)) },
            { .e = etype_prep(a, typeid(i32))    },
            { .e = etype_prep(a, typeid(i32))    }
        };
        ll_named(a, t, "micro");
        ll_body (a, t, members, 3, true);
    ...
    else if (au == typeid(handle))   ll_ptr(a, t);
    ...
    else if (au == typeid(bf16))     ll_bf16(a, t);
    else if (au == typeid(fp16))     ll_fp16(a, t);
...
AU_EXPORT bool is_accessible(aether a, Au_t m) {
    if (m->access_type == interface_undefined || 
        m->access_type == interface_public    || a->autype->module == a->au_module)
        return true;
    return false;
}

AU_EXPORT Au_t read_arg_type(Au_t stored_arg) {
...
AU_EXPORT etype get_type_t_ptr(etype t) {
    ...
    if (t->schema)
        return u(etype, t->schema->autype->ptr);

    // schema must have it's static_value set to the instance
    etype schema = etype(mod, a, autype, t->autype, is_schema, true);
    t->schema = hold(schema);
    
    etype mt = etype_ptr(a, schema->autype, null);
    return mt;
}

// type registration is global: always run it as the root, so the result
// does not depend on which worker first asked for it
AU_EXPORT etype implement_type_id(etype t) {
    type_guard;
    emit_guard;
    aether a  = au_active(t->mod);
    aether rt = a->root ? a->root : a;
    __attribute__((cleanup(restore_codegen_active))) aether _prev_ = au_codegen_active;
    au_codegen_active = rt;
    a = rt;
    Au_t   au = t->autype;

    if (t->schema) {
        etype tt = u(etype, t->autype);
        if (t && tt->type_id)
            return u(etype, t->schema->autype->ptr);
    }
    ...
    // schema must have it's static_value set to the instance
    if (!t->schema)
        t->schema = hold(etype(mod, a, autype, t->autype, is_schema, true));
    ...
    if (!au->typesize && !au->is_class) {
        ...
        if (walk && walk->typesize)
            ...
            au->typesize = LLVMPointerSize(ll_td(a));
    }
    ...
    bool au_is_module = au->member_type == AU_MEMBER_MODULE;
    string n = f(string, "%s_%s", au->ident, au_is_module ? "module" : "info");
    ...
    // type_id is a constant address, not a GEP instruction — it is read
    // from every core, so it can carry no builder insert point
    etype tt  = u(etype, t->autype);
    enode tid = enode(mod, a, autype, type_f, loaded, false);
    ll_const_gep(a, tid, au_t, (enode)schema_i, type_f->member_index);
    tt->type_id = hold(tid);
...
none etype_implement(etype t, bool w) { sequencer
    // fast path: already built and not an enum re-entry — no lock
    if (t->is_implemented && !is_enum(t)) return;
    type_guard;
    Au_t    au = t->autype;
    aether a  = au_active(t->mod);
    ...
    if (au->member_type == AU_MEMBER_VAR) {
        ...
        if (!au->context || (!au->context->context && !parent_is_record) || (au->is_c && au->is_static)) {
            ...
            bool set_init = !a->is_Au_import && !(au->is_c && au->is_static);
            ll_global(a, n, mdl, llvm_id(a, (symbol)au->ident),
                is_external, false, set_init);
            n->loaded = false;
            if (!is_external)
                emit_debug_global(a, au, _llvalue(n));
        } else if (!au->is_static && (au->context->is_lambda ||
                    is_struct(au->context) || is_class(au->context))) {
            // no value on these (slightly lazy to call them evar -- they should be called eprop)
            ...
            verify(!_llvalue((enode)n), "unexpected value set in enode");
            ...
            if (au->is_static && au->member_type == AU_MEMBER_VAR) {
                ...
                cstr  c_id    = (cstr)llvm_id(a, c_name->chars);
                ll_global(a, (enode)static_node, mdl, c_id,
                    a->is_Au_import, true, !a->is_Au_import);
                ...
                _llvalue_set((enode)n, entry_alloca(a, alloca_type, id));
                LLVMPositionBuilderAtEnd(B, current_block);

                if (n->autype->src && n->autype->src->src) {
                    etype t1 = etype_prep(a, n->autype->src->src);
                }
                etype src = etype_prep(a, n->autype->src);
                // declare local variable for debugger
                emit_debug_variable(a, n, 0, 0);
            }
        } else if (is_func(au->context)) {
            verify(_llvalue((enode)n), "expected evar to be set for arg");
            verify(isa(t) == typeid(evar) || isa(t) == typeid(enode), "expected evar/enode instance for arg");
            // an inlay struct arrives by value: it is already loaded
            n->loaded = au->is_explicit_ref ? au->src->is_c :
                (!is_struct(au_arg_type((Au)au)) || au->is_inlay);
            /*
    ...
    if (!au->is_alias && !au->is_pointer && !au->is_funcptr && (is_rec(t) || au->is_union)) {
        array cl = (au->is_union || au->is_struct) ? a(t) : etype_class_list(t);
        ...
        each(cl, etype, tt) {
            if (!multi_Au || tt->autype != typeid(Au))
                for (int i = 0; i < tt->autype->members.count; i++) {
                    Au_t m = (Au_t)tt->autype->members.origin[i];
                    if (m->member_type == AU_MEMBER_VAR && !m->is_static && !m->is_elaborate && !m->is_override)
                        count++;
                }

            if (is_class(t) && (!multi_Au || tt->autype != typeid(Au)))
                count++; // u8 Type_interns[isize]
        }

        // Au uses declare_base: has fn but no af_bits.
        // Au-native (define_class) + Silver-defined classes use declare_class*: fn + af_bits.
        // Manually-registered types (Au_t, collective_abi): no prefix.
        bool is_class_type = is_class(t) && !au->is_system && !au->is_schema;
        bool has_fn_only   = is_class_type && (au == typeid(Au));
        bool has_fbits     = is_class_type && !has_fn_only &&
                             (au->is_au_native || au->module != typeid(Au)->module);

        LLVMTypeRef* fbits_type = null;
        if (has_fbits) {
            count += 2; // fn ptr + fbits struct
            char fbits_name[256];
            snprintf(fbits_name, sizeof(fbits_name), "%s_fbits", au->ident);
            ll_field fbits_fields[] = {
                { .bits = 64 }, { .bits = 64 }, { .bits = 64 }, { .bits = 64 } };
            fbits_type = ll_named_raw(a, fbits_name, fbits_fields, 4, false);
        } else if (has_fn_only) {
            count += 1; // fn ptr only
        }

        ll_field* struct_members = calloc(count + 2, sizeof(ll_field));
        etype    e_handle = etype_prep(a, typeid(handle));
        int      ilargest = 0;
        unsigned union_align = 1; // track max alignment of union members

        if (has_fbits) {
            struct_members[0].e       = e_handle; // fn
            struct_members[1].percore = fbits_type;
            index = 2;
        } else if (has_fn_only) {
            struct_members[0].e = e_handle; // fn
            index = 1;
        ...
        each(cl, etype, tt) {
            ...
            //if (len(cl) > 1 && tt->autype == typeid(Au)) break;
            int prop_index = 0;
            ...
            for (int i = 0; i < tt->autype->members.count; i++) {
                ...
                if (is_func((Au)m)) {
                    ...
                    if (pass == 1) continue;
                    etype_create(a, m->rtype);
                    etype_implement(u(etype, m->rtype), false);

                    arg_types (m, t) {
                        if (t->is_pointer && t->src && !u(etype, t)) {
                            etype_create(a, t->src);
                            etype_implement(u(etype, t->src), false);
                            etype_ptr(a, t->src, null);
                        }
                        etype_create(a, t);
                    ...
                    if (m->is_static ) {
                        ...
                        ll_global(a, (enode)static_node, u(etype, m->src), c_id,
                            a->is_Au_import, true, !a->is_Au_import);
                        ...
                        etype s = u(etype, src);
                        ll_field* f = &struct_members[index];
                        ...
                        LLVMTypeRef cur = ll_field_at(a, f, 0);
                        if (!src->abi_size && cur) {
                            int base_size = LLVMABISizeOfType(ll_td(a), cur);
                            if (base_size) src->abi_size = base_size * 8;
                        }

                        f->elements = m->elements;
                        f->arr      = m->elements > 0;
                        cur = ll_field_at(a, f, 0);

                        verify(cur, "no lltype found for member %s.%s", au->ident, m->ident);
                        int abi_member  = LLVMABISizeOfType(ll_td(a), cur);
                        ...
                        if (au->is_union) {
                            unsigned mem_align = LLVMABIAlignmentOfType(ll_td(a), cur);
            ...
            // set icount for this level (needed for fbits_index across modules)
            {
                ...
                if (!tt->autype->icount) tt->autype->icount = ic;
            }

            // lets define this as a form of byte accessible opaque.
            if (is_class(t) && (!multi_Au || tt->autype != typeid(Au))) {
                struct_members[index].e        = etypeid(u8);
                struct_members[index].arr      = true;
                struct_members[index].elements = tt->autype->isize;
                index++;
            }
        }
        ...
        if (au->is_union) {
            ...
            struct_members[0] = (ll_field) {
                .bits = elem_bits, .arr = true,
                .elements = padded / (int)union_align };
            if (au->ident && strcmp(au->ident, "v4l2_format") == 0)
        ...
        for (int ci = 0; ci < count; ci++)
            verify(ll_field_at(a, &struct_members[ci], 0),
                "etype_implement %s: member slot %d has null lltype",
                au->ident ? au->ident : "?", ci);
        ll_body(a, t, struct_members, count, au->is_c ? 0 : 1);
        ...
        if (!au->is_union && !has_scalable_vector(_lltype_slot(t))) {
            ...
            for (unsigned i = 0; i < ecount; i++) {
                ...
                if (!packed) {
                    unsigned al = LLVMABIAlignmentOfType(ll_td(a), et);
                    if (al) eoff = (eoff + al - 1) / al * al;
                }
                eoffs[i] = eoff;
                eoff += LLVMABISizeOfType(ll_td(a), et);
            }
            each(cl, etype, tt) {
        ...
        if (au->traits & AU_TRAIT_ENUM) {
            if (!lltype(t) && au->src)
                _lltype_set(t, lltype(u(etype, au->src)));
            ...
            for (int i = 0; i < au->members.count; i++) {
                ...
                if (!n) {
                    ...
                    int *lit_i = (int*)lit;
                    n = enode(mod, a, autype, mem, literal, lit);
                ...
                if (n && !n->value) {
                    ...
                    if (lit) {
                        ...
                        n->value   = e_create(a, u(etype, au->src), (Au)prim, false)->value;
                        n->loaded  = true;
                    }
                }
            }
        ...
        for (int i = 0; i < au->members.count; i++) {
            ...
            if (!u(etype, m)) {
                etype_register(a, (Au)m, (Au)hold(with_value(null, enode(mod, a, loaded, false, autype, m))), false);
            }
            ...
            if (!a->is_Au_import) {
                if (et == typeid(i32))
                    ll_const_int(a, nn, t, *((i32*)m->value), 1);
                else if (et == typeid(u32))
                    ll_const_int(a, nn, t, *((u32*)m->value), 0);
                else if (et == typeid(i16))
                    ll_const_int(a, nn, t, *((i16*)m->value), 1);
                else if (et == typeid(u16))
                    ll_const_int(a, nn, t, *((u16*)m->value), 0);
                else if (et == typeid(i8))
                    ll_const_int(a, nn, t, *((i8*)m->value), 1);
                else if (et == typeid(u8))
                    ll_const_int(a, nn, t, *((u8*)m->value), 0);
                else if (et == typeid(i64))
                    ll_const_int(a, nn, t, *((i64*)m->value), 1);
                else if (et == typeid(u64))
                    ll_const_int(a, nn, t, *((u64*)m->value), 0);
                else if (et == typeid(f32))
                    ll_const_real(a, nn, t, (double)*((f32*)m->value));
                else if (et == typeid(f64))
                    ll_const_real(a, nn, t, *((f64*)m->value));
                else
        ...
        if (is_lambda(au))
            ll_param(a, fn->context_node, (enode)fn, 0); // context sits before the args
        ...
        fn->target = au_target ?
            hold(enode(mod, a, autype, au_target, arg_index, 0, loaded, true)) : null;

        // structs are a loaded:false target, with a struct interface (non-ptr)
        if (fn->target && is_struct((Au)au_target)) {
            fn->target->loaded = fn->target->autype->is_pointer;
        }

        if (fn->target) {
            Au_t target_type = au_arg_type((Au)fn->target->autype);
            if (target_type)
                etype_prep(a, target_type);
        }

        string label = f(string, "%s_entry", au->ident);
        bool is_user_implement = fn->autype->module == a->autype && fn->has_code;

        if (is_user_implement)
            ll_entry(a, (enode)fn, label->chars);

        bool global_public_fn = au->access_type != interface_intern;
        ll_linkage(a, (enode)fn,
        ...
        int index = 0;
        arg_list(fn->autype, arg) {
            verify(arg != typeid(Au), "unexpected Au");
            evar t = u(evar, arg);
            if (!t) {
                bool is_explicit_ref = arg->is_explicit_ref;
                t = evar(mod, a, autype, arg, is_explicit_ref, is_explicit_ref, arg_index, index,
                    loaded, (!is_struct(arg->src) || arg->is_inlay) && !is_explicit_ref);
            }
            index++;
        }
    } else if (au->is_alias && is_ptr(au->src)) {
        etype src = etype_prep(a, au->src);
        if (!_lltype_slot(src))
            ll_ptr(a, src);
    }
    ...
    if (!au->is_void && !is_opaque(au) && !is_func((Au)au) && _lltype_slot(t)) {
        ...
        /// /// /// /// /// /// /// /// /// /// /// /// /// /// /// /// ///
            ...
            if (!ptr_view_of_class) {
                if (!au->abi_size)
                    au->abi_size   = LLVMABISizeOfType(ll_td(a), _lltype_slot(t)) * 8;
                if (!au->typesize || !au->is_au || au->is_c)
                    au->typesize = LLVMABISizeOfType(ll_td(a), _lltype_slot(t));
                if (!au->align_bits)
                    au->align_bits = LLVMABIAlignmentOfType(ll_td(a), _lltype_slot(t)) * 8;
            }
        }
}
...
enode aether_e_asm(aether a, array body, array input_nodes, etype out_type, string return_name)
{
    ...
    if (return_name)
        for (int i = 0; i < n_in; i++) {
            ...
            if (strcmp(n->autype->ident, return_name->chars) == 0) {
    ...
    for (int i = 0; i < n_in; i++) {
        ...
        params[i]    = LLVMTypeOf(_llvalue((enode)loaded));
        args[i]      = _llvalue((enode)loaded);
    }
    ...
    if (has_out)
        return with_value(result, enode(mod, a, autype, out_type->autype, loaded, true));
...
enode enode_offset(enode ptr, etype offset_type, i64 index) { sequencer
    emit_guard;
    aether a = au_active(ptr->mod);
    if (a->no_build) return e_noop(a, (etype)ptr);
    
    LLVMValueRef base = _llvalue((enode)ptr);
    ...
    return with_value(result, enode(mod, a, autype, au_arg_type((Au)ptr->autype), loaded, false));
}
...
static void emit_expect_tests(aether a, Au_t module_base, efunc f) {
    ...
    // the runtime starts before the tests, as main would start it
    efunc fn_engage = (efunc)u(efunc,
        find_member(etypeid(Au)->autype, "engage", AU_MEMBER_FUNC, 0, false));
    if (fn_engage) {
        LLVMTypeRef  eng_ty = LLVMFunctionType(LLVMVoidTypeInContext(a->module_ctx), &i8p, 1, 0);
        LLVMValueRef nil    = LLVMConstNull(i8p);
        LLVMBuildCall2(B, eng_ty, _llvalue((enode)fn_engage), &nil, 1, "");
    }
    efunc fn_drop = (efunc)u(efunc,
    ...
    members(module_base, mem) {
        ...
        arg_list(mem, arg) {
            etype at = etype_prep(a, au_arg_type((Au)arg));
            enode inst = e_init(a, e_alloc(a, at, false), null, null, null);
            push(vals, (Au)inst);
        }
...
AU_EXPORT none aether_build_module_initializer(aether a, enode init) {
    emit_guard;
    Au_t au = init->autype;
    if (!au->is_mod_init)
        return;
    ...
    members(module_base, mem) {
        ...
        if (is_func(mem) && mem->access_type != interface_intern) {
            ...
            enode fn = e_fn_call(a, fn_def_func, a(
                ...
                _u64(mem->traits),
                value(etypeid(ARef), _llvalue((enode)mf)),
    ...
    pairs(a->user_type_ids, i) {
        ...
        bool is_alias_t  = mdl->autype->is_alias || (mdl->autype->is_pointer && !is_class_t);
        ...
        if (!tau->typesize && tau->is_class) {
            ...
            if (ce && _lltype_slot(ce) &&
                ...
                char* dl_rep = LLVMCopyStringRepOfTargetData(ll_td(a));
        ...
        //aether_eputs(a, f(string, "%o: var", type_id->autype));
        if (!mdl->autype->typesize && !is_class_t) continue; // skip opaque types (e.g. GLFWwindow); classes still need ft copy
        ...
        members(tau, mem) {
            ...
            if (is_func(mem)) {
                ...
                LLVMValueRef slot = LLVMBuildGEP2(B, LLVMInt8TypeInContext(a->module_ctx), _llvalue((enode)type_id), &offset, 1, "ft_slot");
                enode fn_slot = with_value(slot, enode(mod, a, autype, typeid(ARef), loaded, false));
                ...
                // for attrib members, build the initializer and pass as value
                enode e_attrib_val = e_null(a, etypeid(ARef));
                if (mem->is_attrib) {
                    evar var = u(evar, mem);
                    enode constructed = build_attrib_value(a, var);
                    if (constructed && _llvalue((enode)constructed)) {
                        efunc fn_hold = efind(efunc, etypeid(Au), hold);
                        if (fn_hold)
                            e_attrib_val = e_fn_call(a, fn_hold, a(constructed), false, false);
                    }
                }
        ...
        if (!tau->is_c && max_ft_end > 0) {
            ...
            LLVMValueRef ts_ptr = LLVMBuildGEP2(B, LLVMInt8TypeInContext(a->module_ctx),
                _llvalue((enode)type_id), &ts_offset, 1, "table_size_ptr");
    ...
    pairs(a->user_type_ids, ti) {
        ...
        if (!mdl || !mdl->autype) continue;
        if (!is_class(mdl) && !is_struct(mdl)) continue;
        members(mdl->autype, smem) {
...
AU_EXPORT statements aether_context_code(aether a) {
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
        if (u(statements, ctx))
...
AU_EXPORT efunc aether_context_func(aether a) {
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
...
AU_EXPORT etype aether_context_class(aether a) {
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
        etype ctx_u = u(etype, ctx);
...
AU_EXPORT etype aether_context_struct(aether a) {
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
        etype ctx_u = u(etype, ctx);
...
AU_EXPORT etype aether_context_model(aether a, Au_t type) {
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
        etype ctx_u = u(etype, ctx);
...
AU_EXPORT catcher context_catcher_depth(aether a, int depth) {
    int found = 0;
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
...
AU_EXPORT etype aether_context_record(aether a) {
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
        etype ctx_u = u(etype, ctx);
...
AU_EXPORT Au_t aether_top_scope(aether a) {
    Au_t open = null;
...
AU_EXPORT none aether_push_scope(aether a, Au arg, int label) {
    emit_guard;
    Au_t au = au_arg(arg);
    Au_t auu = au;
    ...
    // create DWARF lexical block for namespace scopes (loops, if/else blocks)
    // so that same-named variables in different scopes get distinct DILocalVariable entries
    if (st && a->debug && !a->no_build && mtype == AU_MEMBER_NAMESPACE && peek) {
        etype st_etype = u(etype, auu);
        if (st_etype && !_llscope(st_etype)) {
            llscope_set(st_etype, debug_create_lexical_block(a, peek->line, peek->column));
        }
    }

    efunc fn = context_func(a);
    if (fn && isa(fn) == typeid(efunc) && (fn != prev_fn) && !a->no_build) {
        // building core appends the entry block on first scope entry
        if (!_llentry((enode)fn) && fn->has_code && fn->autype->module == a->autype && _llvalue((enode)fn)) {
            string elabel = f(string, "%s_entry", fn->autype->ident);
            ll_entry(a, (enode)fn, elabel->chars);
        }
        // worker cores derive their own params for shared arg nodes
        if (a->core > 0 && _llvalue((enode)fn)) {
            LLVMValueRef fv     = _llvalue((enode)fn);
            int          offset = is_lambda((Au)fn->autype) ? 1 : 0;
            arg_list(fn->autype, arg) {
                enode av = (enode)u(etype, arg);
                if (av && instanceof(av, enode) && av->autype->member_type == AU_MEMBER_VAR)
                    _llvalue_set(av, LLVMGetParam(fv, av->arg_index + offset));
            }
            if (fn->target)
                _llvalue_set(fn->target, LLVMGetParam(fv, 0));
            if (fn->context_node)
                _llvalue_set(fn->context_node, LLVMGetParam(fv, 0));
        }
        // attach DWARF subprogram for debugger (breakpoints, stack frames)
        if (fn->has_code && fn->autype->module == a->autype)
            emit_debug_function(a, fn, false);
#ifdef _WIN32
        // pe: an importing module binds this through our export table, so a
        // method we define has to be named in it
        // hidden means the split build had to promote an internal function to
        // external linkage -- still module-private, so it stays unexported
        if (fn->has_code && fn->autype->module == a->autype && _llvalue((enode)fn) &&
            LLVMGetVisibility(_llvalue((enode)fn)) == LLVMDefaultVisibility)
            LLVMSetDLLStorageClass(_llvalue((enode)fn), LLVMDLLExportStorageClass);
#endif
        LLVMPositionBuilderAtEnd(B, _llentry((enode)fn));
        LLVMSetCurrentDebugLocation2(B, _lldbg((enode)fn));
...
static Au_t map_etype(aether a, symbol name, int *out_depth) {
    // a C keyword type names a silver primitive
    if (strcmp(name, "int")    == 0) return typeid(i32);
    if (strcmp(name, "char")   == 0) return typeid(i8);
    if (strcmp(name, "short")  == 0) return typeid(i16);
    if (strcmp(name, "float")  == 0) return typeid(f32);
    if (strcmp(name, "double") == 0) return typeid(f64);
    if (strcmp(name, "void")   == 0) return typeid(none);
    Au_t t = au_lookup(name);
    if (!t) return null;
    ...
    if (mac->autype->is_functional || len(mac->def) == 0) return null;
...
// an object-like macro naming a type (#define T int) becomes its alias
AU_EXPORT none aether_macro_type(aether a, Au_t m) {
    if (!m || m->member_type != AU_MEMBER_MACRO || m->is_functional) return;
    int  depth = 0;
    Au_t base  = map_etype(a, m->ident, &depth);
    if (!base || base == m) return;
    m->member_type = AU_MEMBER_TYPE;
    m->src         = base;
    m->is_alias    = true;
    for (int i = 0; i < depth; i++)
        m->src = pointer(a, (Au)m->src)->autype;
    // sizeof reads the alias itself: it carries the size it names
    m->typesize = depth ? (u64)sizeof(void*) : m->src->typesize;
    // the type's etype replaces the one it had as a macro
    etype_register(a, (Au)m, (Au)hold(etype(mod, a, autype, m)), true);
}

AU_EXPORT none aether_import_models(aether a, Au_t ctx, bool au_mode) {
...
void aether_import_Au(aether a, string ident, Au lib) {
    ...
    if (!u(etype, typeid(Au))) {
        etype_ptr(a, etype(mod, a, autype, typeid(Au))->autype, null);
        etype_ptr(a, etype(mod, a, autype, typeid(Au_t))->autype, null);
    }
    ...
    if (!u(etype, (Au)au_module)) {
        emodule(mod, a, autype, au_module);
    }
    if (!lib) {
        ...
        Au_t cov = find_member(au_module, "coverage", AU_MEMBER_TYPE, 0, false);
        if (cov) {
            if (!u(etype, cov)) {
                etype t = etype(mod, a, autype, cov);
...
AU_EXPORT void aether_llflag(aether a, symbol flag, i32 ival) {
    emit_guard;
...
AU_EXPORT bool aether_emit(aether a, ARef ref_ll, ARef ref_bc) {
    emit_guard;
    path* ll = (path*)ref_ll;
    path* bc = (path*)ref_bc;
    cstr err = NULL;
    ...
    // every core carries its own stubs: the entry block is declared up front
    // but only the core that builds the body fills it in.
    for (int ci = 0; ci < ll_n(a); ci++)
        strip_unbuilt_functions(ll_mod(a, ci));
    ...
    *bc = form(path, "%o/%o.bc", bdir, a);
    ...
    // a try's setjmp returns twice: locals written before a throw
    // must reach memory, so their loads and stores go volatile
    for (int ci = 0; ci < ll_n(a); ci++) {
        ...
        for (LLVMValueRef f = LLVMGetFirstFunction(m); f; f = LLVMGetNextFunction(f)) {
            ...
            bool twice = false;
            for (LLVMBasicBlockRef bb = LLVMGetFirstBasicBlock(f); bb && !twice; bb = LLVMGetNextBasicBlock(bb))
                for (LLVMValueRef in = LLVMGetFirstInstruction(bb); in && !twice; in = LLVMGetNextInstruction(in)) {
                    if (LLVMGetInstructionOpcode(in) != LLVMCall) continue;
                    LLVMValueRef callee = LLVMGetCalledValue(in);
                    size_t nl = 0;
                    const char* nm = callee ? LLVMGetValueName2(callee, &nl) : null;
                    if (nm && nl == 6 && memcmp(nm, "setjmp", 6) == 0) twice = true;
                }
            if (!twice) continue;
            for (LLVMBasicBlockRef bb = LLVMGetFirstBasicBlock(f); bb; bb = LLVMGetNextBasicBlock(bb))
                for (LLVMValueRef in = LLVMGetFirstInstruction(bb); in; in = LLVMGetNextInstruction(in)) {
                    LLVMOpcode op = LLVMGetInstructionOpcode(in);
                    if (op != LLVMLoad && op != LLVMStore) continue;
                    LLVMValueRef ptr = LLVMGetOperand(in, op == LLVMLoad ? 0 : 1);
                    if (ptr && LLVMIsAAllocaInst(ptr)) LLVMSetVolatile(in, 1);
                }
        }
    }
...
static path core_object(aether a, path obj_path, int core) {
    return f(path, "%o.core%i.o", obj_path, core);
}

// a module is worth emitting only if a body was built into it
static bool module_has_body(LLVMModuleRef m) {
    for (LLVMValueRef fn = LLVMGetFirstFunction(m); fn; fn = LLVMGetNextFunction(fn))
        if (LLVMCountBasicBlocks(fn)) return true;
    return false;
}
...
AU_EXPORT string aether_core_objects(aether a, path obj_path) {
    ...
    for (int i = 1; i < ll_n(a); i++) {
        if (!module_has_body(ll_mod(a, i))) continue;
        append(s, " ");
        append(s, core_object(a, obj_path, i)->chars);
    }
    return s;
}
...
AU_EXPORT bool aether_emit_object(aether a, path obj_path) {
    ...
    for (int i = 0; i < ll_n(a); i++) {
        LLVMModuleRef m = ll_mod(a, i);
        if (i > 0 && !module_has_body(m)) continue;
        path p = i == 0 ? obj_path : core_object(a, obj_path, i);
        jobs[n].tm   = (LLVMTargetMachineRef)a->target_machines[i];
        jobs[n].m    = m;
    ...
    for (int i = 1; i < n; i++)
        pthread_create(&th[i], null, emit_job_run, &jobs[i]);
    if (n) emit_job_run(&jobs[0]);
    bool ok = n ? jobs[0].ok : true;
    for (int i = 1; i < n; i++) {
...
AU_EXPORT void llvm_reinit(aether a) {
    emit_guard;
    ...
    for (int i = 0; i < AU_CORES; i++)
        if (a->target_machines[i]) {
            LLVMDisposeTargetMachine((LLVMTargetMachineRef)a->target_machines[i]);
            a->target_machines[i] = null;
        }
    a->target_machine = null;
    LLVMContextDispose  (a->module_ctx);
    LLVMDisposeMessage  (a->target_triple);
    for (int i = 1; a->ctxs && i < a->ctxs->count; i++) {
        if (a->mods->origin[i]) LLVMDisposeModule((LLVMModuleRef)a->mods->origin[i]);
        if (a->ctxs->origin[i]) LLVMContextDispose((LLVMContextRef)a->ctxs->origin[i]);
    }
    ...
    // one context + module per codegen core; types register into all of them
    a->ncores         = AU_CORES;
    a->ctxs           = hold(array(alloc, AU_CORES, unmanaged, true));
    a->mods           = hold(array(alloc, AU_CORES, unmanaged, true));
    a->ctxs->count    = AU_CORES;
    a->mods->count    = AU_CORES;
    a->dbgs           = hold(array(alloc, AU_CORES, unmanaged, true));
    a->files          = hold(array(alloc, AU_CORES, unmanaged, true));
    a->cus            = hold(array(alloc, AU_CORES, unmanaged, true));
    a->dbgs->count    = AU_CORES;
    a->files->count   = AU_CORES;
    a->cus->count     = AU_CORES;
    a->ctxs->origin[0] = (Au)a->module_ctx;
    a->mods->origin[0] = (Au)a->module_ref;
    a->dbgs->origin[0] = (Au)a->dbg_builder;
    for (int i = 1; i < AU_CORES; i++) {
        LLVMContextRef c = LLVMContextCreate();
        LLVMModuleRef  m = LLVMModuleCreateWithNameInContext(a->name->chars, c);
        a->ctxs->origin[i] = (Au)c;
        a->mods->origin[i] = (Au)m;
        a->dbgs->origin[i] = (Au)LLVMCreateDIBuilder(m);
    }
    ...
    for (int i = 0; i < AU_CORES; i++)
        a->target_datas[i] = (ARef)LLVMCreateTargetDataLayout(
            (LLVMTargetMachineRef)a->target_machines[i]);
    ...
    path rel = path_cwd();
    for (int i = 0; i < AU_CORES; i++)
        a->files->origin[i] = (Au)LLVMDIBuilderCreateFile(
            (LLVMDIBuilderRef)a->dbgs->origin[i],
            a->module_file->chars, len(a->module_file), rel->chars, len(rel));
    a->file = (LLVMMetadataRef)a->files->origin[0];
    a->current_file = a->file;

    a->target_data = (LLVMTargetDataRef)a->target_datas[0];
#ifndef NDEBUG
    aether_init_listen(a);
#endif
    if (a->debug) {
        for (int i = 0; i < AU_CORES; i++) {
            LLVMDIBuilderRef  db = (LLVMDIBuilderRef)a->dbgs->origin[i];
            LLVMModuleRef     m  = (LLVMModuleRef)   a->mods->origin[i];
            LLVMContextRef    c  = (LLVMContextRef)  a->ctxs->origin[i];
            a->cus->origin[i] = (Au)LLVMDIBuilderCreateCompileUnit(
                db, LLVMDWARFSourceLanguageC, (LLVMMetadataRef)a->files->origin[i],
                "silver", 6, 0, "", 0,
...
/// Clone an aether/silver instance for parallel phase-3 codegen. Every
/// core owns a context, module, DIBuilder and compile unit, all built
/// up front; the clone binds to its core's set and emits into its own
/// module, which becomes its own object on the link line.
AU_EXPORT aether aether_clone(aether a, int core) {
    emit_guard;
    aether c = (aether)Au_copy((Au)a);
    c->core    = core < ll_n(a) ? core : 0;
    c->root    = a->root ? a->root : a;
    c->builder = LLVMCreateBuilderInContext(ll_ctx(a, c->core));

    // own lexical scope stack + token save-stack, seeded from parent state
    c->module_ctx   = ll_ctx(a, c->core);
    c->module_ref   = ll_mod(a, c->core);
    c->dbg_builder  = (LLVMDIBuilderRef)a->dbgs->origin[c->core];
    c->file         = (LLVMMetadataRef) a->files->origin[c->core];
    c->current_file = c->file;
    if (a->debug) c->compile_unit = (LLVMMetadataRef)a->cus->origin[c->core];

    c->lexical = array(alloc, 32, unmanaged, true, assorted, true);
    concat(c->lexical, a->lexical);
    c->stack   = array(16);
    concat(c->stack, a->stack);
    c->last_return      = null;
    c->statement_origin = null;
    return c;
}


AU_EXPORT void aether_unload_libs(aether a) {
...
AU_EXPORT void aether_reinit_startup(aether a) {
    ...
    verify(a->autype,       "no module registered for aether");
    verify(g != a->autype,  "aether using global module");
    ...
    a->autype->is_namespace = true; // for the 'module' namespace at [1], i think we dont require the name.. or, we set a trait
    a->autype->is_nameless  = false; // we have no names, man. no names. we are nameless! -cereal
    push_scope(a, (Au)a->autype, 17);
} 

AU_EXPORT none aether_test_write(aether a) {
    emit_guard;
    char* err = 0;
    LLVMPrintModuleToFile(a->module_ref, "crashing.ll", &err);
}

AU_EXPORT enode aether_sequence_enode(aether a) {
    emit_guard;
    LLVMTypeRef  i64t = LLVMInt64TypeInContext(a->module_ctx);
    LLVMValueRef v    = (a->coverage_seq_local && !a->no_build)
        ? LLVMBuildLoad2(B, i64t, a->coverage_seq_local, "sequence")
        : LLVMConstInt(i64t, 0, 0);
    return with_value(v, enode(mod, a, autype, etypeid(i64)->autype, loaded, true));
}

// overlay hand-off: the checkout child ctor is at its prop-pair max, so
// the importer stashes these right before constructing it
static __thread path g_overlay_install;
static __thread path g_overlay_base;
AU_EXPORT void aether_overlay(path install, path base) {
    g_overlay_install = install;
    g_overlay_base    = base;
}
...
AU_EXPORT none aether_init(aether a) {
    ...
    if (!a->install && g_overlay_install) {
        a->install      = hold(g_overlay_install);
        a->base_install = hold(g_overlay_base);
        g_overlay_install = null;
        g_overlay_base    = null;
    }
    if (!a->install) {
    ...
    if (a->base_install)
        push(a->include_paths, (Au)f(path, "%o/include", a->base_install));
...
AU_EXPORT none aether_dealloc(aether a) {
    ...
    for(item i = a->registry->first; i; i = i->next) {
        ...
        if (!e || !e->autype || e->autype == a->autype || e == (etype)a)
            continue;

        if (e->autype->module == a->autype)
            e->autype = null;
...
AU_EXPORT string etype_cast_string(etype t) {
    return t->autype ? f(string, "%o", t->autype) : string("[no-type]");
}

AU_EXPORT Au_t etype_cast_Au_t(etype t) {
    return t->autype;
}


AU_EXPORT none enode_init(enode n) {
    emit_guard;
    Au_t n_isa = isa(n);
    aether a = au_active(n->mod);
    ...
    if (!a->no_build && is_func((Au)n->autype->context) && !n->symbol_name && !_llvalue((enode)n)) {
        ...
        enode fn = (enode)u(enode, (Au)n->autype->context);
        if (!fn) {
        ...
        //n->loaded = n->autype->is_explicit_ref ? n->autype->src->is_c : (!is_struct(n->autype->src) && !n->autype->is_inlay);
        n->loaded = n->autype->is_explicit_ref ? true : (!is_struct(n->autype->src) || n->autype->is_inlay);
...
AU_EXPORT none enode_dealloc(enode a) {
}

AU_EXPORT Au_t enode_cast_Au_t(enode a) {
    return a->autype;
}
...
AU_EXPORT array macro_expand(macro m, array args) {
    aether a = au_active(m->mod);
...
AU_EXPORT array codegen_generate_fn(codegen code, efunc f, array query) {
...
AU_EXPORT void aether_e_print_node(aether a, enode n) {
    ...
    if (can == etypeid(string) || can == etypeid(path)) {
        enode as_cstr = e_create(a, etypeid(cstr), (Au)n, false);
        enode fmt_node = with_value(const_cstr(a, "%s", 2), enode(mod, a, autype, etypeid(cstr)->autype, loaded, true));
        e_fn_call(a, printf_fn, a(fmt_node, as_cstr), false, false);
        return;
    }
    ...
    enode fmt_node = with_value(const_cstr(a, fmt, strlen(fmt)), enode(mod, a, autype, etypeid(cstr)->autype, loaded, true));
    e_fn_call(a, printf_fn, a(fmt_node, n), false, false);
}

// f = format string; this is evaluated from nodes given at runtime
AU_EXPORT void aether_eprint(aether a, symbol f, ...) {
    if (a->no_build) return;
    va_list args;
    va_start(args, f);
...
AU_EXPORT void code_init(code c) {
    emit_guard;
    efunc fn = context_func(c->mod);
    aether a = au_active(fn->mod);
    verify(fn, "no function in context for code block");
    c->block = LLVMAppendBasicBlockInContext(a->module_ctx, _llvalue((enode)fn), c->label);
}

AU_EXPORT void code_seek_end(code c) {
    emit_guard;
    LLVMPositionBuilderAtEnd(c->mod->builder, c->block);
}

AU_EXPORT void aether_e_cmp_code(aether a, enode l, comparison comp, enode r, code lcode, code rcode) {
    emit_guard;
    if (a->no_build) {
        a->is_const_op = false;
        return;
    ...
    LLVMValueRef cond = LLVMBuildICmp(
        B, (LLVMIntPredicate)comp, _llvalue((enode)l), _llvalue((enode)r), "cond");
    LLVMBuildCondBr(B, cond, lcode->block, rcode->block);
}

AU_EXPORT void aether_e_inc(aether a, enode v, num amount) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return;
    enode lv = e_load(a, v, null);
    LLVMValueRef one = LLVMConstInt(LLVMInt64TypeInContext(a->module_ctx), amount, 0);
    LLVMValueRef nextI = LLVMBuildAdd(a->mod->builder, _llvalue((enode)lv), one, "nextI");
    LLVMBuildStore(a->mod->builder, nextI, _llvalue((enode)v));
}

AU_EXPORT void aether_e_branch(aether a, code c) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return;
...
AU_EXPORT enode aether_e_cmp(aether a, enode L, enode R) {
    emit_guard;
    Au_t Lt = isa(L->literal);
    Au_t Rt = isa(R->literal);
    bool Lc = is_class(L);
    ...
    if (Lc || Rc) {
        ...
        Au_t eq = find_member(L->autype, "compare", AU_MEMBER_FUNC, 0, true);
        if (eq) {
            verify(eq->rtype == typeid(i32), "compare function must return i32, found %o", eq->rtype);
            return e_fn_call(a, (efunc)u(efunc, eq), a(L, R), false, false);
        }

        LLVMValueRef lt = LLVMBuildICmp(B, LLVMIntULT, _llvalue((enode)L), _llvalue((enode)R), "cmp_lt");
        LLVMValueRef gt = LLVMBuildICmp(B, LLVMIntUGT, _llvalue((enode)L), _llvalue((enode)R), "cmp_gt");
    ...
    if (Ls || Rs) {
        verify(Ls && Rs && (canonical(L) == canonical(R)),
            "struct type mismatch %o != %o", L->autype->ident, R->autype->ident);
        ...
        members(L->autype, mem) {
            ...
            LLVMValueRef is_nonzero = LLVMBuildICmp(
                B, LLVMIntNE, _llvalue((enode)cmp),
            ...
            push(phi_vals,   (Au)_llvalue((enode)cmp));
    ...
    // Float comparison
    if (Lr || Rr) {
        LLVMValueRef lt = LLVMBuildFCmp(B, LLVMRealOLT, _llvalue((enode)L), _llvalue((enode)R), "cmp_lt");
        LLVMValueRef gt = LLVMBuildFCmp(B, LLVMRealOGT, _llvalue((enode)L), _llvalue((enode)R), "cmp_gt");
        LLVMValueRef neg_one = LLVMConstInt(LLVMInt32TypeInContext(a->module_ctx), -1, true);
        LLVMValueRef pos_one = LLVMConstInt(LLVMInt32TypeInContext(a->module_ctx),  1, false);
        LLVMValueRef zero    = LLVMConstInt(LLVMInt32TypeInContext(a->module_ctx),  0, false);
        LLVMValueRef lt_val  = LLVMBuildSelect(B, lt, neg_one, zero, "lt_val");
...
AU_EXPORT enode aether_e_break(aether a, catcher cat) {
    emit_guard;
    a->is_const_op = false;
    if (!a->no_build) {
        if (cat->rtype) {
            LLVMValueRef def = LLVMConstNull(lltype(cat->rtype));
            LLVMAddIncoming(_llvalue((enode)cat->phi), &def,
                &(LLVMBasicBlockRef){LLVMGetInsertBlock(B)}, 1);
        }
        LLVMBuildBr(B, cat->block);
    }
...
AU_EXPORT enode aether_e_continue(aether a, catcher cat) {
    emit_guard;
    a->is_const_op = false;
    if (!a->no_build) {
...
AU_EXPORT enode aether_e_bitwise_not(aether a, enode L) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, etypeid(bool));
    if (!L->loaded && !is_ptr(L) && !is_class(L) && !is_func_ptr((Au)L))
        L = enode_value(L, true);
    return value(L, LLVMBuildNot(B, _llvalue((enode)L), "bitwise-not"));
}

AU_EXPORT enode aether_e_neg(aether a, enode L) {
    emit_guard;
    a->is_const_op = false;
    ...
    if (Lm->autype->is_realistic)
        return value(L, LLVMBuildFNeg(B, _llvalue((enode)L), "neg"));
    return value(L, LLVMBuildNeg(B, _llvalue((enode)L), "neg"));
}

static LLVMValueRef scalar_max(aether a, LLVMValueRef Lv, LLVMValueRef Rv, etype common) {
    emit_guard;
    LLVMValueRef cmp;
    if (common->autype->is_realistic)
...
static enode vector_binary_op(aether a, enode L, enode R, scalar_op_fn op, cstr name) {
    ...
    a->is_const_op = false;
    Au_t Lve = null, Rve = null;
    bool L_vec = vec_prim_elem(L, &Lve);
    bool R_vec = vec_prim_elem(R, &Rve);
    Au_t L_au = L->autype, R_au = R->autype;
    bool L_arr = L_vec || (L_au->elements > 0 && L_au->src && L_au->src->is_primitive);
    bool R_arr = R_vec || (R_au->elements > 0 && R_au->src && R_au->src->is_primitive);

    Au_t L_elem = L_vec ? Lve : (L_arr ? L_au->src : L_au);
    Au_t R_elem = R_vec ? Rve : (R_arr ? R_au->src : R_au);
    etype L_ety = u(etype, L_elem); if (!L_ety) L_ety = etype_prep(a, L_elem);
    etype R_ety = u(etype, R_elem); if (!R_ety) R_ety = etype_prep(a, R_elem);
    ...
    if (!L_arr && !R_arr) {
        ...
        return value(common, op(a, _llvalue((enode)L), _llvalue((enode)R), common));
    }
    if (a->no_build) return e_noop(a, common);

    LLVMTypeRef  lt     = lltype(common);
    LLVMTypeRef  i32_ty = LLVMInt32TypeInContext(a->module_ctx);
    LLVMValueRef Lbase  = null, Rbase = null, cnt;
    if      (L_vec) Lbase = vec_origin(a, L);
    else if (L_arr) Lbase = _llvalue((enode)L);
    if      (R_vec) Rbase = vec_origin(a, R);
    else if (R_arr) Rbase = _llvalue((enode)R);
    if (L_vec || R_vec)
        cnt = vec_count(a, L_vec ? L : R);
    else
        cnt = LLVMConstInt(i32_ty, L_arr ? L_au->elements : R_au->elements, 0);

    // vec input yields a vec result; C arrays keep the stack array form
    etype        rtype;
    LLVMValueRef out, res_obj = null;
    if (L_vec || R_vec) {
        etype vt  = etype_prep(a, typeid(vector));
        etype vm  = etype(mod, a, autype, vt->autype, meta_a, (Au)common->autype);
        enode vn  = e_create(a, vm, null, false);
        vn->meta_a = (Au)common->autype;
        efunc f_rs = (efunc)u(efunc, find_member(typeid(vector), "resize",
            AU_MEMBER_FUNC, 0, true));
        enode cn  = with_value(LLVMBuildSExt(B, cnt, LLVMInt64TypeInContext(a->module_ctx), "cnt64"),
            enode(mod, a, autype, typeid(i64), loaded, true));
        ...
        out = entry_alloca(a, LLVMArrayType(lt, count), name);
        Au_t res_au = def(a->autype, null, AU_MEMBER_TYPE, 0);
        res_au->src = common->autype;
        res_au->elements = count;
        res_au->is_pointer = true;
        rtype = etype(mod, a, autype, res_au);
    }
    ...
    LLVMValueRef zero = LLVMConstInt(i32_ty, 0, 0);
    LLVMValueRef nz   = LLVMBuildICmp(B, LLVMIntUGT, cnt, zero, "mm_nz");
    LLVMBuildCondBr(B, nz, loop_bb, done_bb);
    ...
    LLVMValueRef Lv, Rv;
    if (L_arr) {
        LLVMValueRef gep = LLVMBuildGEP2(B, L_lt, Lbase, &idx, 1, "l_ptr");
        Lv = LLVMBuildLoad2(B, L_lt, gep, "l_v");
    } else Lv = _llvalue((enode)L);
    if (R_arr) {
        LLVMValueRef gep = LLVMBuildGEP2(B, R_lt, Rbase, &idx, 1, "r_ptr");
        Rv = LLVMBuildLoad2(B, R_lt, gep, "r_v");
    } else Rv = _llvalue((enode)R);

    if (L_elem != common->autype) {
        enode tmp = with_value(Lv, enode(mod, a, autype, L_elem, loaded, true));
        Lv = _llvalue((enode)((enode)e_create(a, common, (Au)tmp, false)));
    }
    if (R_elem != common->autype) {
        enode tmp = with_value(Rv, enode(mod, a, autype, R_elem, loaded, true));
        Rv = _llvalue((enode)((enode)e_create(a, common, (Au)tmp, false)));
    }

    LLVMValueRef rv  = op(a, Lv, Rv, common);
    LLVMValueRef dst = LLVMBuildGEP2(B, lt, out, &idx, 1, "dst_ptr");
    LLVMBuildStore(B, rv, dst);
    ...
    LLVMPositionBuilderAtEnd(B, done_bb);
    return with_value(res_obj ? res_obj : out, enode(mod, a,
        loaded, res_obj ? true : false,
        autype, rtype->autype,
        meta_a, rtype->meta_a));
}

AU_EXPORT enode aether_e_max(aether a, enode L, enode R) {
    return vector_binary_op(a, L, R, scalar_max, "vmax");
}

AU_EXPORT enode aether_e_min(aether a, enode L, enode R) {
    return vector_binary_op(a, L, R, scalar_min, "vmin");
}

AU_EXPORT enode aether_e_abs(aether a, enode L) {
    emit_guard;
    a->is_const_op = false;
    etype Lm = canonical(L);
    if (a->no_build) return e_noop(a, Lm);
    if (Lm->autype->is_realistic) {
        LLVMValueRef neg = LLVMBuildFNeg(B, _llvalue((enode)L), "abs_neg");
        LLVMValueRef cmp = LLVMBuildFCmp(B, LLVMRealOLT, _llvalue((enode)L),
            LLVMConstReal(lltype(Lm), 0.0), "abs_cmp");
        return value(Lm, LLVMBuildSelect(B, cmp, neg, _llvalue((enode)L), "abs"));
    }
    LLVMValueRef neg = LLVMBuildNeg(B, _llvalue((enode)L), "abs_neg");
    LLVMValueRef cmp = LLVMBuildICmp(B, LLVMIntSLT, _llvalue((enode)L),
...
AU_EXPORT enode aether_e_clamp(aether a, enode val, enode lo, enode hi) {
    emit_guard;
    a->is_const_op = false;
    bool v_arr = val->autype->elements > 0 && val->autype->src && val->autype->src->is_primitive;

    Au_t v_elem = v_arr ? val->autype->src : val->autype;
    etype common = determine_rtype(a, OPType__add, u(etype, v_elem), (etype)lo);
    common = determine_rtype(a, OPType__add, common, (etype)hi);

    if (!v_arr) {
        val = e_create(a, common, (Au)val, false);
        lo  = e_create(a, common, (Au)lo, false);
        hi  = e_create(a, common, (Au)hi, false);
        if (a->no_build) return e_noop(a, common);
        return value(common, scalar_clamp(a, _llvalue((enode)val), _llvalue((enode)lo), _llvalue((enode)hi), common));
    }
    if (a->no_build) return e_noop(a, common);
    ...
    LLVMValueRef gep = LLVMBuildGEP2(B, v_lt, _llvalue((enode)val), &idx, 1, "v_ptr");
    LLVMValueRef v = LLVMBuildLoad2(B, v_lt, gep, "v");
    if (v_elem != common->autype) {
        enode tmp = with_value(v, enode(mod, a, autype, v_elem, loaded, true));
        v = _llvalue((enode)((enode)e_create(a, common, (Au)tmp, false)));
    }

    LLVMValueRef res = scalar_clamp(a, v, _llvalue((enode)lo), _llvalue((enode)hi), common);
...
AU_EXPORT enode aether_e_math(aether a, i32 op, enode val) {
    ...
    if (!is_arr) {
        ...
        enode v = e_create(a, elem_ty, (Au)val, false);
...
AU_EXPORT enode aether_e_math2(aether a, i32 op, enode L, enode R) {
    emit_guard;
    a->is_const_op = false;
    ...
    LLVMValueRef call_args[] = { _llvalue((enode)L), _llvalue((enode)R) };
    LLVMValueRef result = LLVMBuildCall2(B, fn_ty, fn, call_args, 2, "math2_r");
    return with_value(result, enode(mod, a, autype, elem_au, loaded, true));
}

AU_EXPORT enode aether_e_stack_array(aether a, etype elem_type, i32 count) {
    emit_guard;
    a->is_const_op = false;
    ...
    return with_value(alloc, enode(mod, a, loaded, false, autype, arr_au));
}

AU_EXPORT enode aether_e_stack_array_dynamic(aether a, etype elem_type, enode count_expr) {
    emit_guard;
    a->is_const_op = false;
    ...
    LLVMValueRef count_val = _llvalue((enode)loaded_ct);
...
AU_EXPORT enode aether_e_direct_cast(aether a, enode input, etype target) {
    emit_guard;
    a->is_const_op = false;
...
AU_EXPORT enode efunc_fptr(efunc f) {
    ...
    return with_value(_llvalue((enode)f), enode(mod, f->mod, autype, func_ptr->autype, loaded, true));
}
...
enode enode_deref(enode n) {
    ...
    etype typed = canonical(n);
    Au_t src = typed->autype->src;
    ...
    if (src->elements > 0)
        return with_value(_llvalue((enode)n), enode(mod, a, autype, src, loaded, false));
    return with_value(LLVMBuildLoad2(B, _lltype_slot(src_etype), _llvalue((enode)n), "deref"), enode(mod, a, autype, src, loaded, true));
}

AU_EXPORT enode aether_e_not(aether a, enode L) {
    emit_guard;
    a->is_const_op = false;
    if (a->no_build) return e_noop(a, etypeid(bool));
    ...
    bool val_is_ptr = LLVMGetTypeKind(LLVMTypeOf(_llvalue((enode)L))) == LLVMPointerTypeKind;
    if (val_is_ptr || is_ptr(Lm) || is_class(Lm) || is_func_ptr((Au)Lm)) {
        // for pointers, compare with null
        result = LLVMBuildICmp(B, LLVMIntEQ, _llvalue((enode)L),
            LLVMConstNull(LLVMTypeOf(_llvalue((enode)L))), "ptr-not");
    } else if (is_float(Lm)) {
        // for floats, compare with 0.0 and return true if > 0.0
        result = LLVMBuildFCmp(B, LLVMRealOLE, _llvalue((enode)L),
                               LLVMConstReal(lltype(Lm), 0.0), "float-not");
    } else if (is_unsign(Lm)) {
        // for unsigned integers, compare with 0
        result = LLVMBuildICmp(B, LLVMIntULE, _llvalue((enode)L),
                               LLVMConstInt(lltype(Lm), 0, 0), "unsigned-not");
    } else {
        // for signed integers, compare with 0
        result = LLVMBuildICmp(B, LLVMIntSLE, _llvalue((enode)L),
                               LLVMConstInt(lltype(Lm), 0, 0), "signed-not");
    }
    return value(etypeid(bool), result);
}

AU_EXPORT enode enode_retain(enode mem) {
    aether a = au_active(mem->mod);
    etype mdl = (etype)evar_type((evar)mem);
    a->is_const_op = false;
    if (mdl->autype->is_class && !a->no_build) {
        efunc fn_hold = (efunc)u(efunc, find_member(etypeid(Au)->autype, "hold", AU_MEMBER_FUNC, 0, true));
        e_fn_call(a, fn_hold, a(mem), false, false);
    }
    return mem;
}

AU_EXPORT enode enode_emit_release(enode mem) {
    aether a = au_active(mem->mod);
    etype mdl = (etype)evar_type((evar)mem);

    a->is_const_op = false;
    if (mdl->autype->is_class && !a->no_build) {
        efunc fn_drop = (efunc)u(efunc, find_member(etypeid(Au)->autype, "drop", AU_MEMBER_FUNC, 0, true));
        e_fn_call(a, fn_drop, a(mem), false, false);
    }
    return mem;
}

AU_EXPORT Au_t aether_pop_scope(aether a) {
    emit_guard;
...
AU_EXPORT etype aether_return_type(aether a) {
    for (int i = len(a->lexical) - 1; i >= 0; i--) {
        Au_t ctx = (Au_t)a->lexical->origin[i];
        etype ctx_u = u(etype, ctx);
...
AU_EXPORT efunc aether_function(aether a, etype place, string ident, etype rtype, array args, u8 member_type, u32 traits, u8 operator_type) {
    Au_t context = place->autype;
    ...
    au->src = (Au_t)hold(rtype->autype);
    efunc f = efunc(mod, a, autype, au, used, true, loaded, true);
    return f;
}

etype aether_record(aether a, etype place, etype based, string ident, u32 traits) { sequencer
    Au_t au = def(place->autype, ident->chars, AU_MEMBER_TYPE, traits);
    au->context = (traits & AU_TRAIT_STRUCT) ? null : (based ? based->autype : etypeid(Au)->autype);
    ...
    etype n = etype(mod, a, autype, au);
    return n;
}

AU_EXPORT Au enode_literal_value(enode a, Au_t of_type) {
    if (!a) return null;
...
AU_EXPORT efunc aether_module_initializer(aether a) {
    emit_guard;
    ...
    if (_llvalue((enode)init)) {
        // exported for dlsym: split builds must not leave it hidden
        LLVMSetLinkage  (_llvalue((enode)init), LLVMExternalLinkage);
        LLVMSetVisibility(_llvalue((enode)init), LLVMDefaultVisibility);
#ifdef _WIN32
        // pe has no default export; this is visibility's counterpart there
        LLVMSetDLLStorageClass(_llvalue((enode)init), LLVMDLLExportStorageClass);
#endif
    }
    a->fn_init = hold(init);
    return init;
...
AU_EXPORT none aether_e_memcpy(aether a, enode _dst, enode _src, Au_t au_size) {
    emit_guard;
    if (a->no_build) return;
    a->is_const_op = false;
    LLVMTypeRef i8ptr = LLVMPointerTypeInContext(a->module_ctx, 0);
    LLVMValueRef dst  = LLVMBuildBitCast(B, _llvalue((enode)_dst), i8ptr, "dst");
    LLVMValueRef src  = LLVMBuildBitCast(B, _llvalue((enode)_src), i8ptr, "src");
...
AU_EXPORT enode aether_e_memop(aether a, enode dst, enode arg2, enode size, bool is_memcpy) {
    emit_guard;
    if (a->no_build) return e_noop(a, null);
    a->is_const_op = false;

    // a vector operand is its element memory (origin)
    Au_t dv9 = dst->autype, sv9 = arg2->autype;
    while (dv9 && dv9->member_type == AU_MEMBER_VAR) dv9 = dv9->src;
    while (sv9 && sv9->member_type == AU_MEMBER_VAR) sv9 = sv9->src;
    bool dst_vec = dv9 && au_is_vector(dv9);
    bool src_vec = is_memcpy && sv9 && au_is_vector(sv9);
    LLVMValueRef vdst0 = dst_vec ? vec_origin(a, dst) : null;
    LLVMValueRef vsrc0 = src_vec ? vec_origin(a, arg2) : null;
    ...
    LLVMValueRef vdst  = vdst0 ? vdst0 : LLVMBuildBitCast(B, _llvalue((enode)dst), i8ptr, "dst");
    LLVMValueRef vsz   = _llvalue((enode)size);

    if (LLVMTypeOf(vsz) != i64)
        vsz = LLVMBuildZExt(B, vsz, i64, "sz64");

    if (is_memcpy) {
        LLVMValueRef vsrc = vsrc0 ? vsrc0 : LLVMBuildBitCast(B, _llvalue((enode)arg2), i8ptr, "src");
        LLVMBuildMemCpy(B, vdst, 1, vsrc, 1, vsz);
    } else {
        LLVMValueRef vval = _llvalue((enode)arg2);
...
AU_EXPORT void aether_module_anchor(void) { }
