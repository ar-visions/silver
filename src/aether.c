...
static array expand_tokens(aether mod, array tokens, map expanding) {
    ...
    for (int i = 0; i < ln; i += skip) {
        ...
        if (m && b && eq(b, "(") && !get(expanding, mname)) {
            ...
            set(expanding, mname, A_bool(true));
...
array macro_expand(macro m, array args, map expanding) {
    ...
    set(expanding, m->mem->name, A_bool(true));
...
void model_init(model mdl) {
    ...
    if (!mdl->type && type && ((type->traits & A_TRAIT_PRIMITIVE) != 0 ||
        ...
        if (type == typeid(f32))
            ...
            model mdl_src = null;
            cstr src_name = null;
            string n = string(mdl->atype_src->name);
    ...
    if (mdl->type && mdl->type != LLVMVoidType()) { /// we will encounter errors with aliases to void
        ...
        //if (!cl || !cl->is_abstract) {
            ...
            if (res && res->size_bits) {
                mdl->size_bits      = res->size_bits;
                mdl->alignment_bits = res->alignment_bits;
            } else {
...
enode aether_e_element(aether e, enode array, A index) {
    ...
    model imdl = (model)(array->meta->elements[0] ? 
        (model)array->meta->elements[0] : emodel("A"));
    ...
    pairs (module_base->members, i) { // these members include our types, then we go through members on those
        ...
        pairs (mdl->members, ii) {
            ...
            if (func) {
                ...
                enode gvalue = get(en->global_values, f->mem->name);
                verify(gvalue, "value not found for enum %o", f->mem->name);
...
void fn_use(fn fn) {
    ...
    if (fn->args)
        pairs(fn->args->members, i) {
            ...
            arg_types[index++] =
                (isa(arg_mdl) == typeid(Class) || !arg_mdl->type) ? 
                    pointer(arg_mdl, null)->type : arg_mdl->type;
        }
...
void record_finalize(record rec) {
    ...
    each (a, record, r) {
        pairs(r->members, i) {
            ...
            print("member %o: %o = %i", mem->name, mem->mdl->mem->name, abi_size);
...
enode aether_e_typeid(aether e, model mdl) {
    ...
    verify(i_member, "type not publicizable in context: %o", name);
...
enode aether_e_const_array(aether e, model mdl, array a) {
    model atype = emodel("AType");
    model vector_type = pointer(mdl, null);
    if (!a || !len(a))
    ...
    for (i32 i = 0; i < ln; i++) {
        ...
        elems[i] = n->value;  // each is an AType*
    }
...
enode aether_e_create(aether e, model mdl, array meta, A args) {
    ...
    if (mdl->is_ref && mdl->src && isa(mdl->src) == typeid(structure) && static_a) {
        ...
        if (imap) {
            ...
            if (is_array) {
                int   ln = len(a);
                enode n  = e_operand(e, A_i64(ln), emodel("i64"), null);
                bool all_const = ln > 0;
                ...
                if (const_array) {
                    enode tru = e_operand(e, A_bool(ln), emodel("bool"), null);
        ...
        if (isa(mdl) == typeid(structure) || is_ref_struct) {
            ...
            if (imap) {
                ...
                for (int i = 0; i < field_count; i++) {
                    ...
                    } // string and cstr strike again -- it should be calling the constructor on string for this, which has been enumerated by A-type already
                    ...
                    AType type_m0 = isa(m0); // this is string for our struct member at 0 (string str2)
                    LLVMTypeRef tr = LLVMStructGetTypeAtIndex(rmdl->type, i);
                    LLVMTypeRef expect_ty = is_class(field_type) ?
                        pointer(field_type, null)->type : field_type->type;
...
enode aether_e_offset(aether e, enode n, A offset) {
    ...
    if (instanceof(offset, typeid(array))) {
        ...
        i = e_operand(e, A_i64(0), null, null);
...
emember aether_initializer(aether e) {
    ...
    e->mem_init = register_model(e, fn_init,
        f(string, "_%o_module_init", e->name), true);
    return e->mem_init;
}
...
static void register_vbasics(aether e) {
    ...
    typeid(glyph)->user  = register_model(e, mdl_glyph,  string("glyph"),  true);
    typeid(i8)->user     = register_model(e, mdl_i8,     string("i8"),     true);
    typeid(u8)->user     = register_model(e, mdl_u8,     string("u8"),     true);
    typeid(ref_u8)->user = register_model(e, mdl_ref_u8, string("ref_u8"), true);
    typeid(handle)->user = register_model(e, mdl_handle, string("handle"), true);
    typeid(symbol)->user = register_model(e, mdl_symbol, string("symbol"), true);
    typeid(cstr)->user   = register_model(e, mdl_cstr,   string("cstr"),   true);
}
...
static void register_basics(aether e) {
    ...
    string _AType_name = string("_AType");
    structure _AType = structure (mod, e, ident, _AType_name, members, null);
    register_model(e, _AType, _AType_name, false);

    model     _AType_ptr            = pointer       (_AType);
    model     _ATypes_ptr           = pointer       (_AType_ptr);
    
    typeid(AType) ->user = _AType_ptr; // without this, its double-registered in A_import
    typeid(ATypes)->user = _ATypes_ptr;

    register_model(e, _AType_ptr,  string("AType"),  false); // makes sense to auto-register if a name is given.
    register_model(e, _ATypes_ptr, string("ATypes"), false);
    ...
    _member->members = m(
        ...
        "type",             emem(_AType_ptr,   null, "type"),
    ...
    _meta_t->members = m(
        "count",  emem(_i64,         null, "count"),
        "meta_0", emem(_AType_ptr,   null, "meta_0"),
        "meta_1", emem(_AType_ptr,   null, "meta_1"),
        "meta_2", emem(_AType_ptr,   null, "meta_2"),
        "meta_3", emem(_AType_ptr,   null, "meta_3"),
        "meta_4", emem(_AType_ptr,   null, "meta_4"),
        "meta_5", emem(_AType_ptr,   null, "meta_5"),
        "meta_6", emem(_AType_ptr,   null, "meta_6"),
        "meta_7", emem(_AType_ptr,   null, "meta_7"),
        "meta_8", emem(_AType_ptr,   null, "meta_8"),
        "meta_9", emem(_AType_ptr,   null, "meta_9"));
    pop(e);
    ...
    push(e, _AType);
    _AType->members = m(
        "parent_type",     emem(_AType_ptr, null, "parent_type"),
        "name",            emem(_symbol,    null, "name"),
        "module",          emem(_symbol,    null, "module"),
        "sub_types",       emem(_handle,    null, "sub_types"), // needs to be _ATypeRef
        ...
        "vmember_type",    emem(_AType_ptr, null, "vmember_type"),
...
void aether_A_import(aether e, path lib) {
    ...
    for (num i = 0; i < ln; i++) {
        ...
        register_model(e, mdl, name, is_prim || is_enum || 
            eq(name, "A")     ||
            eq(name, "string") || 
            eq(name, "array") || 
            eq(name, "map"));
    ...
    pairs(processing, i) {
        ...
        mdl = pointer(mdl->ptr ? mdl->ptr : mdl); // we have to have these ref_i32's and such from A-type
        ...
        if    (is_class || is_struct) {
            ...
            for (int m = 0; m < atype->member_count; m++) {
                ...
                if ((amem->member_type & A_FLAG_CONSTRUCT) != 0) {
                    ...
                    member arg = earg(args, arg_mdl, null, "");
                    ...
                    member arg = earg(args, index_mdl, null, "");
                    set(args->members, string(""), arg);
                    mem_mdl    = fn(
                        mod,           e,
                        extern_name,   f(string, "%s_%o", atype->name, n),
                    ...
                    for (int a = (is_inst || is_f) ? 1 : 0; a < amem->args.count; a++) {
                        ...
                        member arg      = earg(args, arg_mdl, null, arg_mdl->name->chars);
...
void aether_init(aether e) {
    ...
    model main = Class(mod, e, ident, string("main"),
        src, emodel("A"), imported_from, path("A"), is_abstract, true);
    Class main_cl = main;
...
void create_schema(model mdl, string name) {
    ...
    structure mdl_type  = structure(
        mod, e, members, ident, type_name, map(hsize, 8), is_system, true, is_typeid, true);
    emember   type_mem  = register_model(e, mdl_type, type_name, use_now);
    ...
    each (acl, Class, cl) {
        ...
        pairs(cl->members, a) {
            ...
            fn f = m->mdl;
            use(f);
            // todo: this was for making methods global
            //model   ptr = pointer(m->mdl);
            //emember mm  = register_model(e, ptr, false); // init needs its fn->type set; and its not
        }
    }
    pop(e);
    finalize(mdl_type);
    ...
    structure _type_info = structure(mod, e, ident, info_name, members, map(),
    ...
    pop(e);
    register_model(e, _type_info, info_name, use_now);
    finalize(_type_info);

    token info_ptr_name = f(token, "%o_info", name);
    model type_info = model(mod, e, src, _type_info, is_system, true);
    register_model(e, type_info, info_ptr_name, use_now);

    interface access = from_module ? interface_public : interface_intern;
    emember type_i = emember(mod, e, name, f(string, "%o_i", name),
    ...
    register_model(e, type_alias, f(string, "%o_f", name, access, access), use_now);
...
enode aether_e_cmp(aether e, enode L, enode R) {
    ...
    if (Lt && Rt) { // ifdef functionality
        bool Lr = (Lt->traits & A_TRAIT_REALISTIC);
        bool Rr = (Rt->traits & A_TRAIT_REALISTIC);
        i32 diff = 0;
    ...
    if (Lc || Rc) {
        ...
        if (eq) {
            verify(eq->rtype == emodel("i32"), "compare function must return i32, found %o", eq->rtype->name);
...
enode aether_e_fn_call(aether e, fn fn, enode target, array args) { // we could support an array or map arg here, for args
    ...
    if (args) {
        ...
        each (args, A, value) {
            ...
            enode      conv  = e_create(e, f_arg->mdl, f_arg->meta, value);
...
A read_numeric(token a) {
    ...
    if (is_unsigned) {
        if (is_long == 0) {
            return (!a->mod->cmode || (!is_base10 && digits > 8) || (val > 0xffffffff)) ?
                A_u64((i64)val) : A_u32((i32)val);
        } else if (is_long == 1) {
