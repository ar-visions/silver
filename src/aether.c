...
enode aether_e_eq(aether a, enode L, enode R) {
    ...
    if (is_class(L) || is_class(R)) {
        ...
        Au_t fn = Au_find_member(L->au, "compare", AU_MEMBER_FUNC, true);
...
enode aether_e_eq_prev(aether a, enode L, enode R) {
    ...
    if (Lc || Rc) {
        ...
        Au_t au_f = Au_find_member(L->au, "eq", AU_MEMBER_FUNC, true);
        if (au_f) {
...
enode e_operand_primitive(aether a, Au op) {
    ...
    else if (instanceof(op, bool)) return uint_value(8,  op);
...
enode aether_e_fn_call(aether a, enode fn, array args) {
    ...
    if (args) {
        ...
        each (args, Au, arg_value) {
            ...
            Au_t arg_type = array_get(&fn->au->args, i);
            enode n   = instanceof(arg_value, enode);
            if (index == fmt_idx) {
                Au fmt = n ? instanceof(n->literal, const_string) : null;
    ...
    if (fmt_idx >= 0) {
        ...
        string fmt_str  = instanceof(fmt_node->literal, const_string);
...
static enode convertible(etype fr, etype to) {
    ...
    if (resolve(ma) == resolve(mb) && ref_level(ma) == ref_level(mb)) return (enode)true;
...
enode enode_access(enode target, string name) {
    aether a = target->mod;
    Au_t   m = Au_find_member(target->au, name->chars, 0, true);
    ...
    return enode(
        mod,    a,
        au,     Au_register_member(null, name->chars, ptr, AU_MEMBER_VAR, 0),
...
enode aether_e_typeid(aether a, etype mdl) {
    Au info = head(mdl);
    a->is_const_op = false; // we go from enode sometimes, which is NOT the base etype (its not brilliant to require etype be unique here but we circle back)
    verify(mdl->au->user->type_id, "schema instance not found for %o", mdl);
    return mdl->au->user->type_id;
}
...
enode aether_e_create(aether a, etype t, Au args) {
    ...
    static_array static_a = instanceof(args, static_array);
    enode   n    = null;
    enode ctr  = null;
    ...
    enode input = instanceof(args, enode);
    ...
    if (input) {
        ...
        enode fn = instanceof(fmem, enode);
    ...
    etype        f_alloc      = Au_find_member(Au_type, "alloc_new", AU_MEMBER_FUNC, false)->user;
    etype        f_initialize = Au_find_member(Au_type, "initialize", AU_MEMBER_FUNC, false)->user;
    enode        res;
    ...
    if (is_ptr(t) && is_struct(t->au->src) && static_a) {
        ...
        if (imap) {
            ...
            if (is_array) {
                ...
                for (int i = 0; i < ln; i++) {
                    enode node = instanceof(ar->origin[i], enode);
                ...
                if (const_array) {
                    etype f_push_vdata = Au_find_member(t->au, "push_vdata", AU_MEMBER_FUNC, true)->user;
                    e_fn_call(a, f_push_vdata, a(res, const_array, n));
                } else {
                    etype f_push = Au_find_member(t->au, "push", AU_MEMBER_FUNC, true)->user;
                    for (int i = 0; i < ln; i++) {
                        Au      aa = ar->origin[i];
        ...
        enode alloc = e_fn_call(a, f_alloc, a( e_typeid(a, t), _i32(1), metas_node ));
        alloc->au = t->au; // we need a general cast method that does not call function
        res = e_fn_call(a, ctr, a(alloc, input));
    } else {
        array   is_arr        = instanceof(args, array);
        ...
        if (is_struct(t) || is_ref_struct) {
            ...
            if (imap) {
                ...
                pairs(imap, i) {
                    string  k = i->key;
                    Au_t   t = isa(i->value);
                    Au_t   m = Au_find_member(rmdl->au, k->chars, AU_MEMBER_VAR, true);
                    i32 index = m->index;
...
enode aether_e_ternary(aether a, enode cond_expr, enode true_expr, enode false_expr) {
    aether mod = a;
    etype rmdl  = false_expr ? (etype)prefer_mdl(true_expr, false_expr) :  (etype)true_expr;
    ...
    subprocedure expr_builder)
{
    ...
    for (int i = 0; i < ln_conds; i++) {
        ...
        enode cond_node  = invoke(cond_builder, cond_obj);
        LLVMValueRef condition =
            e_create(a, elookup("bool"), cond_node)->value;
...
static void aether_finalize_types(aether a, etype module_init_fn) {
    ...
    members(module_base, mem) {
        ...
        if (is_class_t || is_struct_t) {
            arg_types(mdl->au, t)
                push(meta, t); // not pushing the user here, by design
        }

        int internal_size = 0;
        members(mdl->au, f) {
            if (f->access_type == interface_intern &&
                f->member_type == AU_MEMBER_VAR)
                internal_size += f->user->au->abi_size / 8;
        }

        map tmap = map(hsize, 8, unmanaged, true, assorted, true);
        mset(tmap, "ident",   string(mdl->au->ident));
        mset(tmap, "module",  e_fn_call(a, Au_find_member(
            au_lookup("Au"), "module", AU_MEMBER_FUNC, false)->user, a(a->name)));
        mset(tmap, "size",    _i32(mdl->au->abi_size / 8));
        mset(tmap, "traits",  _u64(mdl->au->traits));
        mset(tmap, "meta",    (Au)meta);
        mset(tmap, "isize",   _i32(internal_size));
        mset(tmap, "members", (Au)members_array);
        ...
        if (is_class_t) {
            etype parent = (etype)mdl->au->context->user;
        ...
        Au_t fn_push = Au_find_member(au_lookup("Au"), "push_type", AU_MEMBER_FUNC, false);
        e_fn_call(a, (enode)fn_push->user, a(target));
    }
...
static void build_entrypoint(aether a, etype module_init_fn) {
    ...
    Au_t argc = Au_register_member(au_f_main,  "argc", au_lookup("i32"),   AU_MEMBER_VAR,  0);
    Au_t argv = Au_register_member(au_f_main,  "argv", au_lookup("cstrs"), AU_MEMBER_VAR,  0);
    ...
    etype Au_engage = Au_find_member(au_lookup("Au"), "engage", AU_MEMBER_FUNC, false); // this is only for setting up logging now, and we should likely emit it after the global construction, and before the user initializers
...
etype etype_canonical(etype t) {
    Au_t au = t->au;
    if (au->is_typeid && au->src) au = au->src;
    while (au && au->src) {
...
none etype_init(etype t) {
    if (t->mod == null) t->mod = instanceof(t, aether);
    ...
    if (isa(t) == typeid(aether) || isa(t)->context == typeid(aether)) {
        ...
        for (int i = 0; i < ref->members.count; i++) {
            ...
            args(au_mem, mem)    array_qpush((array)&new_mem->args,    (Au)mem);
...
none etype_implement(etype t) {
    ...
    if (au->member_type == AU_MEMBER_VAR) {
        ...
        etype mt = null;
        if (is_class(t) && t->au != au_lookup("Au_t") && (t->au->traits & AU_TRAIT_SCHEMA) == 0) {
            ...
            evar schema_i = evar(mod, a, au, Au_register_member(
        ...
        ((enode)t)->target = au_target ? enode(mod, a, au, au_target ? au_target->src : null, arg_index, 0) : null;

        int index = 0;
        args(fn->au, arg) {
            arg->user = (etype)enode(mod, a, au, arg->src, arg_index, index);
            index++;
        }
...
enode aether_e_cmp(aether a, enode L, enode R) {
    ...
    if (Lt && Rt) {
        ...
        if (Lr || Rr) {
            f64 L_64 = (Lt == typeid(f32)) ? *(f32*)L->literal :
                       ...
                       ((f64)fault("unexpected float format"));
    ...
    if (Lc || Rc) {
        ...
        Au_t eq = Au_find_member(L->au, "compare", AU_MEMBER_FUNC, true);
        if (eq) {
...
none enode_release(enode mem) {
    ...
    enode f_dealloc = Au_find_member(mem->au, "dealloc", AU_MEMBER_FUNC, true)->user;
    if (f_dealloc) {
...
Au_t aether_pop_scope(aether a) {
    statements st = instanceof(top_scope(a)->user, statements);
    if (st) {
        members(st->au, mem) {
...
enode aether_function(aether a, string ident, etype rtype, array args, u8 member_type, u32 traits, u8 operator_type) {
    ...
    each(args, Au, arg) {
        Au_t a = au_arg(arg);
        Au_t argument = Au_register(au, null, AU_MEMBER_VAR, 0);
        argument->src = a;
        array_qpush((array)&au->args, (Au)argument);
    }
    au->src = hold(rtype->au);
