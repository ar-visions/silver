...
enode aether_lambda_fcall(aether a, enode mem, array user_args) {
    // access fn and ctx from lambda instance
    enode fn_ptr     = enode_value(access(mem, string("fn")));
    enode target_ptr = enode_value(access(mem, string("target")));
    enode ctx_ptr    = enode_value(access(mem, string("context")));
    enode rtype      = (enode)mem->meta->origin[0];
    enode lambda_fn  = (enode)mem->au->src->user;

    array args = array(alloc, 32, assorted, true);
    push(args, (Au)target_ptr);
...
enode enode_access(enode target, string name) {
    ...
    if (is_func((Au)m))
        return enode(mod, a, au, m, loaded, true, target, enode_value(target));
...
enode aether_e_create(aether a, etype mdl, Au args) {
    ...
    if (is_ptr(mdl) && is_struct(mdl->au->src) && static_a) {
        ...
        if (instanceof(args, array)) {
            ...
            enode metas_node = e_meta_ids(a, mdl->meta);
            res = e_fn_call(a, (enode)f_alloc, a( e_typeid(a, mdl), _i32(0), metas_node ));
        ...
        enode alloc = e_fn_call(a, (enode)f_alloc, a( e_typeid(a, mdl), _i32(0), metas_node ));
...
static void build_entrypoint(aether a, etype module_init_fn) {
    ...
    e_fn_call(a, (enode)Au_engage, a(argv->user));
    ...
    enode r = e_fn_call(a, (enode)fn_run->user, a(m));
    e_fn_return(a, (Au)r);
...
etype struct_from_vars(aether a, string name, array vars, bool is_system) {
    Au_t au    = def(a->au, name->chars, AU_MEMBER_TYPE, AU_TRAIT_STRUCT | (is_system ? AU_TRAIT_SYSTEM : 0));
    int  index = 0;
...
none etype_init(etype t) {
    ...
    if (is_func((Au)au)) {
        ...
        if (is_lambda(au)) {
            n_args += 1; // for context

            string context_name = f(string, "%s_%s_context", au->context->ident, au->ident);
            etype etype_context = struct_from_vars(a, context_name, (array)&au->members, false);
            ...
            arg_types[n_args - 1] = lltype(fn->context_node);
        }
...
none etype_implement(etype t) {
    ...
    if (!au->is_pointer && (is_rec(t) || au->is_union)) {
        ...
        bool multi_Au = len(cl) && cl->origin[0] == elookup("Au");
        int count = 0;
        int index = 0;
        each(cl, etype, tt) {
        ...
        if (is_lambda(au)) {
            LLVMValueRef value_ctx  = LLVMGetParam(fn->value, fn->au->args.count); // we have one more than args for the context
...
enode aether_function(aether a, etype place, string ident, etype rtype, array args, u8 member_type, u32 traits, u8 operator_type) {
    ...
    enode n = enode(mod, a, au, au, used, true, loaded, true);
