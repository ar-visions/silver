...
void aether_register_member(aether e, emember mem, bool register_now) {
    ...
    if (mem->mdl->name && eq(mem->mdl->name, "none")) {
        int test2 = 2;
        test2    += 2;
    }
...
void model_init(model mdl) {
    ...
    if (mdl->name && eq(mdl->name, "none")) {
        int test2 = 2;
        test2    += 2;
    }

    if ((e && e != mdl) && !mdl->is_global && !f && !args && !st && (!mdl->name || eq(mdl->name, "")) && !mdl->src) {
        print("useless model");
    }
...
void emember_set_value(emember mem, A value) {
    ...
    if (mem->mdl && eq(mem->mdl->name, "none")) {
        int test2 = 2;
        test2    += 2;
    }
}
...
enode aether_e_cmp(aether e, enode L, enode R) {
    ...
    if (Lc || Rc) {
        ...
        verify(Ls && Rs && (L->mdl == R->mdl), "struct type mismatch %o != %o", L->mdl->name, R->mdl->name);
