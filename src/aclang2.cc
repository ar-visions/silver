...
static Au_t map_clang_type(const QualType& qt, ASTContext& ctx, aether2 e, symbol use_name);
static Au_t create_record(RecordDecl* decl, ASTContext& ctx, aether2 e, std::string name);
static Au_t create_class(CXXRecordDecl* cxx, ASTContext& ctx, aether2 e, std::string qname);
static Au_t create_enum(EnumDecl* decl, ASTContext& ctx, aether2 e, std::string name);
static Au_t create_fn(FunctionDecl* decl, ASTContext& ctx, aether2 e, std::string name);
...
static void push_context(NamedDecl* decl, aether2 e) {
...
static void pop_context(NamedDecl* decl, aether2 e) {
    auto s = namespace_stack((NamespaceDecl*)decl);
...
static Au_t map_builtin_type(const BuiltinType* bt, aether2 e) {
    switch (bt->getKind()) {
...
static Au_t map_function_type(const FunctionProtoType* fpt, ASTContext& ctx, aether2 e) {
    ...
    for (unsigned i = 0; i < fpt->getNumParams(); i++) {
        ...
        if (param) {
            ...
            Au_t arg = Au_register(null, strdup(name_buf), AU_MEMBER_PROP, 0);
            arg->type = param;
...
static Au_t map_function_pointer(QualType pointee_qt, ASTContext& ctx, aether2 e, symbol use_name) {
    ...
    if (const FunctionProtoType* fpt = dyn_cast<FunctionProtoType>(pointee)) {
        ...
        Au_t ptr = Au_register_pointer(func, use_name);
        return ptr;
    }
    ...
    if (const FunctionNoProtoType* fnpt = dyn_cast<FunctionNoProtoType>(pointee)) {
        ...
        if (!fn->rtype) fn->rtype = au_lookup("none");
        Au_t ptr = Au_register_pointer(fn, use_name);
        return ptr;
    }
...
static Au_t map_clang_type(const QualType& qt, ASTContext& ctx, aether2 e, symbol use_name) {
    ...
    if (const TypedefType* tt = dyn_cast<TypedefType>(t)) {
        ...
        symbol new_name = use_name ? use_name : (name.length() ? strdup(name.c_str()) : null);
    ...
    if (const PointerType* pt = dyn_cast<PointerType>(type)) {
        ...
        Au_t ptr = Au_register_pointer(base, use_name);
    ...
    if (auto* L = dyn_cast<LValueReferenceType>(type)) {
        ...
        return Au_register_pointer(base, use_name);
    }
...
static void set_fields(RecordDecl* decl, ASTContext& ctx, aether2 e, Au_t rec) {
    ...
    if (decl->isCompleteDefinition() && !decl->isInvalidDecl() && !decl->isDependentType()) {
        ...
        for (auto field : decl->fields()) {
            ...
            Au_t m = Au_register_member(rec, strdup(field_name.c_str()), AU_MEMBER_PROP, 0);
            m->type = mapped;
...
static Au_t create_record(RecordDecl* decl, ASTContext& ctx, aether2 e, std::string name) {
    ...
    symbol n = has_name ? strdup(name.c_str()) : null;
...
static Au_t create_opaque_class(CXXRecordDecl* cxx, aether2 e) {
    std::string qname = cxx->getQualifiedNameAsString();
    symbol n = strdup(qname.c_str());
...
static Au_t create_class(CXXRecordDecl* cxx, ASTContext& ctx, aether2 e, std::string qname) {
    ...
    for (const auto& B : cxx->bases()) {
        ...
        Au_t m = Au_register_member(rec, strdup(bname), AU_MEMBER_PROP, 0);
        m->type = base_rec;
    ...
    for (auto* md : cxx->methods()) {
        ...
        Au_t fn = Au_register(rec, strdup(method_name.c_str()), AU_MEMBER_FUNC, 
        ...
        if (!md->isStatic()) {
            ...
            Au_t self_ptr = Au_register_pointer(self_type, null);
            Au_t self_arg = Au_register(null, strdup("self"), AU_MEMBER_PROP, 0);
        ...
        for (unsigned i = 0; i < md->getNumParams(); ++i) {
            ...
            Au_t ap = Au_register(null, strdup(pname.c_str()), AU_MEMBER_PROP, 0);
            ap->type = mt;
...
static Au_t create_enum(EnumDecl* decl, ASTContext& ctx, aether2 e, std::string name) {
    symbol n = name.length() ? strdup(name.c_str()) : null;
...
static Au_t create_fn(FunctionDecl* decl, ASTContext& ctx, aether2 e, std::string name) {
    ...
    for (unsigned i = 0; i < decl->getNumParams(); i++) {
        ...
        Au_t arg = Au_register(null, strdup(param_name.c_str()), AU_MEMBER_PROP, 0);
        arg->type = mt;
        array_qpush((array)&fn->args, (Au)arg);
    }
    ...
    if (decl->hasAttr<FormatAttr>()) {
        for (auto *attr : decl->specific_attrs<FormatAttr>()) {
            ...
            if (idx > 0 && idx <= (int)fn->args.count) {
                ...
                // Could mark as formatter - need a flag
            }
        }
    }
...
static Au_t create_namespace(NamespaceDecl* ns, ASTContext& ctx, aether2 e) {
    ...
    for (clang::NamedDecl* ndecl: s) {
        ...
        symbol name = strdup(ns_name.c_str());
        index++;
...
public:
    AetherDeclVisitor(ASTContext& context, aether2 ae) : ctx(context), e(ae) {}
...
class AetherASTConsumer : public clang::ASTConsumer {
    aether2 e;
public:
    AetherASTConsumer(aether2 e) : e(e) {}
...
public:
    AetherEmitAction(aether2 e) : e(e) {}
    ...
    CreateASTConsumer(clang::CompilerInstance &CI, llvm::StringRef InFile) override {
        ...
        public:
            CombinedConsumer(aether2 e, std::unique_ptr<clang::ASTConsumer> backend)
...
public:
    ...
    explicit MacroCollector(aclang2_cc instance)
...
path aether2_lookup_include(aether2 e, string include) {
...
path aether2_include(aether2 e, Au inc, ARef _instance) {
    aclang2_cc* instance = (aclang2_cc*)_instance;
