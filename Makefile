...
debug:
	$(MAKE) BUILD_ROOT=$(SILVER)/platform/native/debug build

release:
	$(MAKE) BUILD_ROOT=$(SILVER)/platform/native/release build

bootstrap:
ifeq ($(OS),Windows_NT)
	@case "$(BUILD_ROOT)" in *debug) \
		"$(SILVER)/bootstrap.bat" --debug;; \
	*) \
		"$(SILVER)/bootstrap.bat" --release;; \
	esac
else
	@case "$(BUILD_ROOT)" in *debug) \
		"$(SILVER)/bootstrap.sh" --debug;; \
	*) \
