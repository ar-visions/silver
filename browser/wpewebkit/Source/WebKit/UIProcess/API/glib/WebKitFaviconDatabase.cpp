...
#elif USE(TRINITY)
        auto images = icons.map([](PlatformImagePtr image) -> GRefPtr<WebKitImage> {
            auto pixels = image->bgraPremultiplied();
            auto size = image->size();
            auto bytes = adoptGRef(g_bytes_new(pixels.span().data(), pixels.size()));
            return adoptGRef(webkitImageNew(size.width(), size.height(), size.width() * 4, WTF::move(bytes)));
        });
