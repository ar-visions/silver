...
Gfx::BrokeredFont FontService::materialize_typeface(NonnullRefPtr<Gfx::TypefaceTrinity> typeface, String cache_key)
{
    ...
    if (Gfx::TypefaceTrinity::load_from_buffer(typeface->font_data(), ttc_index).is_error()) {
...
Gfx::BrokeredFont FontService::match_font(String const& family, u16 weight, u16 width, u8 slope)
{
    ...
    auto typeface = Gfx::TypefaceTrinity::match_family_style(family.bytes_as_string_view(), weight, width, slope);
...
Gfx::BrokeredFont FontService::match_font_for_code_point(u32 code_point, u16 weight, u16 width, u8 slope, bool prefer_color_emoji)
{
    ...
    auto typeface = Gfx::TypefaceTrinity::find_typeface_for_code_point(code_point, weight, width, slope, prefer_color_emoji);
    if (typeface.is_error() || !typeface.value())
        return {};
    return materialize_typeface(typeface.release_value().release_nonnull(), move(cache_key));
}
...
Optional<FlyString> FontService::resolve_generic_family(String const& family, u16 weight, u8 slope)
{
    ...
    if (wait_until_ready().is_error())
        return {};
    return Gfx::TypefaceTrinity::resolve_generic_family(family.bytes_as_string_view(), weight, slope);
}
