...
ErrorOr<NonnullRefPtr<Bitmap>> Bitmap::scaled(int const width, int const height, ScalingMode const scaling_mode) const
{
    ...
    auto surface = PaintingSurface::create_with_size({ width, height }, format(), alpha_type());
    {
        PainterTrinity painter(surface);
        painter.draw_bitmap(scaled_bitmap->rect().to_type<float>(), DecodedImageFrame { *this }, rect(), scaling_mode, {}, 1.0f, CompositingAndBlendingOperator::SourceOver);
    }
    surface->read_into_bitmap(*scaled_bitmap);
...
#else
    auto premultiply = alpha_type == AlphaType::Premultiplied;
    for (int y = 0; y < height(); ++y) {
        auto* p = scanline_u8(y);
        for (int x = 0; x < width(); ++x, p += 4) {
            ...
            for (int c = 0; c < 3; ++c) {
                if (premultiply)
                    p[c] = static_cast<u8>(p[c] * a / 255);
                else if (a != 0)
                    p[c] = static_cast<u8>(min(255, p[c] * 255 / a));
            }
        }
    }
#endif
