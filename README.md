...
origin repository (2017) [silver](https://github.com/krworks/silver) 

**silver is a native build language.** One `.ag` file defines a module, its `import` dependencies by its own git service (default) or from any url. silver defines a reflective component model which interoperates in C and CC.  using native, we may build to all devices as binary or shared library. `import` a git repository by `owner:name/commit` ; silver detects its build system, builds it with its own tools, and links the result. `import` a C header, its models and macros are made available at design time through an integrated libclang frontend. 
...
The same `*` appears in type declarations — `element*` as a parameter means "any subtype of element," signaling that the code is designed to operate generically. This keeps polymorphism visible and intentional: the vast majority of calls compile to direct invocations with zero overhead, and the developer decides exactly where dynamic dispatch is worth the cost. There is no baseline vtable overhead, just direct calls unless you say otherwise. 

This isn't just about dispatch cost: direct calls let LLVM see through the call boundary, enabling inlining, constant propagation, and auto-vectorization that indirect calls make impossible. Every vtable lookup is an optimization barrier — silver removes it by default and gives it back only when you need it.  The emphasis on explicit expression, an opt-in, makes this a more secure approach without the need to change base behavior.  The anti-pattern with the 'virtual' keyword is simply that it's an unknown to any user unless they inspect.  This makes code less readable.
...
### Special note
# designed for Audrey and Rebecca.  silver is an effort to produce more succinct code, inspiring more to enjoy the craft.
